...
# Development Setup (External Editor)

If you're like me and prefer using a real editor like VS Code instead of the in-game terminal, I've set this project up with [esbuild-bitburner-plugin](https://github.com/shyguy1412/esbuild-bitburner-plugin). This allows for a much nicer development experience with full TypeScript support and automatic syncing.

### Prerequisites

You'll need Node.js v20 or higher installed on your machine.

### Getting Started

1. Clone this repo.
2. Run `npm install` to grab all the dependencies.
3. Run `npm start` to kick off the build and watch server.

### Connecting to Bitburner

1. In Bitburner, head to **Settings** -> **Remote API**.
2. Enter port `12525` and hit **Connect**.
3. Any changes you save in your editor will now automatically push to the game.

### Project Structure

The folder structure here maps directly to the servers in-game:

- Scripts for your `home` server live in `servers/home/`.
- You can use subdirectories like `servers/home/Remote/` or `servers/home/Tasks/` to keep things organized.
- If you want to put scripts on other servers, just create a folder with the server's name, like `servers/n00dles/`.

### Features

- **Language Support**: I've got it set up to handle JS, JSX, TS, and TSX files.
- **React**: Imports for `React` and `ReactDOM` are automatically resolved to the versions running inside Bitburner.
- **Advanced Config**: If you want to dive deeper, check out `config.mjs`. You can enable things like `mirror`, `distribute`, `extensions`, and `remoteDebugging`. See the [plugin repo](https://github.com/shyguy1412/esbuild-bitburner-plugin) for the full scoop on what those do.

# Downloading the whole repository

If you manually `nano git-pull.js` from the terminal and copy the [contents of that script](https://raw.githubusercontent.com/saberzero1/bitburner-scripts/main/servers/home/git-pull.js), you should be able to run it once and download the rest of the files I use. Early-game, many will be useless because they are only enabled by late-game features, but they shouldn't give you too many problems just being there. Note that scripts now live under `servers/home/` in the repo, but `git-pull.js` has been updated to handle this automatically.
...
- `corporation.js` - (Requires SF3) Automates corporation management from startup through multiple investment rounds. Handles warehouse budgeting, employee optimization, boost materials, smart supply, and product development. Configurable via flags like `--self-fund`, `--corp-name`, and `--target-round`.
- `corp-helpers.js` - Helper library for `corporation.js`. Contains industry definitions, production formulas, warehouse space calculations, and employee distribution optimization — all extracted from the BitBurner source code.
- `darknet.js` - (Requires BN15) Main darknet orchestrator. Manages network exploration, server authentication, probe deployment, stasis links, migration charging (air gap crossing), and clue file scanning. The darknet is a 40×8 grid that mutates over time — this script handles all the chaos.
- `darknet-scan.js` - (Requires BN15) Terminal-based network visualizer for darknet. Displays topology, server details, authentication status, difficulty tiers, and password hints. Supports `--json` output for programmatic use and `--compact` for minimal output.
- `darknet-helpers.js` - Helper library for the darknet scripts. Contains password solvers, labyrinth solvers, log parsers, and difficulty estimation.
