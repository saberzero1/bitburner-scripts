...
export async function main(ns) {
    ...
    const targetIds = [
        FragmentId.Rep,
        FragmentId.Hacking2,
        FragmentId.Hacking1, // Basics, always want
        FragmentId.HackingSpeed, // Priority 2, improve hack EXP gain and income?
        FragmentId.HacknetMoney,
        FragmentId.HacknetCost, // Priority 3, hacknet good for lots of things?
        FragmentId.HackingGrow,
        FragmentId.HackingMoney, // Priority 4, improves growth, income for RAM from hacking?
        //etc...
    ];
    const allFragments = ns.stanek.fragmentDefinitions();
    const statFrags = allFragments.filter((frag) =>
        targetIds.includes(frag.id),
    );
    const boosterFrags = allFragments.filter(
        (frag) => frag.type == FragmentType.Booster,
    );
    ...
    for (let height = 3; height <= 5; height++)
        for (let width = height; width <= height + 1; width++) {
            const [score, plan] = await planFragments(
                ns,
                width,
                height,
                statFrags,
            ...
            for (const elem of [...plan.stats, ...plan.boosters])
                strFragments.push(
                    `{"id":${elem.fragment.id},"x":${elem.x},"y":${elem.y},"rotation":${elem.rot}}`,
                );
            ns.tprint(
...
async function planFragments(ns, width, height, statFrags, boosterFrags) {
    ...
    /** @type {Map<number, Placement[]>} */
    const boosterFragsPlacements = new Map(
        boosterFrags.map((frag) => [frag.id, []]),
    );
    /** @type {number[][][]} */
            ...
            for (let rot = 0; rot < 4; rot++) {
                for (const frag of [...statFrags, ...boosterFrags]) {
                    ...
                    if (coords.every(([x, y]) => x < width && y < height)) {
                        const key =
                            frag.type == FragmentType.Booster
                                ? boosterSeqn++
                                : statSeqn++; //`${frag.id}@${x}-${y}-${rot}`;
                        const placement = {
                            key,
                            fragment: frag,
                            x,
                            ...
                            rot,
                            coords,
                            adjacent: adjacents(width, height, coords),
                        };
    ...
    const canonicalCoords = [...new Array(width)].map((_, x) =>
        [...new Array(height)].map((_, y) => [x, y]),
    );
    for (const placement of placements) {
        placement.coords = placement.coords.map(
            ([x, y]) => canonicalCoords[x][y],
        );
        placement.adjacent = placement.adjacent.map(
            ([x, y]) => canonicalCoords[x][y],
        );
    }
    ...
    for (const placement of placements) {
        ...
        for (const other of boosterPlacements) {
            if (
                placement.coords.some((coord) => other.adjacent.includes(coord))
            )
                placement.adjacentBoosters.push(other.key);
            if (
                placement.coords.some((coord) => other.coords.includes(coord))
            ) {
    ...
    for (const placement of placements) {
        placement.adjacentBoosters = Int16Array.from(
            placement.adjacentBoosters,
        );
        placement.adjacentStats = Int16Array.from(placement.adjacentStats);
        placement.overlapWithBoosters = Int16Array.from(
            placement.overlapWithBoosters,
        );
        placement.overlapWithStats = Int16Array.from(
            placement.overlapWithStats,
        );
    }
    ...
    const statFragsKeys = [...statFrags]
        ...
        .map((frag) =>
            statFragsPlacements.get(frag.id).map((placement) => placement.key),
        );
    statFragsKeys[0] = statFragsKeys[0].filter((key) =>
        width == height
            ? statPlacements[key].rot == 0
            : statPlacements[key].rot == 0 || statPlacements[key].rot == 1,
    );
    ...
    const bestResult0 = [
        -Infinity,
        { stats: [...plan0.stats], boosters: [...plan0.boosters] },
    ];

    planStatsCount = 0;
    planBoostersCount = 0;
    const t1 = performance.now();
    const [score, plan] = await planStats(
        ns,
        statPlacements,
        boosterPlacements,
        statFragsKeys,
    ...
    tlog(
        ns,
        "DEBUG",
        "Computed Stanek plan. Prep work %.3fmsec, layout search %.3fmsec, %d planStats calls, %d planBoosters calls",
        t1 - t0,
        t2 - t1,
        planStatsCount,
        planBoostersCount,
...
async function planStats(
    ...
    bestResult,
    blockedBoosters,
    boosterStatAdjacencies,
) {
    planStatsCount++;
    if (planStatsCount % 100000 == 0) await ns.sleep(0); // Don't hang the game
    ...
    if (plan.stats.length > 0) {
        ...
        for (let i = 0; i < blockedBoosters.length; i++) {
            if (boosterStatAdjacencies[i] === 0)
                // No adjacent stat fragments => block
                blockedBoosters[i]++;
            else if (blockedBoosters[i] === 0)
                // Has adjacent stat fragments, and not blocked
                availableBoostersCount++;
        }

        const [boosterScore, boosterPlan] = planBoosters(
            plan,
            boosterPlacements,
            boosterStatAdjacencies,
            blockedBoosters,
        ...
        // Undo changes
        for (let i = 0; i < blockedBoosters.length; i++)
            if (boosterStatAdjacencies[i] === 0) blockedBoosters[i]--;
    ...
    if (statFragsKeys.length > 0) {
        for (const key of statFragsKeys[0]) {
            ...
            const [bestPlanScore, bestPlan] = await planStats(
                ns,
                statPlacements,
                boosterPlacements,
                statFragsKeys.slice(1),
                blockedStats,
                plan,
                bestResult,
                blockedBoosters,
...
function planBoosters(
    ...
    for (let i = startIdx; i < blockedBoosters.length; i++) {
        ...
        for (let j = 0; j < overlapWithBoosters.length; j++)
            if (blockedBoosters[overlapWithBoosters[j]]++ === 0)
                availableCount--; // Placement became blocked?

        // Find and score best plan that includes this fragment placement
        bestResult = planBoosters(
            plan,
            boosterPlacements,
            boosterStatAdjacencies,
            blockedBoosters,
        ...
        for (let j = 0; j < overlapWithBoosters.length; j++)
            if (--blockedBoosters[overlapWithBoosters[j]] === 0)
                availableCount++; // Placement became free?
    }
...
function coverage(x0, y0, rotation, fragment) {
    ...
    for (let i = 0; i < rotation; i++)
        shape = shape[0].map((_, y) =>
            shape.map((_, x) => shape[shape.length - 1 - x][y]),
        );

    return shape
        .map((row, y) =>
            row.map((filled, x) => (filled ? [x0 + x, y0 + y] : undefined)),
        )
        .flat()
...
function adjacents(width, height, coords) {
    const adjacent = [...new Array(width)].map(() =>
        [...new Array(height)].map(() => false),
    );
    ...
    for (const [x, y] of coords) adjacent[x][y] = false;

    return adjacent
        .map((col, x) => col.map((is, y) => (is ? [x, y] : undefined)))
        .flat()
        .filter((elem) => elem != undefined);
}
