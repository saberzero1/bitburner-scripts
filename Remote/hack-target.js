...
export async function main(ns) {
    ...
    if (sleepDuration < 0) {
        if (!silentMisfires)
            ns.toast(
                `Misfire: Hack started ${-sleepDuration} ms too late. ${JSON.stringify(ns.args)}`,
                "warning",
            );
        sleepDuration = 0;
    }
    ...
    if (loopingMode) hgwOptions.additionalMsec += duration * 3.0; // (duration * 4.0 (time-to-weaken) - duration)
    ...
    do {
        ...
        if (stolen == 0 && !silentMisfires)
            ns.toast(
                `Misfire: Hack stole 0 money. ${JSON.stringify(ns.args)}`,
                "warning",
            );
