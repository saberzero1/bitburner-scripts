...
export async function main(ns) {
    ...
    let maxBatches = 0; // (Set in command line args) The max number of batches this daemon will spool up to avoid running out of IRL ram
    ...
    let toolsByShortName = /**@returns{{[id: string]: Tool;}}*/ (() =>
        undefined)(); // Dictionary of tools keyed by tool short name
    ...
    let dictSourceFiles = /**@returns{{[bitNode: number]: number;}}*/ (() =>
        undefined)(); // Available source files
    let bitNodeMults = /**@returns{BitNodeMultipliers}*/ (() => undefined)();
    let bitNodeN = 1; // The bitnode we're in
    let haveTixApi = false,
        have4sApi = false; // Whether we have WSE API accesses
    ...
    async function getPlayerInfo(ns) {
        ...
        return (_cachedPlayerInfo = await getNsDataThroughFile(
            ns,
            `ns.getPlayer()`,
        ));
    }
    ...
    function playerHackSkill() {
        return _cachedPlayerInfo.skills.hacking;
    }

    function getPlayerHackingGrowMulti() {
        return _cachedPlayerInfo.mults.hacking_grow;
    }
    ...
    async function filesExist(ns, fileNames, hostname = undefined) {
        return await getNsDataThroughFile(
            ...
            "/Temp/files-exist.txt",
            [hostname ?? daemonHost, ...fileNames],
        );
    }

    let psCache =
        /**@returns{{[serverName: string]: ProcessInfo[];}}*/ (() => ({}))();
    ...
    function processList(ns, serverName, canUseCache = true) {
        let psResult = null;
        if (canUseCache) psResult = psCache[serverName];
    ...
    function reservedMoney(ns) {
        ...
        const fourSigmaCost =
            bitNodeMults.FourSigmaMarketDataApiCost * 25000000000;
        ...
        if (bitNodeN == 10 && playerMoney >= 10e15)
            // 10q - 10% the cost of the last sleeve
    ...
    async function startup(ns) {
        ...
        const competingDaemons = processList(
            ns,
            daemonHost,
            false /* Important! Don't use the (global shared) cache. */,
        ).filter((s) => s.filename == scriptName && s.pid != ns.pid);
        if (competingDaemons.length > 0) {
            // We expect only 1, due to this logic, but just in case, generalize the code below to support multiple.
            const daemonPids = competingDaemons.map((p) => p.pid);
            log(
                ns,
                `Info: Killing another '${scriptName}' instance running on home (pid: ${daemonPids} args: ` +
                    `[${competingDaemons[0].args.join(", ")}]) with new args ([${ns.args.join(", ")}])...`,
                true,
            );
            const killPid = await killProcessIds(ns, daemonPids);
            await waitForProcessToComplete_Custom(
                ns,
                getHomeProcIsAlive(ns),
                killPid,
            );
        ...
        ((targetsByExp = []),
            ...
            (farmXpReentryLock = []),
            (nextXpCycleEnd = []));
        ((loopsHackThreadsByServer = {}),
            (loopsByServer_Grow = {}),
            (loopsByServer_Weaken = {}));
        // Stock mode related caches
        ((serversWithOwnedStock = []),
            (shouldManipulateGrow = []),
            (shouldManipulateHack = []));
        ...
        dictSourceFiles = await getActiveSourceFiles_Custom(
            ns,
            getNsDataThroughFile,
        );
        log(
            ns,
            "The following source files are active: " +
                JSON.stringify(dictSourceFiles),
        );
        ...
        stockMode =
            ...
            !options["disable-stock-manipulation"];
        stockFocus =
            options["stock-manipulation-focus"] &&
            !options["disable-stock-manipulation"];
        if (bitNodeN == 8 && !options["disable-stock-manipulation"]) {
            if (!stockFocus)
                log(
                    ns,
                    `INFO: In BN8, enabling --stock-manipulation-focus by default.`,
                );
            stockFocus = true;
            stockMode = true;
        }
        useHacknetNodes =
            options.n ||
            options["use-hacknet-nodes"] ||
            options["use-hacknet-servers"];
        ...
        if (stockFocus) {
            // If the user explicitly requested to focus on stocks, ensure we start with as many targets as there are stock symbols
            maxTargets = Math.max(
                maxTargets,
                Object.keys(serverStockSymbols).length,
            );
            log(
                ns,
                `Defaulting --initial-max-targets to ${maxTargets} so that we may manipulate every stock (due to --stock-manipulation-focus flag)`,
            );
        }
        ...
        if (useHacknetNodes)
            log(ns, "-n - Using hacknet nodes to run scripts!");
        if (verbose) log(ns, "-v - Verbose logging activated!");
        if (runOnce) log(ns, "-o - Run-once mode activated!");
        if (stockMode)
            log(
                ns,
                "Stock market manipulation mode is active (now enabled by default)",
            );
        if (!stockMode)
            log(
                ns,
                "--disable-stock-manipulation - Stock manipulation has been disabled.",
            );
        if (stockFocus)
            log(
                ns,
        ...
        if (loopingMode) {
            log(
                ns,
                "--looping-mode - scheduled remote tasks will loop themselves",
            );
            // cycleTimingDelay = 0;
            // queueDelay = 0;
            if (recoveryThreadPadding == 1)
                recoveryThreadPadding = Math.max(
                    2,
                    Math.min(
                        10,
                        Math.ceil(200 / Math.max(10, cycleTimingDelay)),
                    ),
                );
        ...
        if (openTailWindows)
            log(
                ns,
                "Opening tail windows for helper scripts (run with --no-tail-windows to disable)",
            );
        ...
        const reqRam = (ram) =>
            homeServer.totalRam(/*ignoreReservedRam:*/ true) >= ram;
        ...
        asynchronousHelpers = [
            {
                ...
                shouldRun: () => reqRam(64),
                shouldTail: false,
            }, // Adds stats not usually in the HUD (nice to have)
            {
                name: "go.js",
                shouldRun: () => reqRam(64),
                minRamReq: 20.2,
                shouldTail: false,
            }, // Play go.js (various multipliers, but large dynamic ram requirements)
            {
                name: "stockmaster.js",
                shouldRun: () => reqRam(64),
                args: openTailWindows ? ["--show-market-summary"] : [],
            }, // Start our stockmaster
            {
                name: "hacknet-upgrade-manager.js",
                shouldRun: () => shouldUpgradeHacknet(),
                args: ["-c", "--max-payoff-time", "1h", "--interval", "0"],
                shouldTail: false,
            }, // One-time kickstart of hash income by buying everything with up to 1h payoff time immediately
            {
                name: "spend-hacknet-hashes.js",
                shouldRun: () => reqRam(64) && 9 in dictSourceFiles,
                args: [],
                shouldTail: false,
            }, // Always have this running to make sure hashes aren't wasted
            {
                name: "sleeve.js",
                shouldRun: () => reqRam(64) && 10 in dictSourceFiles,
            }, // Script to create manage our sleeves for us
            {
                name: "gangs.js",
                shouldRun: () => reqRam(64) && 2 in dictSourceFiles,
            }, // Script to create manage our gang for us
            {
                ...
                shouldRun: () =>
                    ...
                    reqRam(256 / 2 ** dictSourceFiles[4] && !studying), // Higher SF4 levels result in lower RAM requirements
            },
            {
                ...
                shouldRun: () =>
                    !options["disable-script"].includes("bladeburner.js") &&
                    reqRam(64) &&
                    7 in dictSourceFiles &&
                    bitNodeMults.BladeburnerRank != 0, // Don't run bladeburner in BN's where it can't rank up (currently just BN8)
            },
            {
                name: "darknet.js",
                shouldRun: () =>
        ...
        options["run-script"].forEach((s) =>
            asynchronousHelpers.push({ name: s }),
        );
        ...
        asynchronousHelpers.forEach(
            (helper) => (helper.runOptions = { temporary: false }),
        );
        asynchronousHelpers.forEach((helper) => (helper.isLaunched = false));
        asynchronousHelpers.forEach(
            (helper) => (helper.ignoreReservedRam = true),
        );
        if (openTailWindows)
            // Tools should be tailed unless they explicitly opted out in the config above
            asynchronousHelpers.forEach(
                (helper) => (helper.shouldTail ??= true),
            );
        ...
        periodicScripts = [
            // Buy tor as soon as we can if we haven't already, and all the port crackers
            {
                ...
                shouldRun: () =>
                    4 in dictSourceFiles && !allHostNames.includes("darkweb"),
            },
            {
                interval: 26000,
                ...
                shouldRun: () =>
                    4 in dictSourceFiles &&
                    (ownedCracks.length != 5 || shouldAcquireDarknet()),
                args: () =>
                    shouldAcquireDarknet() ? ["--include-darknet"] : [],
            },
            ...
            },
            {
                ...
                args: () => ["--budget", 0.5, "--reserve", reservedMoney(ns)],
            ...
            },
            {
                ...
                shouldRun: () =>
                    4 in dictSourceFiles &&
                    (_cachedPlayerInfo.factions.length > 0 ||
                        getPlayerMoney(ns) > 1e9) &&
            ...
            },
            {
                ...
                shouldRun: () =>
                    shouldImproveHacking() && getHostManagerBudget() > 0,
                args: () => [
                    "--budget",
                    getHostManagerBudget(),
                    "--absolute-reserve",
                    reservedMoney(ns),
                    // Mechanic to reserve more of our money the longer we've been in the BN. Starts at 0%, after 24h we should be reserving 92%.
                    "--reserve-by-time",
                    true,
                    "--reserve-by-time-decay-factor",
                    0.1,
                    "--reserve-percent",
                    0,
                    "--utilization-trigger",
                    "0",
                ], // Disable utilization-based restrictions on purchasing RAM
            },
            // Check if any new servers can be backdoored. If there are many, this can eat up a lot of RAM, so make this the last script scheduled at startup.
            {
                ...
                shouldRun: () => 4 in dictSourceFiles && playerHackSkill() > 10,
            }, // Don't do this until we reach hack level 10. If we backdoor too early, it's very slow and eats up RAM for a long time,
        ];
        periodicScripts.forEach((tool) => (tool.ignoreReservedRam = true));
        if (verbose)
            // In verbose mode, have periodic sripts persist their logs.
            periodicScripts.forEach(
                (tool) => (tool.runOptions = { temporary: false }),
            );
        // HACK TOOLS (run with many threads)
        hackTools = [
            {
                name: "/Remote/weak-target.js",
                shortName: "weak",
                threadSpreadingAllowed: true,
            },
        ...
        const shouldKickstartHackXp =
            playerHackSkill() < 500 && timeSinceLastAug < 600000 && reqRam(16); // RamReq ensures we don't attempt this in BN1.1
        ...
        if (maxTargets == 0) {
            ...
            maxTargets =
                2 + Math.round(networkStats.totalMaxRam / (500 * 1024));
            log(
                ns,
    ...
    function shouldImproveHacking() {
        return (
            0 !=
                bitNodeMults.ScriptHackMoneyGain *
                    bitNodeMults.ScriptHackMoney || // Check for disabled hack-income
            ...
            bitNodeN === 8
        ); // The exception is in BN8, we still want lots of hacking to take place to manipulate stocks, which requires this infrastructure
    }
    ...
    function getHostManagerBudget() {
        ...
        let budget = Math.max(
            ...
            options["max-purchased-server-spend"] *
                (moneySources?.sinceInstall?.hacking ?? 0) -
                serverSpend,
        ...
        if (bitNodeN == 8) {
            const liquid = Math.max(0, getPlayerMoney(ns) - reservedMoney(ns));
            const reserveForStocks = liquid * 0.5;
            budget = Math.min(budget, Math.max(0, liquid - reserveForStocks));
        }
        ...
        try {
            if (4 in dictSourceFiles && options["initial-study-time"] > 0) {
                ...
                try {
                    ...
                    log(
                        ns,
                        `INFO: Studying for ${studyTime} seconds to kickstart hack XP and speed up initial cycle times. (set --initial-study-time 0 to disable this step.)`,
                    );
                    const money = getPlayerMoney(ns);
                    const { CityName, LocationName, UniversityClassType } =
                        ns.enums;
                    if (money >= 200000) {
                        // If we can afford to travel, we're probably far enough along that it's worthwhile going to Volhaven where ZB university is.
                        log(
                            ns,
                            `INFO: Travelling to Volhaven for best study XP gain rate.`,
                        );
                        await getNsDataThroughFile(
                            ns,
                    ...
                    const university =
                        playerInfo.city == CityName.Sector12
                            ? LocationName.Sector12RothmanUniversity
                            : playerInfo.city == CityName.Aevum
                              ? LocationName.AevumSummitUniversity
                              : playerInfo.city == CityName.Volhaven
                                ? LocationName.VolhavenZBInstituteOfTechnology
                                : null;
                    if (!university)
                        log(
                            ns,
                            `WARN: Cannot study, because you are in city ${playerInfo.city} which has no known university, and you cannot afford to travel to another city.`,
                            false,
                            "warning",
                        );
                    else {
                        const course =
                            playerInfo.city == CityName.Sector12
                                ...
                                : UniversityClassType.algorithms; // Assume if we are still in Sector-12 we are poor and should only take the free course
                        log(
                            ns,
                            `INFO: Studying "${course}" at "${university}" because we are in city "${playerInfo.city}".`,
                        );
                        startedStudying = await getNsDataThroughFile(
                            ns,
                            `ns.singularity.universityCourse(ns.args[0], ns.args[1], ns.args[2])`,
                            null,
                            [university, course, false],
                        );
                        if (startedStudying)
                            ...
                            log(
                                ns,
                                `WARNING: Failed to study to kickstart hack XP: ns.singularity.universityCourse("${university}", "${course}", false) returned "false".`,
                                false,
                                "warning",
                            );
                    }
                    ...
                    log(
                        ns,
                        `WARNING: Caught error while trying to study to kickstart hack XP: ${getErrorInfo(err)}`,
                        false,
                        "warning",
                    );
                }
            ...
            for (const server of getAllServers().filter(
                (s) => !s.hasRoot() && s.canCrack(),
            ))
                await doRoot(ns, server);
            ...
            if (!xpOnly) {
                ...
                if (minCycleTime > maxXpTime * 1000)
                    return log(
                        ns,
                        `INFO: Skipping XP cycle because the best target (${xpTarget.name}) time to weaken (${formatDuration(minCycleTime)})` +
                            ` is greater than the configured --initial-hack-xp-time of ${maxXpTime} seconds.`,
                    );
                log(
                    ns,
                    `INFO: Running Hack XP-focused cycles for ${maxXpTime} seconds to further boost hack XP and speed up main hack cycle times. (set --initial-hack-xp-time 0 to disable this step.)`,
                );
                while (
                    maxXpCycles-- > 0 &&
                    Date.now() - start < maxXpTime * 1000
                ) {
                    let cycleTime = await farmHackXp(ns, 1, verbose, 1);
                    if (cycleTime) await ns.sleep(cycleTime);
                    else
                        return log(
                            ns,
                            "WARNING: Failed to schedule an XP cycle",
                            false,
                            "warning",
                        );
                    log(
                        ns,
                        `INFO: Hacked ${xpTarget.name} for ${cycleTime.toFixed(1)}ms, (${Date.now() - start}ms total) of ${maxXpTime * 1000}ms`,
                    );
                }
            }
        } catch {
            log(
                ns,
                "WARNING: Encountered an error while trying to kickstart hack XP (low RAM issues perhaps?)",
                false,
                "warning",
            );
        } finally {
    ...
    function whichServerIsRunning(ns, scriptName, canUseCache = true) {
        for (const server of getAllServers()) {
            ...
            if (matches.length >= 1) return [server.name, matches[0].pid];
        }
        return [null, null];
    }
    ...
    async function runStartupScripts(ns) {
        ...
        for (const script of asynchronousHelpers.filter((s) => !s.isLaunched)) {
            if (!(await tryRunTool(ns, getTool(script)))) continue; // We may have chosen not to run the script for a number of reasons. Proceed to the next one.
    ...
    async function runPeriodicScripts(ns) {
        ...
        for (const script of periodicScripts) {
            ...
            if (await tryRunTool(ns, getTool(script)))
                if (++launched > 1)
                    // Try to run the task
                    await ns.sleep(1); // If we successfully launch more than 1 script at a time, yeild execution a moment to give them a chance to complete, so many aren't all fighting for temp RAM at the same time.
        }
        ...
        if (9 in dictSourceFiles && !options["disable-spend-hashes"]) {
            // See if we have a hacknet, and spending hashes for money isn't disabled
            if (
                homeServer.getMoney() <
                    options["spend-hashes-for-money-when-under"] && // Only if money is below the configured threshold
                homeServer.ramAvailable(/*ignoreReservedRam:*/ true) >= 5.6
            ) {
                // Ensure we have spare RAM to run this temp script
                await runCommand(
                    ns,
    ...
    const funcResultOrValue = (fnOrVal) =>
        fnOrVal instanceof Function ? fnOrVal() : fnOrVal;
    ...
    async function tryRunTool(ns, tool) {
        if (options["disable-script"].includes(tool.name)) {
            // Ensure the script hasn't been disabled
            if (verbose)
                log(
                    ns,
                    `Tool ${tool.name} was not launched as it was specified with --disable-script`,
                );
            return false;
        }
        if (tool.shouldRun != null && !(await tool.shouldRun())) {
            // Check the script's own conditions for being run
            if (verbose)
                log(
                    ns,
                    `INFO: Tool ${tool.name} was not launched as its shouldRun() function returned false.`,
                );
            return false;
        }
        ...
        let [runningOnServer, runningPid] = whichServerIsRunning(
            ns,
            tool.name,
            false,
        );
        if (runningOnServer != null) {
            ...
            if (verbose)
                log(
                    ns,
                    `INFO: Tool ${tool.name} is already running on server ${runningOnServer} as pid ${runningPid}.`,
                );
            return true;
        }
        ...
        const runResult = lowHomeRam
            ? await arbitraryExecution(
                  ...
                  1,
                  args,
                  getServerByName(backupServerName).hasRoot()
                      ? backupServerName
                      : daemonHost,
              )
            : await exec(ns, tool.name, daemonHost, tool.runOptions, ...args);
        if (runResult) {
            [runningOnServer, runningPid] = whichServerIsRunning(
                ns,
                tool.name,
                false,
            );
            ...
            log(
                ns,
                `INFO: Ran tool: ${tool.name} ` +
                    (args.length > 0
                        ? `with args ${JSON.stringify(args)} `
                        : "") +
                    (runningPid
                        ? `on server ${runningOnServer} (pid ${runningPid}).`
                        : "but it shut down right away."),
            );
            if (tool.shouldTail == true && runningPid) {
                log(
                    ns,
                    `Tailing Tool: ${tool.name}` +
                        (args.length > 0
                            ? ` with args ${JSON.stringify(args)}`
                            : "") +
                        ` on server ${runningOnServer} (pid ${runningPid})`,
            ...
            log(
                ns,
                `WARN: Tool could not be run on ${lowHomeRam ? "any host" : errHost} at this time (likely due to insufficient RAM. Requires: ${formatRam(tool.cost)} ` +
                    (lowHomeRam
                        ? ""
                        : `FREE: ${formatRam(errHost.ramAvailable(/*ignoreReservedRam:*/ true))})`) +
                    `: ${tool.name} [${args}]`,
                false,
                lowHomeRam ? undefined : "warning",
            );
        }
        return false;
    }
    ...
    async function exec(
        ...
        let fnRunScript = () =>
            ns.exec(script, hostname, numThreadsOrOptions, ...args);
        ...
        if (hackTools.some((h) => h.name === script)) return fnRunScript();
        // Otherwise, run with auto-retry to handle e.g. temporary ram issues
        let p;
        const pid = await autoRetry(
            ns,
            async () => {
                p = fnRunScript();
                return p;
            ...
            (p) => {
                if (p == 0)
                    log(
                        ns,
                        `WARNING: pid = ${p} after trying to exec ${script} on ${hostname}. Trying again...`,
                        false,
                        "warning",
                    );
                return p !== 0;
            ...
            () =>
                new Error(
                    `Failed to exec ${script} on ${hostname}. ` +
    ...
    async function doRoot(ns, server) {
        ...
        const pid = await exec(
            ns,
            getFilePath("/Tasks/crack-host.js"),
            daemonHost,
            { temporary: true },
            server.name,
        );
        ...
        do {
            ...
            try {
                ...
                if (!allHelpersRunning && loops % 60 == 0)
                    // If we have not yet launched all helpers see if any are now ready to be run (launch may have been postponed while e.g. awaiting more home ram, or TIX API to be purchased)
                ...
                if (!(4 in dictSourceFiles) && homeRam < 64) {
                    ...
                    if (homeRam == 8)
                        // Note: getAllServersByTargetOrder should be sorting by
                        maxTargets = maxPreppingAtMaxTargets = 1;
                    // Periodically provide a hint to buy more home RAM asap
                    if (loops % 600 == 0)
                        log(
                            ns,
                            `Reminder: Daemon.js can do a lot more if you have more Home RAM. Right now, you must buy this yourself.` +
                                `\n  Head to the "City", visit [alpha ent.] (or other Tech store), and purchase at least 64 GB as soon as possible!` +
                                `\n  Also be sure to purchase TOR and run "buy -a" from the terminal until you own all hack tools.`,
                            true,
                            "info",
                        );
                }
                ...
                if (loops % 60 == 0) {
                    ...
                    if (verbose || loops % 600 == 0) {
                        const targetsLog =
                            "Targetting Order: (* = prepped, ✓ = hackable)\n  " +
                            targetingOrder
                                ...
                                .map(
                                    (s) =>
                                        ...
                                        (xpOnly
                                            ? `Exp: ${formatNumberShort(s.getExpPerSecond(), 4)}/sec`
                                            : `(${formatMoney(s.getMoneyPerRamSecond(), 4)}/ram.sec),`) +
                                        ...
                                        !serverStockSymbols[s.name]
                                            ? ""
                                            : ` Sym: ${serverStockSymbols[s.name]} Owned: ${serversWithOwnedStock.includes(s.name)} ` +
                ...
                const prepping = n(),
                    preppedButNotTargeting = n(),
                    targeting = n(),
                    notRooted = n(),
                    cantHack = n(),
                    cantHackButPrepped = n(),
                    cantHackButPrepping = n(),
                    noMoney = n(),
                    failed = n(),
                    skipped = n();
                let lowestUnhackable = 99999;
                let maxPossibleTargets = targetingOrder.filter((s) =>
                    s.shouldHack(),
                ).length;
                ...
                if (firstUnpreppedServerIndex !== -1 && !stockMode)
                    targetingOrder.unshift(
                        targetingOrder.splice(firstUnpreppedServerIndex, 1)[0],
                    );
                ...
                for (let i = 0; i < targetingOrder.length; i++) {
                    ...
                    if (!server.shouldHack()) {
                    ...
                    } else if (await server.isTargeting()) {
                        ...
                        // Various conditions for which we'll postpone any additional work on servers
                        if (
                            xpOnly &&
                            ((nextXpCycleEnd[server.name] || 0) >
                                start - 10000 ||
                                (await server.isXpFarming()))
                        )
                        ...
                        !hackOnly &&
                        true == (await prepServer(ns, server))
                    ) {
                        // Returns true if prepping, false if prepping failed, null if prepped
                        if (server.previouslyPrepped)
                            log(
                                ns,
                                `WARNING ${server.prepRegressions++}: Server was prepped, but now at security: ${formatNumber(server.getSecurity())} ` +
                        ...
                        log(
                            ...
                            'Prep failed for "' +
                                server.name +
                                '" (RAM Utilization: ' +
                                (getTotalNetworkUtilization() * 100).toFixed(
                                    2,
                                ) +
                                "%)",
                        );
                        ...
                        let performanceSnapshot = optimizePerformanceMetrics(
                            ns,
                            server,
                        ); // Adjust the percentage to steal for optimal scheduling
                        if (server.actualPercentageToSteal() === 0) {
                            ...
                            log(
                                ...
                                'Targeting failed for "' +
                                    ...
                                    '" (RAM Utilization: ' +
                                    (
                                        getTotalNetworkUtilization() * 100
                                    ).toFixed(2) +
                                    "%)",
                            );
                            failed.push(server);
                    ...
                    if (
                        lowUtilizationIterations >= 5 &&
                        targeting.length == maxTargets &&
                        maxTargets < maxPossibleTargets
                    ) {
                        let network = getNetworkStats();
                        let utilizationPercent =
                            network.totalUsedRam / network.totalMaxRam;
                        if (utilizationPercent < lowUtilizationThreshold / 2) {
                            maxTargets++;
                            log(
                                ns,
                                `Increased max targets to ${maxTargets} since utilization (${formatNumber(utilizationPercent * 100, 3)}%) ` +
                ...
                if (
                    ...
                    cantHack.sort(function (a, b) {
                        ...
                        return diff != 0.0
                            ? diff
                            : b.getMoneyPerRamSecond() -
                                  a.getMoneyPerRamSecond(); // Break ties by sorting by max-money
                    });
                    // Try to prep them all unless one of our capping rules are hit
                    const maxCantHackPrep = Math.min(
                        cantHack.length,
                        maxPreppingAtMaxTargets,
                    );
                    for (let j = 0; j < maxCantHackPrep; j++) {
                ...
                if (lowUtilizationIterations > intervalsPerTargetCycle) {
                    ...
                    if (skipped.length > 0 && maxTargets < maxPossibleTargets) {
                        ...
                        recoveryThreadPadding = Math.min(
                            10,
                            recoveryThreadPadding * 1.5,
                        );
                    ...
                    if (actionTaken) {
                        log(
                            ns,
                            `${actionTaken} since utilization (${formatNumber(utilizationPercent * 100, 3)}%) has been quite low for ${lowUtilizationIterations} iterations.`,
                        );
                    ...
                    if (maxTargets > 1) {
                        ...
                        log(
                            ns,
                            `Decreased max targets to ${maxTargets} since utilization has been > ${formatNumber(maxUtilization * 100, 3)}% for 60 iterations and scheduling failed.`,
                        );
                    }
                    highUtilizationIterations = 0; // Reset the counter of high-utilization iterations
                }
                if (targeting.length - 1 > maxTargets) {
                    // Ensure that after a restart, maxTargets start off with no less than 1 fewer max targets
                    maxTargets = targeting.length - 1;
                    log(
                        ns,
                        `Increased max targets to ${maxTargets} since we had previous scripts targetting ${targeting.length} servers at startup.`,
                    );
                }
                ...
                if (xpOnly) {
                    ...
                    let time = await farmHackXp(ns, 1.0, verbose);
                    ...
                    for (let server of Object.keys(nextXpCycleEnd)
                        .filter(
                            (n) =>
                                nextXpCycleEnd[n] > start &&
                                skipped.some((s) => s.name == n),
                        )
                        .map((n) => getServerByName(n))) {
                        targeting.push(server);
                        skipped.splice(
                            skipped.findIndex((s) => s.name == server.name),
                            1,
                        );
                    }
                    ...
                    let freeRamToUse =
                        expectedRunTime < loopInterval // If expected runtime is fast, use as much RAM as we want, it'll all be free by our next loop.
                            ? 1 -
                              (1 - lowUtilizationThreshold) /
                                  (1 - utilizationPercent) // Take us just up to the threshold for 'lowUtilization' so we don't cause unecessary server purchases
                            : 1 -
                              (1 -
                                  maxUtilizationPreppingAboveHackLevel -
                                  0.05) /
                    ...
                    await farmHackXp(
                        ...
                        freeRamToUse,
                        verbose &&
                            (expectedRunTime > 10000 ||
                                lowUtilizationIterations % 10 == 0),
                        1,
                    );
                }
                ...
                if (
                    failed.length <= 0 &&
                    utilizationPercent < maxShareUtilization && // Only share RAM if we have succeeded in all hack cycle scheduling and have RAM to space
                    Date.now() - lastShareTime > options["share-cooldown"] && // Respect the share rate-limit if configured to leave gaps for scheduling
                    options["share"] !== false &&
                    options["no-share"] !== true &&
                    (options["share"] === true || network.totalMaxRam > 1024)
                ) {
                    // If not explicitly enabled or disabled, auto-enable share at 1TB of network RAM
                    ...
                    if (xpOnly)
                        maxThreads -= Math.floor(
                            getServerByName("home").ramAvailable() /
                                shareTool.cost,
                        ); // Reserve home ram entirely for XP cycles when in xpOnly mode
                    network = getNetworkStats(); // Update network stats since they may have changed after scheduling xp cycles above
                    utilizationPercent =
                        network.totalUsedRam / network.totalMaxRam;
                    let shareThreads = Math.floor(
                        (maxThreads *
                            (maxShareUtilization - utilizationPercent)) /
                            (1 - utilizationPercent),
                    ); // Ensure we don't take utilization above (1-maxShareUtilization)%
                    if (shareThreads > 0) {
                        if (verbose)
                            log(
                                ns,
                                `Creating ${shareThreads.toLocaleString("en")} share threads to improve faction rep gain rates. Using ${formatRam(shareThreads * 4)} of ${formatRam(network.totalMaxRam)} ` +
                        ...
                        await arbitraryExecution(
                            ...
                            null,
                            true,
                        ); // Note: Need a unique argument to facilitate multiple parallel share scripts on the same server
                ...
                log(
                    ns,
                    `WARNING: daemon.js Caught an error in the targeting loop: ${getErrorInfo(err)}`,
                    true,
                    "warning",
                );
                continue;
    ...
    let actualWeakenPotency = () =>
        bitNodeMults.ServerWeakenRate * weakenThreadPotency;

    // Get a dictionary from retrieving the same infromation for every server name
    async function getServersDict(ns, command) {
        return await getNsDataThroughFile(
            ns,
            `Object.fromEntries(ns.args.map(server => [server, ns.${command}(server)]))`,
            `/Temp/${command}-all.txt`,
            allHostNames,
        );
    }

    let dictInitialServerInfos =
        /**@returns{{[serverName: string]: globalThis.Server;}}*/ (() =>
            undefined)();
    let dictServerRequiredHackinglevels =
        /**@returns{{[serverName: string]: number;}}*/ (() => undefined)();
    ...
    let dictServerMaxMoney =
        /**@returns{{[serverName: string]: number;}}*/ (() => undefined)();
    let dictServerMaxRam = /**@returns{{[serverName: string]: number;}}*/ (() =>
        undefined)();
    let dictServerProfitInfo =
        /**@returns{{[serverName: string]: {gainRate: number, expRate: number}}}*/ (() =>
            undefined)();
    let dictServerGrowths =
        /**@returns{{[serverName: string]: number;}}*/ (() => undefined)();
    ...
    async function getStaticServerData(ns) {
        ...
        dictServerRequiredHackinglevels = await getServersDict(
            ns,
            "getServerRequiredHackingLevel",
        );
        dictServerNumPortsRequired = await getServersDict(
            ns,
    ...
    async function refreshDynamicServerData(ns) {
        ...
        dictServerMinSecurityLevels = await getServersDict(
            ns,
        ...
        await waitForProcessToComplete_Custom(ns, getHomeProcIsAlive(ns), pid);
        const analyzeHackResult = (dictServerProfitInfo = ns.read(
            "/Temp/analyze-hack.txt",
        ));
        if (!analyzeHackResult)
            log(
                ns,
                "WARNING: analyze-hack info unavailable. Will use fallback approach.",
            );
        else
            dictServerProfitInfo = Object.fromEntries(
                JSON.parse(analyzeHackResult).map((s) => [s.hostname, s]),
            );
        // Double home reserved ram once we reach the configured threshold
        if (
            homeServer &&
            homeServer.totalRam(true) >= options["double-reserve-threshold"]
        )
        ...
        haveTixApi =
            haveTixApi ||
            (await getNsDataThroughFile(ns, `ns.stock.hasTixApiAccess()`));
        have4sApi =
            have4sApi ||
            (await getNsDataThroughFile(ns, `ns.stock.has4SDataTixApi()`));
    ...
    class Server {
        ...
        resetCaches() {
            ...
            this._isPrepped =
                ...
                this._isTargeting =
                this._isXpFarming =
                this._percentStolenPerHackThread =
                this._files =
                    null;
        ...
        getMinSecurity() {
            return dictServerMinSecurityLevels[this.name] ?? 0;
        } // Servers not in our dictionary were purchased, and so undefined is okay
        getMaxMoney() {
            return dictServerMaxMoney[this.name] ?? 0;
        }
        getMoneyPerRamSecond() {
            return dictServerProfitInfo
                ? (dictServerProfitInfo[this.name]?.gainRate ?? 0)
                : (dictServerMaxMoney[this.name] ?? 0);
        }
        getExpPerSecond() {
            return dictServerProfitInfo
                ? (dictServerProfitInfo[this.name]?.expRate ?? 0)
                : (1 / dictServerMinSecurityLevels[this.name] ?? 0);
        }
        getMoney() {
        ...
        canCrack() {
            return ownedCracks.length >= this.portsRequired;
        }
        canHack() {
        ...
        shouldHack() {
            ...
            ); // Hack, but beats wasting 1.05 GB on ns.cloud.getServerNames()
        }
        ...
        async hasFile(fileName) {
            this._files ??= new Set(
                await getNsDataThroughFile(ns, "ns.ls(ns.args[0])", null, [
                    this.name,
                ]),
            );
        ...
        async isSubjectOfRunningScript(filter, useCache = true, count = false) {
            let total = 0;
            const hostNames = dictServerMaxRam
                ? allHostNames.filter(
                      (host) => (dictServerMaxRam[host] ?? 0) > 1.6,
                  )
                : allHostNames;
        ...
        async isPrepping(useCache = true) {
            this._isPrepping ??= await this.isSubjectOfRunningScript(
                (process) =>
                    process.args.length > 3 && process.args[3] == "prep",
                useCache,
            );
            return this._isPrepping;
        }
        async isTargeting(useCache = true) {
            this._isTargeting ??= await this.isSubjectOfRunningScript(
                (process) =>
                    process.args.length > 3 &&
                    process.args[3].startsWith("Batch"),
                useCache,
            );
            return this._isTargeting;
        }
        async isXpFarming(useCache = true) {
            this._isXpFarming ??= await this.isSubjectOfRunningScript(
                (process) =>
                    process.args.length > 3 &&
                    ["FarmXP", "weakenForXp", "growForXp"].includes(
                        process.args[3],
                    ),
                useCache,
            );
        ...
        targetGrowthCoefficientAfterTheft() {
            const percentStolen =
                this.getHackThreadsNeeded() *
                this.percentageStolenPerHackThread();
            const moneyAfterHack = Math.max(
                1,
                this.getMaxMoney() * (1 - percentStolen),
            );
            return this.getMaxMoney() / moneyAfterHack;
        }
        ...
        percentageStolenPerHackThread() {
            ...
            if (this._percentStolenPerHackThread !== null)
            ...
            const skillMult =
                (hackLevel - (this.requiredHackLevel - 1)) / hackLevel;
            const percentMoneyHacked =
                (difficultyMult *
                    skillMult *
                    _cachedPlayerInfo.mults.hacking_money *
                    bitNodeMults.ScriptHackMoney) /
                240;
            return (this._percentStolenPerHackThread = Math.min(
                1,
                Math.max(0, percentMoneyHacked),
            ));
        }
        ...
        getGrowThreadsNeeded() {
            ...
            return Math.max(
                0,
                Math.ceil(ns.growthAnalyze(this.name, targetGrowthCoefficient)),
            );
        }
        ...
        getGrowThreadsNeededAfterTheft() {
            ...
            return Math.max(
                recoveryThreadPadding > 1 ? 2 : 1,
                Math.ceil(
                    Math.min(
                        this.getMaxMoney(),
                        (this.cyclesNeededForGrowthCoefficientAfterTheft() /
                            this.serverGrowthPercentage()) *
                            recoveryThreadPadding,
                    ).toPrecision(14),
                ),
            );
        }
        ...
        getWeakenThreadsNeededAfterGrowth() {
            ...
            return Math.max(
                ...
                Math.ceil(
                    (
                        ((this.getGrowThreadsNeededAfterTheft() *
                            growthThreadHardening) /
                            actualWeakenPotency()) *
                        recoveryThreadPadding
                    ).toPrecision(14),
                ),
        ...
        hasRoot() {
            return (this._hasRootCached ??= ns.hasRootAccess(this.name));
        }
        isHost() {
        ...
        totalRam(ignoreReservedRam = false) {
            ...
            if (maxRam == null)
                throw new Error(
                    `Dictionary of servers' max ram was missing information for ${this.name}`,
                );
            // Complete HACK: but for most planning purposes, we want to pretend home has less ram to leave room for temp scripts to run
            if (
                !ignoreReservedRam &&
                (this.name == "home" ||
                    (this.name == backupServerName &&
                        dictServerMaxRam["home"] <= 16))
            )
                // Double-hack: When home RAM sucks (start of BN 1.1) reserve a server for helpers.
        ...
        ramAvailable(ignoreReservedRam = false) {
            return this.totalRam(ignoreReservedRam) - this.usedRam();
        }
        growDelay() {
    ...
    function getNetworkStats() {
        const rootedServers = getAllServers().filter((server) =>
            server.hasRoot(),
        );
        ...
        const totalMaxRam = rootedServers
            .map((s) => s.totalRam())
            .reduce((a, b) => a + b, 0);
        const totalFreeRam = Math.max(
            0,
            listOfServersFreeRam.reduce((a, b) => a + b, 0),
        ); // Hack, free ram can be negative due to "pretending" reserved home ram doesn't exist. Clip to 0
        return {
    ...
    function getOptimalPacedCycles(currentTarget) {
        return Math.min(
            ...
            Math.max(
                ...
                Math.floor(
                    (
                        currentTarget.timeToWeaken() / cycleTimingDelay
                    ).toPrecision(14),
                ) - 1,
            ),
        );
    ...
    function getScheduleLookaheadMs(currentTarget) {
        const minLookahead = Math.max(2000, cycleTimingDelay * 2);
        const targetWindow = Math.min(
            currentTarget.timeToWeaken(),
            cycleTimingDelay * maxBatches,
        );
        return Math.max(minLookahead, targetWindow);
    }
     ...
     * @param {number} optimalPacedCycles */
    function getPerformanceSnapshot(
        ...
        const growCost =
            ...
            getTool("grow").cost;
        const hackCost =
            currentTarget.getHackThreadsNeeded() * getTool("hack").cost;
        const ramCost = weaken1Cost + weaken2Cost + growCost + hackCost;
        const moneyPerCycle =
            currentTarget.actualPercentageToSteal() *
            currentTarget.getMaxMoney();
        const cycleSeconds = Math.max(1, currentTarget.timeToWeaken()) / 1000;
        const moneyPerRamSecond =
            ramCost > 0 ? moneyPerCycle / ramCost / cycleSeconds : 0;
        ...
        const simulatedRemainingRam = networkStats.listOfServersFreeRam
            .slice()
        ...
        while (canScheduleAnother && maxScheduled++ <= maxBatches) {
            for (const job of jobs) {
                ...
                const freeSlot =
                    simulatedRemainingRam /*.sort((a, b) => b - a)*/
        ...
        return {
            ...
            optimalPacedCycles: optimalPacedCycles,
            ...
            moneyPerRamSecond: moneyPerRamSecond,
        };
    }
    ...
    function optimizePerformanceMetrics(ns, currentTarget) {
        ...
        let increment = Math.ceil(
            (0.01 / percentPerHackThread).toPrecision(14),
        ); // Initialize the adjustment increment to be the number of hack threads to steal roughly 1%
        ...
        const optimalPacedCycles = getOptimalPacedCycles(currentTarget);
        while (++attempts < maxAdjustments) {
            ...
            if (
                adjustment === 0.0 ||
                Math.sign(adjustment) != lastAdjustmentSign
            ) {
                // Each time we change the direction of adjustments, slow the adjustment rate
                increment = Math.max(
                    1,
                    Math.floor((increment / 2.0).toPrecision(14)),
                );
                lastAdjustmentSign =
                    adjustment === 0.0
                        ? lastAdjustmentSign
        ...
        if (
            ...
            (verbose &&
                currentTarget.actualPercentageToSteal() !=
                    oldActualPercentageToSteal)
        ) {
            log(
                ns,
                `Tuned % to steal from ${formatNumber(oldActualPercentageToSteal * 100)}% (${oldHackThreads} threads) to ` +
        ...
        if (verbose && currentTarget.actualPercentageToSteal() == 0) {
            ...
            log(
                ns,
                `Insufficient RAM for min cycle: ${getTargetSummary(currentTarget)}`,
            );
        ...
        if (
            ...
            log(
                ns,
                `WARNING: Big difference between %ToSteal (${formatNumber(currentTarget.percentageToSteal * 100)}%) ` +
                    `and actual%ToSteal (${formatNumber(currentTarget.actualPercentageToSteal() * 100)}%) after ${attempts} attempts. ` +
                    `Min is: ${formatNumber(currentTarget.percentageStolenPerHackThread() * 100)}%`,
                false,
                "warning",
            );
        ...
        if (isOvershot(snapshot)) {
            ...
            currentTarget.percentageToSteal =
                (currentTarget.getHackThreadsNeeded() +
                    incrementalHackThreads) *
                currentTarget.percentageStolenPerHackThread();
            const comparisonSnapshot = getPerformanceSnapshot(
                currentTarget,
                networkStats,
                optimalPacedCycles,
            );
            currentTarget.percentageToSteal = lastP2steal;
            if (isOvershot(comparisonSnapshot)) return 0.0;
            return comparisonSnapshot.moneyPerRamSecond >=
                snapshot.moneyPerRamSecond * 0.98
                ? incrementalHackThreads
                : 0.0;
        }
        return 0.0;
    ...
    async function performScheduling(ns, currentTarget, snapshot) {
        ...
        if (maxCycles === 0)
            return log(
                ns,
                `WARNING: Attempt to schedule ${getTargetSummary(currentTarget)} returned 0 max cycles? ${JSON.stringify(snapshot)}`,
                false,
                "warning",
            );
        if (currentTarget.getHackThreadsNeeded() === 0)
            return log(
                ns,
                `WARNING: Attempted to schedule empty cycle ${maxCycles} x ${getTargetSummary(currentTarget)}? ${JSON.stringify(snapshot)}`,
                false,
                "warning",
            );
        ...
        const scheduleCutoff =
            Date.now() + getScheduleLookaheadMs(currentTarget);
        while (cyclesScheduled < maxCycles) {
            ...
            if (newBatchStart.getTime() > scheduleCutoff) break;
            ...
            if (cyclesScheduled > 0 && lastStart >= firstEnding) {
                if (verbose)
                    log(
                        ns,
                        `Had to stop scheduling at ${cyclesScheduled} of ${maxCycles} desired cycles (lastStart: ${lastStart} >= firstEnding: ${firstEnding}) ${JSON.stringify(snapshot)}`,
                    );
                break;
            }
        ...
        for (const schedObj of scheduledTasks) {
            for (const schedItem of schedObj.scheduleItems) {
                ...
                if (result == false) {
                    ...
                    log(
                        ns,
                        `WARNING: Scheduling failed for ${getTargetSummary(currentTarget)} ${discriminationArg} of ${cyclesScheduled} Took: ${Date.now() - start}ms`,
                        false,
                        "warning",
                    );
        ...
        if (verbose)
            log(
                ns,
                `Scheduled ${cyclesScheduled} x ${getTargetSummary(currentTarget)} Took: ${Date.now() - start}ms`,
            );
    ...
    let logSchedule = (ns, schedule, currentTarget) =>
        log(
            ns,
            `Current Time: ${formatDateTime(new Date())} Established a schedule for ${getTargetSummary(currentTarget)} from requested startTime ${formatDateTime(schedule.batchStart)}:` +
    ...
    function getFlagsArgs(
        toolName,
        target,
        allowLooping = true,
        overrideSilentMisfires = undefined,
    ) {
        const args = [];
        const silentMisfires =
            ...
            (toolName == "hack" &&
                bitNodeMults.ScriptHackMoneyGain *
                    bitNodeMults.ScriptHackMoney ==
                    0);
        if (["hack", "grow", "weak"].includes(toolName)) {
            if (["hack", "grow"].includes(toolName))
                args.push(
                    stockMode &&
                        ((toolName == "hack" && shouldManipulateHack[target]) ||
                            (toolName == "grow" &&
                                shouldManipulateGrow[target]))
                        ? 1
                        : 0,
                );
            else args.push(0);
    ...
    function getScheduleTiming(fromDate, currentTarget) {
        ...
        const t1_fireHackAt = new Date(
            hackOnly
                ? fromDate.getTime()
                : t1_hackResolvesAt.getTime() - hackTime,
        );
    ...
    function getScheduleObject(ns, batchTiming, currentTarget, batchNumber) {
        ...
        const schedHack = getScheduleItem(
            "hack",
            "hack",
        ...
        const schedWeak1 = getScheduleItem(
            "weak1",
            "weak",
        ...
        const schedGrow = getScheduleItem(
            "grow",
            "grow",
        ...
        const schedWeak2 = getScheduleItem(
            "weak2",
            "weak",
    ...
    async function arbitraryExecution(
        ns,
        tool,
        threads,
        args,
        ...
        const rootedServersByFreeRam = getAllServersByFreeRam().filter(
            (server) => server.hasRoot() && server.totalRam(igRes) > 1.6,
        );
        // Sort servers by total ram, and try to fill these before utilizing another server.
        const preferredServerOrder = getAllServersByMaxRam().filter(
            (server) => server.hasRoot() && server.totalRam(igRes) > 1.6,
        );
        if (useSmallestServerPossible)
            // If so-configured, fill up small servers before utilizing larger ones (can be laggy)
            preferredServerOrder.reverse();

        const homeCoreBonus = homeServer?.server?.cpuCores
            ? 1 + (homeServer.server.cpuCores - 1) / 16
            : 1;
        if (
            (tool.shortName == "grow" || tool.shortName == "weak") &&
            preferredServerName == "home" &&
            homeCoreBonus > 1
        )
            threads = Math.ceil(threads / homeCoreBonus);

        const homeIndex = preferredServerOrder.findIndex(
            (i) => i.name == "home",
        );
        if (homeIndex > -1) {
            // Home server might not be in the server list at all if it has insufficient RAM
            ...
            else preferredServerOrder.push(home); // Otherwise, send it to the back (reserve home for scripts that benefit from cores) and use only if there's no room on any other server.
        }
        ...
        while (
            -1 !==
            (hnNodeIndex = preferredServerOrder.indexOf(
                (s) =>
                    s.name.startsWith("hacknet-server-") ||
                    s.name.startsWith("hacknet-node-"),
            ))
        )
            anyHacknetNodes.push(
                ...preferredServerOrder.splice(hnNodeIndex, 1),
            );
        preferredServerOrder.push(
            ...anyHacknetNodes.sort((a, b) =>
                b.totalRam(igRes) != a.totalRam(igRes)
                    ? b.totalRam(igRes) - a.totalRam(igRes)
        ...
        if (
            ...
            preferredServerOrder[0].name != preferredServerName
        ) {
        ...
        if (verbose)
            log(
                ns,
                `Preferred Server ${preferredServerName ?? "(any)"} for ${threads} threads of ${tool.name} (use small=` +
                    `${useSmallestServerPossible})` +
        ...
        let computeMaxThreads = /** @param {Server} server */ function (
            server,
        ) {
        ...
        for (
            ...
            if (maxThreadsHere == remainingThreads) {
                for (let j = 0; j < preferredServerOrder.length; j++) {
                    ...
                    if (
                        remainingThreads <=
                        computeMaxThreads(nextMostPreferredServer)
                    ) {
            ...
            if (
                ...
                !(await tool.existsOnHost(targetServer))
            ) {
                let missing_scripts = [tool.name];
                if (
                    !(await doesFileExist(
                        ns,
                        getFilePath("helpers.js"),
                        targetServer.name,
                    ))
                )
                    missing_scripts.push(getFilePath("helpers.js")); // Some tools require helpers.js. Best to copy it around.
                if (tool.name.includes("/Tasks/contractor.js"))
                    // HACK: When home RAM is low and we're running this tool on another sever, copy its dependencies
                    missing_scripts.push(
                        getFilePath("/Tasks/contractor.js.solver.js"),
                        getFilePath("/Tasks/run-with-delay.js"),
                    );
                if (verbose)
                    log(
                        ns,
                        `Copying ${tool.name} and ${missing_scripts.length - 1} dependencies from ${daemonHost} to ${targetServer.name} so that it can be executed remotely.`,
                    );
                await getNsDataThroughFile(
                    ns,
                    `ns.scp(ns.args.slice(2), ns.args[0], ns.args[1])`,
                    "/Temp/copy-scripts.txt",
                    [targetServer.name, daemonHost, ...missing_scripts],
                );
            ...
            const pid = await exec(
                ...
                targetServer.name,
                {
                    ...
                    temporary: tool.runOptions.temporary ?? true,
                },
                ...(args || []),
            );
            if (pid == 0) {
                log(
                    ns,
                    `ERROR: Failed to exec ${tool.name} on server ${targetServer.name} with ${maxThreadsHere} threads`,
                    false,
                    "error",
                );
                return false;
            ...
            if (remainingThreads > 0) {
                if (!(allowThreadSplitting || tool.isThreadSpreadingAllowed))
                    break;
                if (verbose)
                    log(
                        ns,
                        `INFO: Had to split ${threads} ${tool.name} threads across multiple servers. ${maxThreadsHere} on ${targetServer.name}`,
                    );
                splitThreads = true;
            }
        }
        ...
        if (remainingThreads > 0 && threads < Number.MAX_SAFE_INTEGER) {
            const keepItQuiet =
                options["silent-misfires"] ||
                homeServer.ramAvailable(true) <= 16; // Don't confuse new users with transient errors when first getting going
            log(
                ns,
                `${keepItQuiet ? "WARN" : "ERROR"}: Ran out of RAM to run ${tool.name} on ${splitThreads ? "all servers (split)" : `${targetServer?.name} `}- ` +
                    `${threads - remainingThreads} of ${threads} threads were spawned.`,
                false,
                keepItQuiet ? undefined : "error",
            );
        }
    ...
    async function prepServer(ns, currentTarget) {
        ...
        if (currentTarget.isPrepped() || (await currentTarget.isTargeting()))
            return null;
        let start = Date.now();
        let now = new Date(start.valueOf());
        let weakenTool = getTool("weak"),
            growTool = getTool("grow");
        ...
        if (verbose)
            log(
                ns,
                `INFO: Need ${weakenThreadsNeeded} threads to weaken from ${currentTarget.getSecurity()} to ${currentTarget.getMinSecurity()}. There is room for ${weakenThreadsAllowable} threads (${currentTarget.name})`,
            );
        // Plan grow if needed, but don't bother if we didn't have enough ram to schedule all weaken threads to reach min security
        let growThreadsAllowable,
            growThreadsNeeded,
            growThreadsScheduled = 0;
        if (
            weakenThreadsNeeded < weakenThreadsAllowable &&
            (growThreadsNeeded = currentTarget.getGrowThreadsNeeded())
        ) {
            ...
            growThreadsAllowable =
                growTool.getMaxThreads(/*^*/ true /*^*/) - weakenThreadsNeeded; // Take into account RAM that will be consumed by weaken threads scheduled
            growThreadsScheduled = Math.min(
                growThreadsNeeded,
                growThreadsAllowable - 1,
            ); // Cap at threads-1 because we assume we will need at least one of these threads for additional weaken recovery
            if (verbose)
                log(
                    ns,
                    `INFO: Need ${growThreadsNeeded} threads to grow from ${currentTarget.getMoney()} to ${currentTarget.getMaxMoney()}. There is room for ${growThreadsAllowable} threads (${currentTarget.name})`,
                );
            // Calculate additional weaken threads which should be fired after the grow completes.
            let weakenForGrowthThreadsNeeded = Math.ceil(
                (
            ...
            const subscription =
                (growThreadsScheduled + weakenForGrowthThreadsNeeded) /
                growThreadsAllowable;
            if (subscription > 1) {
                // Scale down threads to schedule until we are no longer over-subscribed
                const scaleFactor =
                    (growThreadsScheduled + weakenForGrowthThreadsNeeded + 1) /
                    growThreadsAllowable; // +1 is because we will need to round weaken threads up, rather than down, to avoid under-recovery
                const scaledGrowThreads = Math.floor(
                    (growThreadsScheduled / scaleFactor).toPrecision(14),
                );
                const scaledWeakThreads = Math.ceil(
                    (weakenForGrowthThreadsNeeded / scaleFactor).toPrecision(
                        14,
                    ),
                );
                log(
                    ns,
                    `INFO: Insufficient RAM to schedule ${weakenForGrowthThreadsNeeded} required weaken threads to recover from ${growThreadsScheduled} prep grow threads. ` +
        ...
        let weakenThreadsScheduled = Math.min(
        ...
        if (weakenThreadsScheduled) {
            if (weakenThreadsScheduled < weakenThreadsNeeded)
                log(
                    ns,
                    `INFO: At this time, we only have enough RAM to schedule ${weakenThreadsScheduled} of the ${weakenThreadsNeeded} ` +
            ...
            prepSucceeding = await arbitraryExecution(
                ...
                [
                    ...
                    currentTarget.timeToWeaken(),
                    "prep",
                    ...getFlagsArgs("weak", currentTarget.name, false, true),
                ],
            );
            if (prepSucceeding == false)
                log(
                    ns,
                    `WARN: Failed to schedule ${weakenThreadsScheduled} prep weaken threads despite there ostensibly being room for ${weakenThreadsAllowable} (${currentTarget.name})`,
                );
        }
        ...
        if (prepSucceeding && growThreadsScheduled > 0) {
            prepSucceeding = await arbitraryExecution(
                ...
                [
                    currentTarget.name,
                    now.getTime(),
                    currentTarget.timeToGrow(),
                    "prep",
                    ...getFlagsArgs("grow", currentTarget.name, false, true),
                ],
                undefined,
                undefined,
                /*allowThreadSplitting*/ true,
            ); // Special case: for prep we allow grow threads to be split
            if (prepSucceeding == false)
                log(
                    ns,
                    `WARN: Failed to schedule ${growThreadsScheduled} prep grow threads despite there ostensibly being room for ${growThreadsAllowable} (${currentTarget.name})`,
                );
        }
        ...
        if (
            ...
            log(
                ns,
                `Prepping with ${weakenThreadsScheduled} weaken, ${growThreadsScheduled} grow threads (${weakenThreadsNeeded || 0} / ${growThreadsNeeded || 0} needed)` +
                    " ETA " +
                    Math.floor(
                        (currentTarget.timeToWeaken() + queueDelay) / 1000,
                    ) +
                    "s (" +
                    currentTarget.name +
                    ")" +
    ...
    async function farmHackXp(
        ns,
        fractionOfFreeRamToConsume = 1,
        verbose = false,
        numTargets = undefined,
    ) {
        if (!xpOnly)
            // Only use basic single-target hacking unless we're in XP mode
            return await scheduleHackExpCycle(
                ns,
                getBestXPFarmTarget(),
                fractionOfFreeRamToConsume,
                verbose,
                false,
        ...
        const serversByMaxRam = getAllServersByMaxRam().filter((s) =>
            s.hasRoot(),
        );
        ...
        if (jobHosts.length == 0)
            // Lower our standards if we're early-game and nothing qualifies
            jobHosts = serversByMaxRam.filter((s) => s.totalRam() >= 16);
        if (verbose)
            log(
                ns,
                `INFO: Potential Exp Job Hosts (${jobHosts.length}): ` +
                    jobHosts.map((s) => ` ${s.name}: ${s.totalRam()}`),
            );
        ...
        numTargets = Math.min(
            maxTargets,
            Math.floor(
                jobHosts.filter((s) => s.totalRam() > 0.01 * homeRam).length,
            ),
        ...
        const shouldRefreshTargets =
            !loopingMode &&
            (targetsByExp.length == 0 ||
                now - lastTargetsByExpRefresh > xpTargetCacheMs);
        const newTargets =
            loopingMode || shouldRefreshTargets
                ? getXPFarmTargetsByExp()
                : targetsByExp;
        if (!loopingMode) {
        ...
        } else if (loopingMode && targetsByExp.length < numTargets) {
            // In looping mode, we must keep the target-host mapping stable, we only revisit if we have capacity for new targets
            targetsByExp = targetsByExp.concat(
                ...newTargets
                    .filter((t) => !targetsByExp.includes(t)) // Only take targets not already in the target list
                    .slice(0, numTargets - targetsByExp.length),
            ); // Only take as many as we have are willing to target right now, allowing for the future target priority order to change
            ...
            for (let target of targetsByExp)
                if (!(target.name in jobHostMappings))
                    jobHostMappings[target.name] = jobHosts.filter(
                        (h) => !(h.name in Object.values(jobHostMappings)),
                    )[0];
            if (verbose) {
                log(
                    ns,
                    `INFO: Hack XP targetting order: ${targetsByExp.map((h) => `${h.name} (${formatNumber(h.getExpPerSecond())})`).join(",")}`,
                );
                log(
                    ns,
                    `INFO: Hack XP host (RAM) order: ${jobHosts.map((h) => `${h.name} (${formatRam(h.totalRam())})`).join(",")}`,
                );
            }
        }
        ...
        const etas = [];
        const totalServerRam = jobHosts.reduce(
            (total, s) => total + s.totalRam(),
            0,
        );
        ...
        if (verbose)
            log(ns, `INFO: farmHackXp has processed ${numTargets} targets.`);
    ...
    async function waitForCycleEnd(
        ...
        if (verbose)
            log(
                ns,
                `Waiting for last ${server.name} FarmXP process to complete... (ETA ${eta ? formatDuration(activeCycleTimeLeft) : "unknown"})`,
            );
    ...
    let loopsHackThreadsByServer = {},
        loopsByServer_Grow = {},
        loopsByServer_Weaken = {}; // Tracks active looping scripts
    /** @param {NS} ns
    ...
    async function scheduleHackExpCycle(
        ...
        const [logPrefix, toastLevel] = options["silent-misfires"]
            ? ["INFO:", undefined]
            : ["WARNING:", "warning"];
        try {
            ...
            let getStrAllocatedServer = () =>
                `allocated server ` +
                (allocatedServer == null
                    ? "(any server)"
                    : `${allocatedServer.name} with ${formatRam(allocatedServer.ramAvailable())} free RAM`);
            ...
            if (!loopRunning) {
                if (await server.isXpFarming()) {
                    if (verbose && activeCycleTimeLeft < -50)
                        ...
                        log(
                            ns,
                            `${logPrefix} ${server.name} FarmXP process is ` +
                                (eta
                                    ? `more than ${formatDuration(-activeCycleTimeLeft)} overdue...`
                ...
                if (threads == 0)
                    return log(
                        ns,
                        `${logPrefix} Cannot farm XP from ${server.name}, threads == 0 for ${getStrAllocatedServer()}`,
                        false,
                        toastLevel,
                    );
            }
            ...
            if (advancedMode) {
                ...
                effectiveHackThreads = Math.ceil(
                ...
                growThreadsNeeded = Math.ceil(
                    effectiveHackThreads * recoveryThreadPadding,
                ); // To hack for money, server must have at least 1$ per thread that "counts" for the steal (threads required to steal 100%)
                ...
                weakenThreadsNeeded = Math.ceil(
                    (securityHardeningToCombat / actualWeakenPotency()) *
                        recoveryThreadPadding,
                );
                const priorThreads = threads;
                if (singleServer)
                    // If set to only use a single server, free up the hack threads to make room for recovery threads on the same host
                    threads = Math.max(
                        0,
                        threads -
                            Math.ceil(
                                ((growThreadsNeeded + weakenThreadsNeeded) *
                                    1.75) /
                                    expTool.cost,
                            ),
                    );
                // Make room for recovery threads
                    ...
                    // If not in single-server mode, but the remaining hosts on the network can't fit 4 sets of grow + weaken recovery threads needed, we similarily need to reduce hack threads until required recovery threads can fit
                    const recoveryTool = getTool("weak"); // Weak and grow take the same ram, so we can just use this
                    const threadsOnExpToolHost =
                        allocatedServer == null
                            ? recoveryTool.getMaxThreads()
                            : allocatedServer.ramAvailable() /
                              recoveryTool.cost; // If we ran on the same host as expTool, how many threads would fit?
                    const globalThreads = getTool("weak").getMaxThreads(true); // How many threads would fit on the entire server?
                    const schedulableThreads =
                        globalThreads - threadsOnExpToolHost; // Subtract thread we cannot schedule due to the expTool using that entire host
                    const missingThreads =
                        growThreadsNeeded +
                        weakenThreadsNeeded -
                        schedulableThreads; // This is the number of threads we won't be able to schedule
                    if (missingThreads > 0)
                        // Note, we have to free up a slightly different number of hack threads, because the RAM cost is different
                        threads = Math.max(
                            0,
                            threads -
                                Math.ceil(
                                    (missingThreads * 1.75) / expTool.cost,
                                ),
                        );
                }
                ...
                if (threads <= effectiveHackThreads) {
                    log(
                        ns,
                        `INFO: Cannot farm XP from ${server.name} on ${getStrAllocatedServer()} in advanced mode: Hack threads=${threads} after releasing ` +
            ...
            let msToCycleEnd =
                queueDelay + (loopingMode ? expTime * 4.0 : expTime);
            nextXpCycleEnd[server.name] = now + msToCycleEnd; // Store how many MS before when this server's next cycle is expected to end
            const allowLoop =
                advancedMode /*&& singleServer*/ && allTargetsPrepped; // Allow looping mode only once all targets are prepped
            ...
            let success = true;
            if (!loopRunning) {
                // In looping mode, we only schedule one FarmXp (hack) loop, so skip this if one is already running
                const farmXpArgs = [
                    server.name,
                    scheduleTime,
                    expTime,
                    "FarmXP",
                    ...getFlagsArgs(expTool.shortName, server.name, allowLoop),
                ];
                if (verbose)
                    log(
                        ns,
                        `Scheduling ${threads}x ${expTool.shortName} on ${allocatedServer?.name ?? "(any)"} targetting ${server.name}`,
                    );
            ...
            if (success && allowLoop)
                loopsHackThreadsByServer[server.name] = threads;
            ...
            if (advancedMode) {
                ...
                const securityAtStart = Math.max(
                    1,
                    server.getSecurity(),
                    server.getMinSecurity(),
                );
                const securityAfterHack =
                    securityAtStart +
                    effectiveHackThreads * hackThreadHardening;
                const growTimeAtHackSecurity =
                    server.timeToGrow() * (securityAfterHack / securityAtStart);
                ...
                do {
                    const allWeakLoopsScheduled =
                        loopingMode &&
                        (loopsByServer_Weaken[server.name] ?? 0) >= 4;
                    ...
                    if (verbose)
                        log(
                            ns,
                            `Scheduling ${weakenThreadsNeeded}x weak on ${allocatedServer?.name ?? "(any)"} targetting ${server.name}`,
                        );
                    success &&= await arbitraryExecution(
                        ns,
                        getTool("weak"),
                        ...
                        [
                            ...
                            server.timeToWeaken(),
                            "weakenForXp",
                            ...getFlagsArgs("weak", server.name, allowLoop),
                        ],
                        singleServer ? allocatedServer?.name : null,
                        !singleServer,
                    );
                    if (success && allowLoop && !allWeakLoopsScheduled)
                        loopsByServer_Weaken[server.name] =
                            1 + (loopsByServer_Weaken[server.name] ?? 0);
                    if (verbose)
                        log(
                            ns,
                            `Looping ${weakenThreadsNeeded} x Weak starting in ${Math.round(scheduleWeak - now)}ms, ` +
                ...
                do {
                    const allGrowLoopsScheduled =
                        loopingMode &&
                        (loopsByServer_Grow[server.name] ?? 0) >= 4;
                    ...
                    if (verbose)
                        log(
                            ns,
                            `Scheduling ${growThreadsNeeded}x grow on ${allocatedServer?.name ?? "(any)"} targetting ${server.name}`,
                        );
                    success &&= await arbitraryExecution(
                        ns,
                        getTool("grow"),
                        ...
                        [
                            ...
                            server.timeToGrow(),
                            "growForXp",
                            ...getFlagsArgs("grow", server.name, allowLoop),
                        ],
                        singleServer ? allocatedServer?.name : null,
                        !singleServer,
                    );
                    if (success && allowLoop && !allGrowLoopsScheduled)
                        loopsByServer_Grow[server.name] =
                            1 + (loopsByServer_Grow[server.name] ?? 0);
                    if (verbose)
                        log(
                            ns,
                            `Looping ${growThreadsNeeded} x Grow starting in ${Math.round(scheduleGrow - now)}ms, ` +
                                `Tick: ${Math.round(msToCycleEnd)}ms on ${allocatedServer?.name ?? "(any server)"} targeting "${server.name}"`,
                        );
                    // The next loop (if any) we schedule needs to be offset by an additional +expTime
                ...
                const nextGrowFireMs =
                    (((scheduleGrow - now + server.timeToGrow()) %
                        msToCycleEnd) +
                        msToCycleEnd) %
                    msToCycleEnd;
                const nextWeakenFireMs =
                    (((scheduleWeak - now + server.timeToWeaken()) %
                        msToCycleEnd) +
                        msToCycleEnd) %
                    msToCycleEnd;
                if (verbose)
                    log(
                        ns,
                        `Exp Cycle: ${threads} x Hack in ${Math.round(scheduleTime - now + expTime)}ms, ` +
                ...
                log(
                    ns,
                    `In ${formatDuration(msToCycleEnd)}, ${threads} ${expTool.shortName} threads will fire against ${server.name} on ${allocatedServer?.name ?? "(any server)"} (for Hack Exp)`,
                );
            if (!success) {
    ...
    async function terminateLoopingScripts(ns) {
        const loopTools = [
            getTool("hack").name,
            getTool("grow").name,
            getTool("weak").name,
            getTool("manualhack").name,
        ];
        const loopPids = [];
        for (const host of allHostNames) {
            for (const process of processList(ns, host, false)) {
                if (!loopTools.includes(process.filename)) continue;
                if (
                    process.args.length > 0 &&
                    process.args[process.args.length - 1]
                )
                    loopPids.push(process.pid);
            }
        }
        if (loopPids.length > 0) {
            await killProcessIds(ns, loopPids);
            ((loopsByServer_Grow = {}),
                (loopsByServer_Weaken = {}),
                (loopsHackThreadsByServer = {}));
            nextXpCycleEnd = [];
    ...
    async function terminateScriptsManipulatingStock(ns, servers, toolName) {
        const processes = allHostNames.flatMap((hostname) =>
            processList(ns, hostname, false),
        );
        const stockManipArgIdx = 4;
        const loopArgIdx = 6;
        const problematicProcesses = processes.filter(
            (process) =>
                servers.includes(process.args[0]) &&
                (loopingMode ||
                    (toolName == process.filename &&
                        process.args.length > stockManipArgIdx &&
                        process.args[stockManipArgIdx])),
        );
        const problematicProcessesIds = problematicProcesses.map(
            (process) => process.pid,
        );
        if (problematicProcessesIds.length > 0) {
            log(
                ns,
                `INFO: Killing ${problematicProcessesIds.length} pids running ${toolName} with stock manipulation in the wrong direction.`,
            );
        ...
        const strGrow = getTool("grow").name,
            strWeak = getTool("weak").name,
            strHack = getTool("hack").name;
        problematicProcesses.forEach((process) => {
            if (
                toolName == strGrow &&
                1 ==
                    (process.args.length > loopArgIdx
                        ? process.args[loopArgIdx]
                        : 0)
            )
    ...
    function addServer(ns, server, verbose) {
        ...
        if (server.name == daemonHost) homeServer = server;
    ...
    function removeServerByName(ns, deletedHostName) {
        ...
        if (findIndex === -1)
            log(
                ns,
                `ERROR: Failed to find server with the name "${deletedHostName}" in the allHostNames list.`,
                true,
                "error",
            );
        else allHostNames.splice(findIndex, 1);
        ...
        if (findIndex === -1)
            log(
                ns,
                `ERROR: Failed to find server by name "${deletedHostName}".`,
                true,
                "error",
            );
        else {
            ...
            log(
                ns,
                `"${deletedHostName}" was found at index ${findIndex} of servers and removed leaving ${arrAllServers.length} items.`,
            );
        }
        resetServerSortCache(); // Reset the cached sorted lists of objects
    }
    ...
    async function buildServerList(ns, verbose = false) {
        ...
        for (const hostName of allHostNames.filter(
            (hostName) => !scanResult.includes(hostName),
        ))
            removeServerByName(ns, hostName);
        // Check if any of the servers are new to us
        const newServers = scanResult.filter(
            (hostName) => !allHostNames.includes(hostName),
        );
    ...
    const resetServerSortCache = () =>
        (_serverListByFreeRam =
            _serverListByMaxRam =
            _serverListByTargetOrder =
                undefined);
    ...
    function getAllServersByFreeRam() {
        return _sortServersAndReturn(
            (_serverListByFreeRam ??= getAllServers().slice()),
            function (a, b) {
    ...
    function getAllServersByMaxRam() {
        return _sortServersAndReturn(
            (_serverListByMaxRam ??= getAllServers().slice()),
            function (a, b) {
    ...
    function sortServerTieBreaker(a, b) {
        ...
        return a.name.startsWith(purchasedServersName) &&
            b.name.startsWith(purchasedServersName)
            ? Number("1" + a.name.substring(purchasedServersName.length + 1)) -
                  Number(
                      "1" + b.name.substring(purchasedServersName.length + 1),
                  )
        ...
        return _sortServersAndReturn(_serverListByTargetOrder, function (a, b) {
            ...
            if (xpOnly) {
                ...
                if (targetIdxA != -1 || targetIdxB != -1)
                    // If one or both are in the targetsByExp list, sort based on this
                    return targetIdxA == -1
                        ? 1
                        : targetIdxB == -1
                          ? -1
                          : targetIdxA < targetIdxB
                            ? -1
                            : 1;
            }
            if (stockFocus) {
                ...
                let stkCmp =
                    ...
                    serversWithOwnedStock.includes(b.name)
                        ...
                        : serversWithOwnedStock.includes(a.name)
                          ...
                          : 1;
                let manipA =
                    shouldManipulateGrow[a.name] ||
                    shouldManipulateHack[a.name]; // Whether we want to manipulate the stock associated with server A
                let manipB =
                    shouldManipulateGrow[b.name] ||
                    shouldManipulateHack[b.name]; // Whether we want to manipulate the stock associated with server A
                if (stkCmp == 0)
                    stkCmp = manipA == manipB ? 0 : manipA ? -1 : 1;
    ...
    async function getNsDataThroughFile(
        ns,
        command,
        fName,
        args = [],
    ...
    function getHomeProcIsAlive(ns) {
        return (pid) =>
            processList(ns, daemonHost, false).some((p) => p.pid === pid);
    }
    ...
    class Tool {
        ...
        async canRun(server) {
            return (
                (await server.hasFile(this.name)) &&
                server.ramAvailable(this.ignoreReservedRam) >= this.cost
            );
        }
        ...
        getMaxThreads(allowSplitting = undefined) {
            ...
            for (const server of getAllServers().filter((s) => s.hasRoot())) {
                // Note: To be conservative, we allow double imprecision to cause this floor() to return one less than should be possible,
                //       because the game likely doesn't account for this imprecision (e.g. let 1.9999999999999998 return 1 rather than 2)
                let serverRamAvailable = server.ramAvailable(
                    this.ignoreReservedRam,
                );
                ...
                const threadsHere = Math.max(
                    0,
                    Math.floor(serverRamAvailable / this.cost),
                );
                //log(server.ns, `INFO: Can fit ${threadsHere} threads of ${this.shortName} on ${server.name} (ignoreReserve: ${this.ignoreReservedRam})`)
                if (allowSplitting) maxThreads += threadsHere;
                else
                    maxThreadsOnSingleServer = Math.max(
                        maxThreadsOnSingleServer,
                        threadsHere,
                    );
            }
    ...
    async function buildToolkit(ns, allTools) {
        ...
        const toolsTyped = allTools.map(
            (toolConfig) => new Tool(toolConfig, toolCosts[toolConfig.name]),
        );
        toolsByShortName = Object.fromEntries(
            toolsTyped.map((tool) => [
                tool.shortName || hashToolDefinition(tool),
                tool,
            ]),
        );
    ...
    /** @returns {string} */
    const hashToolDefinition = (s) =>
        hashCode(s.name + (s.args?.toString() || ""));
        ...
        while (startupAttempts++ <= 5) {
            try {
                ...
                if (startupAttempts == 5)
                    log(
                        ns,
                        `ERROR: daemon.js Keeps catching a fatal error during startup: ${getErrorInfo(err)}`,
                        true,
                        "error",
                    );
                else {
                    log(
                        ns,
                        `WARN: daemon.js Caught an error during startup: ${getErrorInfo(err)}` +
                            `\nWill try again (attempt ${startupAttempts} of 5)`,
                        false,
                        "warning",
                    );
                    await ns.sleep(5000);
