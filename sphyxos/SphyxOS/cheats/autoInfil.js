...
export async function main(ns) {
    ...
    if (args.stop) {
        setTimeout(() => {
            var btn = Array.from(doc.querySelectorAll("button")).find((x) =>
                x.innerText.includes("Cancel"),
            );
...
function winGame() {
    ...
    const state =
        screen[Object.keys(screen).find((k) => k.startsWith("__reactFiber$"))];
...
function playGame() {
    ...
    if (!h4.length) {
        wnd.info = {
            screens,
            message: "no h4.length, calling endInfiltration()",
        };
        endInfiltration();
        return;
    }
    ...
    postTimeout = setTimeout(() => {
        ...
        if (companyEle) {
            if (infiltrationStart) {
                console.info(
                    `FAILED INFILTRATION - ${((new Date().valueOf() - infiltrationStart) / 1000).toFixed(1)} sec, last was ${last_title}`,
                );
                infiltrationStart = 0;
            }
            companyEle.click();
...
function acceptMoney(msg) {
    ...
    postTimeout = setTimeout(() => {
        ...
        if (btn) {
            if (infiltrationStart) {
                console.info(
                    `SUCCESSFUL INFILTRATION - ${((new Date().valueOf() - infiltrationStart) / 1000).toFixed(1)} sec: ${btn.innerText}`,
                );
                infiltrationStart = 0;
            }
...
function acceptReputation() {
    ...
    postTimeout = setTimeout(() => {
        ...
        var e = Array.from(doc.querySelectorAll('[role="combobox"]')).find(
