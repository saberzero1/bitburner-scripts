...
export async function main(ns) {
    ...
    while (true) {
        ...
        if (turn <= 1) results = await movePiece(getOpeningMove());
            ...
            switch (STYLE) {
                case 0: //Netburners
                    if ((results = await movePiece(getRandomCounterLib())))
                        break;
                    if ((results = await movePiece(getRandomLibAttack(88))))
                        break;
                    if ((results = await movePiece(getRandomLibDefend())))
                        break;
                    if ((results = await moveSnakeEyes(getSnakeEyes(6)))) break;
                    if ((results = await movePiece(getAggroAttack(2, 2, 2))))
                        break;
                    if ((results = await movePiece(disruptEyes()))) break;
                    if ((results = await movePiece(getDefPattern()))) break;
                    if (
                        (results = await movePiece(
                            getAggroAttack(3, 3, 3, 1, 6),
                        ))
                    )
                        break;
                    if ((results = await movePiece(getRandomBolster(2, 1))))
                        break;
                    if (
                        (results = await movePiece(
                            getAggroAttack(4, 7, 3, 1, 6),
                        ))
                    )
                        break;
                    if ((results = await movePiece(attackGrowDragon(1)))) break;
                    if ((results = await movePiece(getDefAttack(8, 20, 2))))
                        break;
                    if ((results = await movePiece(getRandomExpand()))) break;
                    if (
                        (results = await movePiece(
                            getRandomBolster(2, 1, false, 1),
                        ))
                    )
                        break;
                    if ((results = await movePiece(getRandomLibAttack())))
                        break;
                    if ((results = await movePiece(getRandomStrat()))) break;
                    if ((results = await moveWallBreaker(getWallBreaker())))
                        break;
                    if ((results = await moveWallBreaker(getWallBreaker(4))))
                        break;
                    if ((results = await moveWallBreaker(getWallBreaker(-1))))
                        break;
                ...
                case 2: //Mr. Mustacio - Slum Snakes
                    ...
                    if ((results = await movePiece(getRandomBolster(2, 1))))
                        break;
                    if (
                        (results = await movePiece(getDefAttack(4, 7, 3, 1, 6)))
                    )
                        break;
                    if ((results = await movePiece(attackGrowDragon(1)))) break;
                    if ((results = await movePiece(getRandomExpand()))) break;
                ...
                case 3: //Daedalus
                    ...
                    if ((results = await movePiece(getRandomBolster(2, 1))))
                        break;
                    if (
                        (results = await movePiece(getDefAttack(5, 7, 3, 2, 6)))
                    )
                        break;
                    if ((results = await movePiece(attackGrowDragon(1)))) break;
                    if ((results = await movePiece(getRandomExpand()))) break;
                ...
                case 4: //Tetrads
                    ...
                    if ((results = await movePiece(disruptEyes()))) break;
                    if ((results = await movePiece(getDefPattern()))) break;
                    if ((results = await movePiece(getAggroAttack(3, 4, 3))))
                        break;
                    if ((results = await movePiece(getRandomBolster(2, 1))))
                        break;
                    if ((results = await movePiece(getAggroAttack(5, 7, 3))))
                        break;
                    if ((results = await movePiece(attackGrowDragon(1)))) break;
                    if ((results = await movePiece(getRandomExpand()))) break;
                    if (
                ...
                case 6: //??????
                    ...
                    if ((results = await movePiece(getRandomBolster(2, 1))))
                        break;
                    if ((results = await movePiece(getDefAttack(5, 7, 3))))
                        break;
                    if ((results = await movePiece(attackGrowDragon(1)))) break;
                    if ((results = await movePiece(getRandomExpand()))) break;
                    if (
    ...
    function checkNewGame(gameInfo, passed) {
        if (
            gameInfo.type === "gameOver" ||
            (gameInfo.type === "pass" && passed)
        ) {
            if (!REPEAT) ns.exit();
            if (playAsWhite || ns.go.getOpponent() === "No AI")
                ns.go.resetBoardState("No AI", 13);
            else {
                try {
                    ...
                    for (let px = 0; px < patternSize && !abort; px++) {
                        ...
                        for (let py = 0; py < patternSize && !abort; py++) {
                            ...
                            if (
                                cx + px === 0 &&
                                cy + py === 0 &&
                                ![me, "*"].includes(patternCheck[px][py])
                            ) {
                                abort = true;
                                break;
                            }
        ...
        for (let x = 0; x < size - 1; x++)
            for (let y = 0; y < size - 1; y++) {
                if (
                    contested[x][y] === me ||
                    board[x][y] !== you ||
                    (validLibMoves[x][y] !== eyesToBreak && eyesToBreak > 0) ||
                    checked.has(JSON.stringify([x, y]))
                )
                    continue;
                //Is it the enemy, with 2 libs (we can kill) and we have not checked this spot and the chain is large enough
    ...
    function getRandomCounterLib() {
        ...
        for (const explore of friendlyToCheckForOpp) {
            ...
            if (fx > 0 && [me].includes(board[fx - 1][fy]))
                friendlyToCheckForOpp.add(JSON.stringify([fx - 1, fy]));
            if (fy > 0 && [me].includes(board[fx][fy - 1]))
        ...
        for (const explore of enemiesToSearch) {
            const [fx, fy] = JSON.parse(explore);
            if (fx < size - 1 && board[fx + 1][fy] === you)
                enemiesToSearch.add(JSON.stringify([fx + 1, fy]));
            if (fx > 0 && board[fx - 1][fy] === you)
                enemiesToSearch.add(JSON.stringify([fx - 1, fy]));
            if (fy > 0 && board[fx][fy - 1] === you)
                enemiesToSearch.add(JSON.stringify([fx, fy - 1]));
            if (fy < size - 1 && board[fx][fy + 1] === you)
                enemiesToSearch.add(JSON.stringify([fx, fy + 1]));
    ...
    function getEyeValueFull(checkx, checky, player) {
        ...
        if (checky < size - 1)
            explored.add(JSON.stringify([checkx, checky + 1]));
        if (checkx < size - 1 && checky < size - 1)
            explored.add(JSON.stringify([checkx + 1, checky + 1]));
        if (checkx > 0 && checky < size - 1)
            explored.add(JSON.stringify([checkx - 1, checky + 1]));
        if (checkx < size - 1 && checky > 0)
            explored.add(JSON.stringify([checkx + 1, checky - 1]));
    ...
    function getChainAttack(x, y) {
        const size = board[0].length;
        let count = 0;
        if (x > 0 && board[x - 1][y] === you)
            count += getChainValue(x - 1, y, you);
        if (x < size - 1 && board[x + 1][y] === you)
            count += getChainValue(x + 1, y, you);
        if (y > 0 && board[x][y - 1] === you)
            count += getChainValue(x, y - 1, you);
        if (y < size - 1 && board[x][y + 1] === you)
            count += getChainValue(x, y + 1, you);
    ...
    function getChainAttackFull(x, y) {
        ...
        if (x < size - 1 && y < size - 1)
            count += getChainValue(x + 1, y + 1, you);
    ...
    function getSurroundLibs(x, y, player) {
        ...
        if (x > 0 && (board[x - 1][y] === "." || board[x - 1][y] === player))
            surround +=
                board[x - 1][y] === "." ? 1 : validLibMoves[x - 1][y] - 1;
        if (
            x < size - 1 &&
            (board[x + 1][y] === "." || board[x + 1][y] === player)
        )
            surround +=
                board[x + 1][y] === "." ? 1 : validLibMoves[x + 1][y] - 1;
        if (y > 0 && (board[x][y - 1] === "." || board[x][y - 1] === player))
            surround +=
                board[x][y - 1] === "." ? 1 : validLibMoves[x][y - 1] - 1;
        if (
            y < size - 1 &&
            (board[x][y + 1] === "." || board[x][y + 1] === player)
        )
            surround +=
                board[x][y + 1] === "." ? 1 : validLibMoves[x][y + 1] - 1;
        return surround;
    ...
    function getSurroundEnemiesFull(x, y) {
        ...
        let surround = 0;
        if (x > 0 && board[x - 1][y] === you)
            surround += getChainValue(x - 1, y, you);
        if (x < size - 1 && board[x + 1][y] === you)
            surround += getChainValue(x + 1, y, you);
        if (y > 0 && board[x][y - 1] === you)
            surround += getChainValue(x, y - 1, you);
        if (y < size - 1 && board[x][y + 1] === you)
            surround += getChainValue(x, y + 1, you);

        if (x > 0 && y > 0 && board[x - 1][y - 1] === you)
            surround += getChainValue(x - 1, y - 1, you);
        if (x < size - 1 && y > 0 && board[x + 1][y - 1] === you)
            surround += getChainValue(x + 1, y - 1, you);
        if (y < size - 1 && x > 0 && board[x - 1][y + 1] === you)
            surround += getChainValue(x - 1, y - 1, you);
        if (y < size - 1 && x < size - 1 && board[x + 1][y + 1] === you)
            surround += getChainValue(x + 1, y + 1, you);
    ...
    function getRandomStrat() {
        ...
        const moves = getAllValidMoves(true);
        for (const [x, y] of moves) {
            if (!["?", you].includes(contested[x][y]) || createsLib(x, y, me))
                continue;
            let isSupport =
                (x > 0 &&
                    board[x - 1][y] === me &&
    ...
    function getAggroAttack(
        ...
        for (const [x, y] of moves) {
            ...
            if (
                x > 0 &&
                board[x - 1][y] === you &&
                validLibMoves[x - 1][y] < lowestLibs
            )
            ...
            if (
                y > 0 &&
                board[x][y - 1] === you &&
                validLibMoves[x][y - 1] < lowestLibs
            )
    ...
    function getDefAttack(
        ...
        for (const [x, y] of moves) {
            ...
            const atk =
                ((((friendlyLibs * chainAtk) / eyeValue) *
                    getHeatMap(x, y, me)) /
                    lowestLibs) *
    ...
    async function movePiece(attack) {
        ...
        if (LOGTIME) {
            if (ns.ui.getGameInfo()?.versionNumber >= 44)
                ns.printf(
                    "Time: Me: %s  Them: %s",
                    ns.format.time(mid - START, true),
                    ns.format.time(END - mid, true),
                );
            else
                ns.printf(
                    "Time: Me: %s  Them: %s",
                    ns.tFormat(mid - START, true),
                    ns.tFormat(END - mid, true),
                );
        }
    ...
    async function moveSnakeEyes(attack) {
        ...
        try {
            ...
            if (chance < 0.7) return false;
            let mid = performance.now();
            const results = await ns.go.cheat.playTwoMoves(
                s1x,
                s1y,
                s2x,
                s2y,
    ...
    function getAllValidMoves(notMine = false) {
        ...
        for (let x = 0; x < board[0].length; x++)
            for (let y = 0; y < board[0].length; y++) {
                if (validMove[x][y]) {
                    if ([you, "?"].includes(contested[x][y]))
                        contestedMoves.push([x, y]);
                    moves.push([x, y]);
                }
            }
        ...
        contestedMoves = contestedMoves.sort(
    ...
    function createsLib(x, y, player) {
        ...
        if (
            ...
            board[x - 1][y] === player &&
            validLibMoves[x - 1][y] === 2 &&
            getSurroundLibs(x - 1, y, player) === 1
        )
            return true;
        if (
            x < size - 1 &&
        ...
        if (
            ...
            board[x][y - 1] === player &&
            validLibMoves[x][y - 1] === 2 &&
            getSurroundLibs(x, y - 1, player) === 1
        )
            return true;
        if (
            y < size - 1 &&
    ...
    async function proxy(func, ...argmnts) {
        return await runIt(
    ...
    async function proxyTry(func, ...argmnts) {
        return await runIt(
    ...
    async function getMoves(playAsWhite) {
        return await runIt("SphyxOS/ipvgo/getvalidmoves.js", false, [
            playAsWhite,
        ]);
    }
    /** @param {NS} ns */
