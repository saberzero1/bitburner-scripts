...
export async function main(ns) {
    ...
    while (
        !c.hasCorporation() &&
        c.canCreateCorporation(selfFund) &&
        !c.createCorporation(corpName, selfFund)
    )
        await ns.sleep(1000);
    ...
    while (round === 1) {
        ...
        if (nState === "SALE") sell(ns);
        if (nState === "PURCHASE") {
            if (
                !teaNeeded &&
                c.getOffice(div1, "Sector-12").employeeJobs.Business > 0
            ) {
                optimizeMats(ns);
            }
            purchase(ns);
    ...
    while (round === 2) {
        ...
        while (
            hasDiv2 &&
            c.getUpgradeLevel("Smart Factories") < 16 &&
            c.getUpgradeLevelCost("Smart Factories") <= corpFunds(ns)
        )
        ...
        if (nState === "PURCHASE") {
            ...
            purchase(ns);
            while (
                corpFunds(ns) > c.getHireAdVertCost(div1) &&
                c.getHireAdVertCount(div1) < 12 &&
                hasDiv2
            )
                try {
                    c.hireAdVert(div1);
            ...
            if (
                c.getHireAdVertCount(div1) < 11 &&
                c.getMaterial(div1, "Sector-12", "Plants").stored > 200
            )
                try {
                    c.hireAdVert(div1);
                } catch {}
            ...
            if (ns.ui.getGameInfo()?.versionNumber === undefined) {
                if (
                    !teaNeeded &&
                    c.getOffice(div1, "Sector-12").employeeJobs.Business > 0 &&
                    c.getUpgradeLevel("DreamSense") === 0
                )
                    try {
        ...
        if (nState === "EXPORT") {
            ...
            if (
                ...
                while (
                    hasDiv2 &&
                    corpFunds(ns) >= c.getUpgradeLevelCost("ABC SalesBots") &&
                    c.getUpgradeLevel("ABC SalesBots") < 30
                )
                    c.levelUpgrade("ABC SalesBots");
                optimizeMats(ns);
            }
            while (
                corpFunds(ns) >= c.getUpgradeLevelCost("ABC SalesBots") &&
                c.getUpgradeLevel("ABC SalesBots") < 10
            )
                c.levelUpgrade("ABC SalesBots");
        }
        await ns.corporation.nextUpdate();
    ...
    while (round === 3 || round === 4) {
        ...
        updateHud(ns);
        while (
            c.getUpgradeLevel("Smart Factories") < 20 &&
            c.getUpgradeLevelCost("Smart Factories") <= corpFunds(ns)
        )
            c.levelUpgrade("Smart Factories");
        manageProducts(ns);
...
function prep(ns) {
    ...
    if (round >= 1) {
        if (!hasDivDB[div1]) {
            try {
                ...
                try {
                    c.expandIndustry("Agriculture", div1);
                } catch {}
                try {
                    const div = c.getDivision(div1);
                    hasDivDB[div1] = div;
        ...
        if (researchedDB["Export"]) {
            if (!hasDivDB[div2]) {
                try {
                    ...
                    try {
                        c.expandIndustry("Chemical", div2);
                    } catch {}
                    try {
                        const div = c.getDivision(div2);
                        hasDivDB[div2] = div;
    ...
    if (round >= 3) {
        if (!researchedDB["Market Research - Demand"]) {
            try {
                c.purchaseUnlock("Market Research - Demand");
            } catch {}
        ...
        if (!researchedDB["Market Data - Competition"]) {
            try {
                c.purchaseUnlock("Market Data - Competition");
            } catch {}
    ...
    if (round >= 5) {
        try {
            ...
            try {
                c.expandIndustry("Restaurant", div4);
            } catch {}
            try {
                const div = c.getDivision(div4);
                hasDivDB[div4] = div;
        ...
        if (c.getCorporation().revenue >= 1e70) {
            if (!researchedDB["Government Partnership"]) {
                try {
                    c.purchaseUnlock("Government Partnership");
                } catch {}
            ...
            if (!researchedDB["Shady Accounting"]) {
                try {
                    c.purchaseUnlock("Shady Accounting");
                } catch {}
            ...
            try {
                ...
                try {
                    c.expandIndustry("Water Utilities", div5);
                } catch {}
                try {
                    const div = c.getDivision(div5);
                    hasDivDB[div5] = div;
            ...
            try {
                ...
                try {
                    c.expandIndustry("Computer Hardware", div6);
                } catch {}
                try {
                    const div = c.getDivision(div6);
                    hasDivDB[div6] = div;
            ...
            try {
                ...
                try {
                    c.expandIndustry("Refinery", div7);
                } catch {}
                try {
                    const div = c.getDivision(div7);
                    hasDivDB[div7] = div;
...
function updateMisc(ns) {
    ...
    if (
        round < 5 &&
        (c.getUpgradeLevelCost("ABC SalesBots") * mult) / 2 < corp.funds
    ) {
        c.levelUpgrade("ABC SalesBots");
        corp = c.getCorporation();
    }
    while (
        round >= 5 &&
        (c.getUpgradeLevelCost("ABC SalesBots") * mult) / 2 < corpFunds(ns)
    )
        c.levelUpgrade("ABC SalesBots");
    corp = c.getCorporation();
    if ((round === 3 && c.getCorporation().revenue >= 8e7) || round >= 4) {
        if (
            c.getUpgradeLevel("Neural Accelerators") < 500 &&
            c.getUpgradeLevelCost("Neural Accelerators") * mult < corp.funds
        ) {
            c.levelUpgrade("Neural Accelerators");
            corp = c.getCorporation();
        }
        if (
            c.getUpgradeLevel("Project Insight") < 500 &&
            c.getUpgradeLevelCost("Project Insight") * mult < corp.funds
        ) {
            c.levelUpgrade("Project Insight");
            corp = c.getCorporation();
        }
        if (
            c.getUpgradeLevel("Nuoptimal Nootropic Injector Implants") < 500 &&
            c.getUpgradeLevelCost("Nuoptimal Nootropic Injector Implants") *
                mult <
                corp.funds
        ) {
        ...
        if (
            c.getUpgradeLevel("FocusWires") < 500 &&
            c.getUpgradeLevelCost("FocusWires") * mult < corp.funds
        ) {
            c.levelUpgrade("FocusWires");
            corp = c.getCorporation();
        }
        if (
            c.getUpgradeLevel("Speech Processor Implants") < 500 &&
            c.getUpgradeLevelCost("Speech Processor Implants") * mult <
                corp.funds
        ) {
    ...
    if (round >= 3 && round <= 4) {
        for (const div of industries) {
            ...
            if (
                ...
                (c.getDivision(div).awareness < Number.MAX_VALUE ||
                    c.getDivision(div).popularity < Number.MAX_VALUE)
            ) {
                c.hireAdVert(div);
                corp = c.getCorporation();
            }
    ...
    for (const div of industries) {
            ...
            case "Agriculture":
                {
                    ...
                    if (!researchedDB[div + "Hi-Tech R&D Laboratory"]) {
                        if (
                            rp / 2 >
                            c.getResearchCost(div, "Hi-Tech R&D Laboratory")
                        ) {
                    ...
                    if (!researchedDB[div + "CPH4 Injections"]) {
                        if (
                            rp / 10 >
                            c.getResearchCost(div, "CPH4 Injections")
                        ) {
            ...
            case "Tobacco":
                {
                    ...
                    if (!researchedDB[div + "uPgrade: Fulcrum"]) {
                        if (
                            rp / 10 >
                            c.getResearchCost(div, "uPgrade: Fulcrum")
                        ) {
        ...
        */
                    if (!researchedDB[div + "Self-Correcting Assemblers"]) {
                        if (
                            rp / 10 >
                            c.getResearchCost(div, "Self-Correcting Assemblers")
                        ) {
                    ...
                    if (!researchedDB[div + "Drones - Assembly"]) {
                        if (
                            rp / 10 >
                            c.getResearchCost(div, "Drones - Assembly")
                        ) {
                    ...
                    if (!researchedDB[div + "Drones - Transport"]) {
                        if (
                            rp / 10 >
                            c.getResearchCost(div, "Drones - Transport")
                        ) {
...
function manageProducts(ns) {
    ...
    for (const div of industries) {
        ...
        for (const prod of division.products) {
            if (
                c.getProduct(div, "Sector-12", prod).developmentProgress === 100
            ) {
        ...
        if (active + calculating === division.maxProducts && calculating <= 1) {
            ...
            for (const prod of division.products) {
                if (
                    c.getProduct(div, "Sector-12", prod).developmentProgress !=
                        100 ||
                    getSellPrice(ns, div, "Sector-12", prod) === 0
                )
                    continue;
            ...
            switch (hasDivDB[div].type) {
                case "Agriculture":
                    switch (round) {
                        case 1:
                            while (
                                c.getOffice(div, city).size < 4 &&
                                c.getOfficeSizeUpgradeCost(div, city, 1) <=
                                    corpFunds(ns)
                            )
                                c.upgradeOfficeSize(div, city, 1);
                            while (
                                c.getOffice(div, city).numEmployees <
                                    c.getOffice(div, city).size &&
                                c.hireEmployee(div, city)
                            ) {}
                        ...
                        case 2:
                            ...
                            if (c.getDivision(div).researchPoints < 700)
                                ...
                                setJob(
                                    ...
                                    "Engineer",
                                    Math.floor(
                                        c.getOffice(div, city).numEmployees / 4,
                                    ),
                                );
                                setJob(div, city, "Business", 1);
                                const remainder =
                        ...
                        case 3:
                            ...
                            resetOffice(ns, div, city);
                            {
                                ...
                                setJob(
                                    ...
                                    "Engineer",
                                    Math.floor(
                                        c.getOffice(div, city).numEmployees / 3,
                                    ),
                                );
                                setJob(div, city, "Business", 1);
                                setJob(
                            ...
                            break;
                        case 4:
                            if (c.getOffice(div, city).size < 60)
                                c.upgradeOfficeSize(div, city, 1);
                            if (
                                c.getOffice(div, city).numEmployees <
                                c.getOffice(div, city).size
                            ...
                            resetOffice(ns, div, city);
                            {
                                ...
                                setJob(
                                    ...
                                    "Engineer",
                                    Math.floor(
                                        c.getOffice(div, city).numEmployees / 2,
                                    ),
                                );
                                setJob(div, city, "Business", 1);
                                setJob(
                            ...
                            break;
                        case 5:
                            if (c.getOffice(div, city).size < 300)
                                c.upgradeOfficeSize(div, city, 1);
                            if (
                                c.getOffice(div, city).numEmployees <
                                c.getOffice(div, city).size
                ...
                case "Tobacco":
                    switch (round) {
                        case 3:
                            {
                                ...
                                if (corpRev > 5e8)
                                    while (
                                        c.getOffice(div, city).size < 116 &&
                                        c.getOfficeSizeUpgradeCost(
                                            div,
                                            city,
                                            1,
                                        ) *
                        ...
                        case 4:
                            {
                                const corpRev = c.getCorporation().revenue;
                                if (c.getOffice(div, city).size < 250)
                                    c.upgradeOfficeSize(div, city, 1);
                                if (corpRev > 100e9)
                                    while (
                                        c.getOffice(div, city).size < 270 &&
                                        c.getOfficeSizeUpgradeCost(
                                            div,
                                            city,
                                            1,
                        ...
                        case 5:
                            while (
                                c.getOffice(div, city).size < 1500 &&
                                corpFunds(ns) >=
                                    c.getOfficeSizeUpgradeCost(div, city, 1)
                            )
                                c.upgradeOfficeSize(div, city, 1);
                ...
                case "Water Utilities":
                    switch (round) {
                        case 5:
                            while (
                                c.getOffice(div, city).size < 6500 &&
                                corpFunds(ns) >=
                                    c.getOfficeSizeUpgradeCost(div, city, 1)
                            )
                                c.upgradeOfficeSize(div, city, 1);
                ...
                case "Computer Hardware":
                    switch (round) {
                        case 5:
                            while (
                                c.getOffice(div, city).size < 4500 &&
                                corpFunds(ns) >=
                                    c.getOfficeSizeUpgradeCost(div, city, 1)
                            )
                                c.upgradeOfficeSize(div, city, 1);
    ...
    for (const div of industries) {
        ...
        for (const city of cities) {
            ...
            if (!indDataDB[hasDivDB[div].type]) {
                indDataDB[hasDivDB[div].type] = c.getIndustryData(
                    hasDivDB[div].type,
                );
            }
            ...
            for (const [matName, [buy, reqMat]] of Object.entries(smartBuy)) {
                ...
                if (mult[0] === 0) {
                    ...
                    c.buyMaterial(div, city, matName, 0);
                    if (c.getMaterial(div, city, matName).quality <= 1)
                        c.sellMaterial(
                            div,
                            city,
                            matName,
        ...
        for (const city of cities) {
            ...
            for (const name of Object.values(
                ...
                if (name === "Plants") {
                    ...
                    try {
                        c.cancelExportMaterial(div, city, div3, city, name);
                    } catch {}
                    try {
                        c.cancelExportMaterial(div, city, div2, city, name);
                    } catch {}
                    try {
                        c.exportMaterial(
                            div,
                    ...
                    try {
                        c.cancelExportMaterial(div, city, div1, city, name);
                    } catch {}
                    try {
                        c.exportMaterial(
                            div,
                    ...
                    try {
                        c.cancelExportMaterial(div, city, div4, city, name);
                    } catch {}
                    try {
                        c.exportMaterial(
                            div,
                    ...
                    try {
                        c.cancelExportMaterial(div, city, div5, city, name);
                    } catch {}
                    try {
                        c.cancelExportMaterial(div, city, div8, city, name);
                    } catch {}
                    try {
                        c.exportMaterial(
                            div,
                    ...
                    try {
                        c.cancelExportMaterial(div, city, div6, city, name);
                    } catch {}
                    try {
                        c.exportMaterial(
                            div,
                    ...
                    try {
                        c.cancelExportMaterial(div, city, div7, city, name);
                    } catch {}
                    try {
                        c.exportMaterial(
                            div,
...
function optimizeMats(ns) {
    ...
    for (const div of industries) {
        ...
        for (const city of cities) {
            ...
            const [hardware, robots, aicores, realestate] =
                optimizeCorpoMaterials(ns, div, warehouse.size - maxProd);
            ...
            const realEstateStored = c.getMaterial(
                div,
                city,
                "Real Estate",
            ).stored;
...
function optimizeCorpoMaterials_raw(
    ...
    for (let i = 0; i < matSizes.length; ++i) {
        let m =
            (spaceConstraint -
                500 *
                    ((matSizes[i] / divWeights[i]) * (p - divWeights[i]) -
                        (w - matSizes[i]))) /
            (p / divWeights[i]) /
            matSizes[i];
...
function optimizeCorpoMaterials(ns, div, spaceConstraint, round = true) {
    ...
    let { hardwareFactor, robotFactor, aiCoreFactor, realEstateFactor } =
        indDataDB[type];
    ...
    if (!matDataDB["Real Estate"])
        matDataDB["Real Estate"] =
            ns.corporation.getMaterialData("Real Estate");
    const matSizes = ["Hardware", "Robots", "AI Cores", "Real Estate"].map(
        (mat) => matDataDB[mat].size,
    );
    return optimizeCorpoMaterials_raw(
        matSizes,
        divWeights,
...
function maxProduced(ns, div, city) {
    ...
    for (const [matName, matAmount] of Object.entries(
        ...
        if (matDataDB[matName] === undefined)
            matDataDB[matName] = c.getMaterialData(matName);
        totalSize +=
            maxMatRequired(ns, div, city, matName) * matDataDB[matName].size;
    }
    if (indDataDB[hasDivDB[div].type].makesMaterials)
        for (const mat of indDataDB[hasDivDB[div].type].producedMaterials) {
            ...
            totalSize +=
                c.getMaterial(div, city, mat).stored * matDataDB[mat].size;
        }
    const division = c.getDivision(div);
    for (const prod of division.products)
        if (c.getProduct(div, city, prod).developmentProgress === 100) {
            ...
            totalSize +=
                c.getProduct(div, city, prod).stored *
                c.getProduct(div, city, prod).size;
        }
    return totalSize;
}
/** @param {NS} ns */
    ...
    while (count < 8) {
        ...
        for (const div of industries) {
            ...
            for (const city of cities) {
                ...
                let divMult = 1;
                try {
                    divMult = researchedDB[div + "Drones - Transport"]
                        ? 1.5
                        : 1;
                } catch {
                    continue;
                ...
                if (
                    ...
                    warehouseRatio = 0; //Early break on Chemical warehouse upgrade until we get all of Chemical
                if (
                ...
                if (round >= 3 && hasDivDB[div].type === "Agriculture") {
                    if (
                        maxProd[0] > maxMatRequired(ns, div4, city, "Food") &&
                        maxProd[0] >
                            maxMatRequired(ns, div2, city, "Plants") +
                                maxMatRequired(ns, div3, "Sector-12", "Plants")
                    )
                        warehouseRatio = 0;
                ...
                if (round >= 3 && hasDivDB[div].type === "Chemical") {
                    if (
                        maxProd[0] >
                            maxMatRequired(ns, div1, city, "Chemicals") ||
                        !hasDiv3
                    )
                        warehouseRatio = 0;
                    else warehouseRatio *= 0.9;
                ...
                if (round >= 5 && hasDivDB[div].type === "Water Utilities") {
                    if (
                        maxProd[0] >
                        maxMatRequired(ns, div1, city, "Water") +
                            maxMatRequired(ns, div2, city, "Water") +
                            maxMatRequired(ns, div4, city, "Water")
                    )
                        warehouseRatio = 0;
                    else warehouseRatio *= 0.9;
                }
                if (round >= 5 && hasDivDB[div].type === "Computer Hardware") {
                    if (
                        maxProd[0] >
                        maxMatRequired(ns, div5, city, "Hardware") +
                            maxMatRequired(ns, div8, city, "Hardware")
                    )
                        warehouseRatio = 0;
                    else warehouseRatio *= 0.9;
                }
        ...
        if (round >= 3) {
            if (bestUpgradeType === "none") break;
                ...
                bestUpgradeType === "Smart" &&
                funds >= c.getUpgradeLevelCost("Smart Storage") * 1.5
            ) {
                c.levelUpgrade("Smart Storage");
            } else if (
                bestUpgradeCity !== "none" &&
                funds >=
                    c.getUpgradeWarehouseCost(
                        bestUpgradeType,
                        bestUpgradeCity,
                    ) *
                        1.5
            ...
            if (bestUpgradeType === "none") break;
                ...
                bestUpgradeType === "Smart" &&
                funds >= c.getUpgradeLevelCost("Smart Storage")
            ) {
                c.levelUpgrade("Smart Storage");
            } else if (
                bestUpgradeCity !== "none" &&
                funds >=
                    c.getUpgradeWarehouseCost(bestUpgradeType, bestUpgradeCity)
            ) {
                c.upgradeWarehouse(bestUpgradeType, bestUpgradeCity);
            } else break;
        }
        ...
        for (const city of cities) {
            ...
            if (
                ...
                if (indDataDB[hasDivDB[div].type].makesProducts) {
                    ...
                    for (const prod of division.products) {
                        if (
                            c.getProduct(div, city, prod)
                                .developmentProgress !== 100
                        )
                            continue;
                        if (c.getProduct(div, city, prod).stored === 0)
                            continue;
...
function getMult(ns, div, city) {
    ...
    for (const scity of cities) {
        ...
        let realestate = Math.pow(
            0.002 * c.getMaterial(div, scity, "Real Estate").stored + 1,
            indDataDB[hasDivDB[div].type].realEstateFactor,
        );
        let hardware = Math.pow(
            0.002 * c.getMaterial(div, scity, "Hardware").stored + 1,
            indDataDB[hasDivDB[div].type].hardwareFactor,
        );
        let robots = Math.pow(
            0.002 * c.getMaterial(div, scity, "Robots").stored + 1,
            indDataDB[hasDivDB[div].type].robotFactor,
        );
        let aicores = Math.pow(
            0.002 * c.getMaterial(div, scity, "AI Cores").stored + 1,
...
function updateHud(ns) {
    ...
    const upgrades =
        c.getUpgradeLevel("Neural Accelerators") +
    ...
    const produpgrades =
        c.getUpgradeLevel("Smart Factories") +
        c.getUpgradeLevel("Smart Storage");
    if (ns.ui.getGameInfo()?.versionNumber >= 44)
        ns.printf(
    ...
    if (ns.ui.getGameInfo()?.versionNumber >= 44)
        ns.printf(
            ...
            upgrades,
            produpgrades,
            c.getUpgradeLevel("ABC SalesBots"),
            c.getUpgradeLevel("Wilson Analytics"),
        );
    else
        ns.printf(
                    ...
                    if (sellPrice === 0)
                        ...
                        if (ns.ui.getGameInfo()?.versionNumber >= 44)
                            ns.printf(
                                "  $%s - %s",
                                ns.format.number(
                                    getSellPrice(ns, div, "Sector-12", product),
                                    3,
                                ),
                                product,
                            );
