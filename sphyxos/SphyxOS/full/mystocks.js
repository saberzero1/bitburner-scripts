...
function printLogs(ns, stocks) {
    ...
    ns.printf(
        "│   SYM │  TYPE │  SHARES │    PAID │   VALUE │   PROFIT │       %s │  FCAST │ VOLI │",
        "%",
    );
    ns.printf(
        "├───────┼───────┼─────────┼─────────┼─────────┼──────────┼─────────┼────────┼──────┤",
    ...
    for (const stk of stocks) {
        ...
        ns.printf(
            ...
            percentchange != 0 ? ns.format.number(percentchange, 2) : "-------",
            ns.format.number(stk.forcast, 2),
            ns.format.number(stk.volitile, 2),
        );
    }
    let totalpercentchange =
        totalpaid > 0 ? (100 - (totalvalue / totalpaid) * 100) * -1 : 0;
    ...
    ns.printf(
        "│Start: %8s│ %7s │ %7s │ %7s │ %8s │ %7s │Gain: %7s%s │",
...
function buyItems(ns, stocks) {
    ...
    while (running) {
        ...
        while (
            topposi[0] == stk.getMaxShares(top.sym) ||
            topposi[2] == stk.getMaxShares(top.sym)
        ) {
            topl--;
            top = stocks[topl];
        ...
        while (
            botposi[0] == stk.getMaxShares(bot.sym) ||
            botposi[2] == stk.getMaxShares(bot.sym)
        ) {
            botl++;
            bot = stocks[botl];
        ...
        if (SHORTS) {
            if (bot.adjfcast >= top.forcast && bot.adjfcast >= BUY_THREASH) {
                ...
                if (
                    buying + botposi[0] + botposi[2] >
                    stk.getMaxShares(bot.sym)
                ) {
                    buying =
                        stk.getMaxShares(bot.sym) - botposi[0] - botposi[2];
                    max = true;
                }
                ...
                let buying = Math.floor(budget / price);
                if (
                    buying + topposi[0] + topposi[2] >
                    stk.getMaxShares(top.sym)
                ) {
                    buying =
                        stk.getMaxShares(top.sym) - topposi[0] - topposi[2];
                    max = true;
                }
...
function updateForcast(ns, stocks) {
    ...
    for (let stk of stocks) {
        ...
        if (S4DATA) {
            stk.hybridforcast =
                (stk.regforcast * HYBRID_WEIGHT_REG +
                    stk.s4forcast * HYBRID_WEIGHT_4S) /
                (HYBRID_WEIGHT_4S + HYBRID_WEIGHT_REG);
            stk.hybridadjfcast =
                stk.hybridforcast >= 50
                    ? stk.hybridforcast
                    : 100 - stk.hybridforcast;
        }
    }
    ...
    if (HYBRIDFCAST && S4DATA) {
        for (const stk of stocks) {
            stk.forcast =
                stk.s4forcast > 50
                    ? stk.hybridforcast + stk.s4volitile - HYBRID_VOL
                    : stk.hybridforcast - stk.s4volitile + HYBRID_VOL;
...
function sellItems(ns, stocks, arg) {
    for (let obj of stocks) {
        ...
        if (
            (posi[0] > 0 && obj.forcast <= SELL_THREASH) ||
            (posi[0] > 0 && arg && arg == "sell")
        ) {
            ...
            if (sellprice >= posi[1]) {
                ...
                if (SHOWSELLS)
                    ns.tprintf(
                        ...
                        obj.sym,
                        ns.format.number(sellprice * posi[0], 2),
        ...
        if (
            (posi[2] > 0 && obj.forcast >= 100 - SELL_THREASH) ||
            (posi[2] > 0 && arg && arg == "sell")
        ) {
...
export async function main(ns) {
    ...
    if (
        ...
        (!stks.hasWseAccount() || !stks.hasTixApiAccess())
    ) {
        ns.tprintf("WSE and TIX API access are required to run this.");
        return;
    } else if (
        ns.ui.getGameInfo()?.versionNumber === undefined &&
        (!stks.hasWSEAccount() || !stks.hasTIXAPIAccess())
    ) {
        ns.tprintf("WSE and TIX API access are required to run this.");
        return;
    ...
    if (ns.args.includes("help")) {
        ...
        ns.tprintf(
            "fundself Only purchase stocks with money from the sale of stocks",
        );
    ...
    if (ns.args.includes("stop")) {
        ns.tprintf(
            "%s is being stopped.  Don't forget to sell.",
            ns.getScriptName(),
        );
    ...
    if (
        ns.ui.getGameInfo()?.versionNumber >= 44 &&
        stks.has4SDataTixApi() &&
        !ns.args.includes("no4s")
    ) {
        S4DATA = true;
    ...
    while (working) {
        ...
        if (Date.now() >= starttime + REPORT) {
            ...
            if (endworth > startworth) {
                ns.tprintf(
                    "INFO: Success!  After 1 hour %s turned into %s (%s%s)",
                    ns.format.number(startworth, 2),
                    ns.format.number(endworth, 2),
                    ns.format.number((endworth / startworth) * 100, 2),
                    "%",
                );
                ...
                ns.tprintf(
                    "INFO: Fail!  After 1 hour %s turned into %s (%s%s)",
                    ns.format.number(startworth, 2),
                    ns.format.number(endworth, 2),
                    ns.format.number((endworth / startworth) * 100, 2),
                    "%",
