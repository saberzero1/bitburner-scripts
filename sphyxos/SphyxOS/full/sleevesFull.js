    ...
    while (true) {
        ...
        for (const slv of sleeves) {
            ...
            if (["Money", "Karma", "Int", "Install"].includes(MODE)) {
                ...
                if (slv.task && slv.task.crimeType !== bestCrime)
                    ns.sleeve.setToCommitCrime(slv.num, bestCrime);
                continue;
            }
        } //End of sleeves
    } //End While True
    ...
    for (const slv of sleeves) {
        if (HASBN5)
            ns.printf(
                ...
                ns.sleeve.getSleeveAugmentations(slv.num).length,
