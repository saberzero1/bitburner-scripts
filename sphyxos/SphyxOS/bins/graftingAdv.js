...
export async function main(ns) {
    ...
    const specificQueue = [];
    let MODE = await ns.prompt("Select Priority:", {
        type: "select",
        choices: priorities,
    });
    let SPECIFIC;
    ...
    if (MODE === "Specific")
        SPECIFIC = await ns.prompt("Select Augment:", {
            type: "select",
            choices: graftable,
        });
    ...
    while (MODE === "Specific" && SPECIFIC !== "" && SPECIFIC !== "Done") {
        ...
        SPECIFIC = await ns.prompt("Select Next Queued Augment:", {
            type: "select",
            choices: ["Done"].concat(graftable),
        });
    ...
    if (city !== "New Tokyo") {
        while (city !== "New Tokyo") {
            ...
            update(
                ns,
                ns.sprintf("Trying to travel to New Tokyo!  In %s", city),
            );
    ...
    if (MODE !== "Specific") {
        ...
        for (const graft of graftable) {
            const record = {
                ...
                stats: await proxy(
                    ns,
                    "singularity.getAugmentationStats",
                    graft,
                ),
                time: await proxy(
                    ns,
    ...
    if (MODE !== "Specific") {
        switch (MODE) {
            ...
            case "H/G/W Speed":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.hacking_speed - a.stats?.hacking_speed;
                });
                break;
            case "Hack Power":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.hacking_money - a.stats?.hacking_money;
                });
                break;
            case "Grow Power":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.hacking_grow - a.stats?.hacking_grow;
                });
                break;
            case "Hack Success":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.hacking_chance - a.stats?.hacking_chance;
                });
                break;
            case "Hacking":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.hacking_exp - a.stats?.hacking_exp;
                });
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.hacking - a.stats?.hacking;
                });
                break;
            case "Strength":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.strength_exp - a.stats?.strength_exp;
                });
                graftable = temp.toSorted((a, b) => {
            ...
            case "Defence":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.defense_exp - a.stats?.defense_exp;
                });
                graftable = temp.toSorted((a, b) => {
            ...
            case "Dexterity":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.dexterity_exp - a.stats?.dexterity_exp;
                });
                graftable = temp.toSorted((a, b) => {
            ...
            case "Agility":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.agility_exp - a.stats?.agility_exp;
                });
                graftable = temp.toSorted((a, b) => {
            ...
            case "Charisma":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.charisma_exp - a.stats?.charisma_exp;
                });
                graftable = temp.toSorted((a, b) => {
            ...
            case "Reputation":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.company_rep - a.stats?.company_rep;
                });
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.faction_rep - a.stats?.faction_rep;
                });
                break;
            case "Work Money":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.work_money - a.stats?.work_money;
                });
                break;
            case "Crime Money":
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.crime_success - a.stats?.crime_success;
                });
                graftable = temp.toSorted((a, b) => {
                    return b.stats?.crime_money - a.stats?.crime_money;
                });
                break;
            case "Crime Chance":
                graftable = temp.toSorted((a, b) => {
            ...
            case "HackNet Production":
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.hacknet_node_money -
                        a.stats?.hacknet_node_money
                    );
                });
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.hacknet_node_ram_cost -
                        a.stats?.hacknet_node_ram_cost
                    );
                });
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.hacknet_node_core_cost -
                        a.stats?.hacknet_node_core_cost
                    );
                });
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.hacknet_node_level_cost -
                        a.stats?.hacknet_node_level_cost
                    );
                });
                graftable = temp.toSorted((a, b) => {
                    return (
            ...
            case "BB Analysis":
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.bladeburner_analysis -
                        a.stats?.bladeburner_analysis
                    );
                });
                break;
            case "BB Stamina":
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.bladeburner_stamina_gain -
                        a.stats?.bladeburner_stamina_gain
                    );
                });
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.bladeburner_max_stamina -
                        a.stats?.bladeburner_max_stamina
                    );
                });
                break;
            ...
            case "BB Success Chance":
                ...
                graftable = temp.toSorted((a, b) => {
                    return (
                        b.stats?.bladeburner_success_chance -
                        a.stats?.bladeburner_success_chance
                    );
                });
                break;
            default:
    ...
    const bench = [];
    while (augments.length > 0) {
        //Expand this while loop
        await getCommands(ns);
        const aug = augments.shift();
        if (!(await proxyTry(ns, "grafting.getAugmentationGraftPrice", aug)))
            continue;
        player = await proxy(ns, "getPlayer");
        city = player.city;
        if (city !== "New Tokyo") {
            while (city !== "New Tokyo") {
                ...
                update(
                    ns,
                    ns.sprintf(
                        "Trying to travel to New Tokyo!  Currently in %s",
                        city,
                    ),
                );
                await proxy(ns, "singularity.travelToCity", "New Tokyo");
        ...
        if (wrk == null || wrk.type !== "GRAFTING") {
            if (!(await proxy(ns, "grafting.graftAugmentation", aug, FOCUS))) {
                ...
                if (augments.length === 0)
                    while (bench.length > 0) augments.unshift(bench.pop());
                await ns.sleep(100);
                continue;
            }
        }
        ...
        while (wrk && wrk.type === "GRAFTING") {
            ...
            update(
                ns,
                ns.sprintf("Neuroreceptor Management Implant: %s", !FOCUS),
            );
            ...
            const augStats = await proxy(
                ns,
                "singularity.getAugmentationStats",
                wrk.augmentation,
            );
            for (const [name, stat] of Object.entries(augStats))
                if (stat !== 1) update(ns, ns.sprintf("%s - %s", name, stat));
...
async function getCommands(ns) {
    ...
    while (ns.peek(23) !== "NULL PORT DATA") {
        ...
        switch (result) {
            ...
            default:
                ns.tprintf(
                    "Invalid command received in GraftingBasic: %s",
                    result,
                );
                break;
        }
