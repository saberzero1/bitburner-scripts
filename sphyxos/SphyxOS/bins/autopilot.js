...
export async function main(ns) {
    ...
    ns.ui.setTailTitle(
        MOVEON ? "AutoPilot - Will move on" : "AutoPilot - Will not move on",
    );
    if (win)
        win.header(
            MOVEON
                ? "AutoPilot - Will move on"
                : "AutoPilot - Will not move on",
        );
    ...
    if (HASBN13 && currentNode !== 8) {
        ...
        if (activeFrags.length === 0) await loadStanek(ns);
    ...
    switch (currentNode) {
        ...
        default:
            while (true) {
                ...
                await prime(ns);
                await getCommands(ns);
                if (HASBN9 && !(await hasAllAugs(ns, "Netburners")))
                    //Netburners
                    await factionWork(
                        ns,
                        "1/8",
            ...
            while (true) {
                ...
                if (!(await proxy(ns, "gang.inGang"))) {
                    ...
                    await factionWork(
                        ...
                        false,
                        true,
                        { job: "field", cstats: 30, hashBuy: 50e12, karma: -9 },
                    );
                } else if (!(await hasAllAugs(ns, "Sector-12"))) {
                    if (
                        moneySources?.sinceInstall.hacking < 10e9 &&
                        GANG_EQ_UPDATE
                    ) {
                    ...
                    await factionWork(
                        ...
                        false,
                        {
                            city: "Sector-12",
                            hashBuy:
                                (await maxMoneyNeeded(ns, "Slum Snakes")) / 2,
                        },
                    );
                    ...
                    if (await hasAllAugs(ns, "Slum Snakes")) {
                        ...
                        await proxy(
                            ...
                            "SphyxOS/singularity/restart.js",
                        );
                    }
                } else if (
                    (await proxy(
                    ...
                    await farmFaction(
                        ...
                        false,
                        true,
                        await maxMoneyNeeded(ns, "Slum Snakes"),
                        "hacking",
                        "Sector-12",
                    );
                }
        ...
        case 7:
            while (true) {
                ...
                if (
                    !(await proxy(ns, "bladeburner.inBladeburner")) &&
                    moneySources?.sinceInstall.casino >= 10000000000
                ) {
                    clearLogs(ns);
                ...
                if (
                    ...
                    ns.writePort(
                        1,
                        HASBN10 ? "bb sleeves on" : "bb sleeves off",
                    );
                ...
                if (await proxy(ns, "bladeburner.inBladeburner")) {
                    ...
                    printLogs(
                        ns,
                        "Waiting for BladeBurner to finish.  " +
                            (ns.bladeburner.getBlackOpNames().length - count) +
                            " / " +
                            ns.bladeburner.getBlackOpNames().length,
                    );
                    if (count === 0) {
                        UPGRADED = false;
                        await endIt(ns);
        ...
        case 8:
            ...
            let data4SCost;
            if (ns.ui.getGameInfo()?.versionNumber >= 44)
                data4SCost = (await proxy(ns, "stock.has4SDataTixApi"))
                    ? 0
                    : 25000000000;
            else
                data4SCost = (await proxy(ns, "stock.has4SDataTIXAPI"))
            ...
            while (true) {
                ...
                if (
                    data4SCost > 0 &&
                    (await proxy(ns, "getServerMoneyAvailable", "home")) >
                        data4SCost
                ) {
                    if (ns.ui.getGameInfo()?.versionNumber >= 44)
                        await proxy(ns, "stock.purchase4SMarketDataTixApi");
                    else await proxy(ns, "stock.purchase4SMarketDataTixApi");
                }
                await proxy(ns, "stock.purchase4SMarketDataTixApi");
                if (!(await hasAllAugs(ns, "Tian Di Hui"))) {
                    //14b for all augments and neuroflux.  + 25b for 4s = 39b minimum to move on.
                    if (MONEYCHANGE2) {
                        ns.writePort(13, "silent");
                    ...
                    await factionWork(
                        ...
                        "1/8 - " +
                            formatNum(
                                ...
                                30000000000 +
                                    data4SCost +
                                    moneySources?.sinceInstall.augmentations,
                                2,
                            ) +
                            " - ",
                        "Tian Di Hui",
                    ...
                    if (
                        ...
                        (await proxy(
                            ...
                            "Tian Di Hui",
                        )) >= (await maxRepNeeded(ns, "Tian Di Hui")) &&
                        (await proxy(ns, "getServerMoneyAvailable", "home")) +
                            (await getWorth(ns)) >
                            30000000000 +
                                data4SCost +
                    ...
                    if (
                        ...
                        (await proxy(
                            ...
                            "singularity.getFactionRep",
                            "CyberSec",
                        )) >= (await maxRepNeeded(ns, "CyberSec")) &&
                        (await proxy(ns, "getServerMoneyAvailable", "home")) +
                            (await getWorth(ns)) >
                            23000000000 +
                                data4SCost +
                    ...
                    await factionWork(
                        ...
                        false,
                        true,
                        { homeRam: 32, buyRep: false, buyAugs: buyAugsFlag },
                    );
                    if (
                        MONEYCHANGE3 &&
                        (await proxy(
                    ...
                    await factionWork(
                        ...
                        false,
                        true,
                        { homeRam: 64, buyRep: false, buyAugs: buyAugsFlag },
                    );
                    if (
                        MONEYCHANGE3 &&
                        (await proxy(
                            ...
                            "singularity.getFactionRep",
                            "The Black Hand",
                        )) >= (await maxRepNeeded(ns, "The Black Hand")) &&
                        (await proxy(ns, "getServerMoneyAvailable", "home")) +
                            (await getWorth(ns)) >
                            53000000000 +
                                data4SCost +
                    ...
                    await factionWork(
                        ...
                        false,
                        true,
                        { homeRam: 128, buyRep: false, buyAugs: buyAugsFlag },
                    );
                    if (
                        MONEYCHANGE3 &&
                        (await proxy(
                            ...
                            "singularity.getFactionRep",
                            "BitRunners",
                        )) >= (await maxRepNeeded(ns, "BitRunners")) &&
                        (await proxy(ns, "getServerMoneyAvailable", "home")) +
                            (await getWorth(ns)) >
                            87000000000 +
                                data4SCost +
                    ...
                    await factionWork(
                        ...
                        false,
                        true,
                        { buyRep: false, buyAugs: buyAugsFlag },
                    );
                    if (
                        MONEYCHANGE3 &&
                        (await proxy(
                            ...
                            "singularity.getFactionRep",
                            "Daedalus",
                        )) >= (await maxRepNeeded(ns, "Daedalus")) &&
                        (await proxy(ns, "getServerMoneyAvailable", "home")) +
                            (await getWorth(ns)) >
                            178000000000 +
                                data4SCost +
        ...
        case 9:
            ...
            while (true) {
                ...
                const player = await proxy(ns, "getPlayer");
                const moneySources = await proxy(ns, "getMoneySources");
                const cctCost = await proxy(
                    ns,
                    "hacknet.hashCost",
                    "Generate Coding Contract",
                );
                const minHash = await proxy(
                    ns,
                    "hacknet.hashCost",
                    "Reduce Minimum Security",
                );
                const maxHash = await proxy(
                    ns,
                    "hacknet.hashCost",
                ...
                if (
                    moneySources?.sinceInstall.casino >= 10000000000 &&
                    moneySources?.sinceInstall.hacknet_expenses >= -10000000000
                ) {
                    const pidof = await runIt(
                        ns,
                        "SphyxOS/bins/hacknetPurchaser.js",
                        false,
                        [],
                    );
                    if (pidof) {
                ...
                if (!(await hasAllAugs(ns, "Netburners")) || neuroAmount < 6) {
                    if (
                        !(await hasAllAugs(ns, "Netburners")) &&
                        player.factions.includes("Netburners") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "Netburners",
                        )) < (await maxRepNeeded(ns, "Netburners"))
                    )
                        await factionWork(
                            ns,
                            "01/18",
                        ...
                        await factionWork(
                            ...
                            80,
                            false,
                            { hashBuy: 11e20, hashType: "money", hackLvl: 80 },
                        );
                    if (
                        player.factions.includes("Netburners") &&
                        (await hasAllAugs(ns, "Netburners"))
                ...
                //Tian has 8 unique augs to purchase here.  We want at least 3 more NFG, bringing us up to 9 in total for NFG
                    ...
                    !(await hasAllAugs(ns, "Tian Di Hui")) ||
                    neuroAmount < 9
                ) {
                    if (
                        !(await hasAllAugs(ns, "Tian Di Hui")) &&
                        ...
                        await factionWork(
                            ...
                            false,
                            false,
                            { city: "Chongqing", hashBuy: 11e20, hackLvl: 50 },
                        );
                    else
                        await factionWork(
                            ns,
                ...
                //CyberSec has 5 unitue augs to purchase.  We want at least 6 more NFG, bringing us up to 15
                    ...
                    if (
                        ...
                        await factionWork(
                            ...
                            false,
                            false,
                            { hashBuy: 1e20, homeRam: 32, hackLvl: 55 },
                        );
                    else
                        await factionWork(
                            ns,
                    ...
                    if (
                        (await maxRepNeeded(ns, "Netburners", false)) >
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "Netburners",
                    ...
                    if (player.factions.includes("Netburners")) {
                        await dump(ns);
                        if (
                            myAugs.length - augments.length >= 7 ||
                            myAugs.length - augments.length + neuroAmount >= 40
                        ) {
                            //We want at least 50 when done.
                            UPGRADED = false;
                            await proxy(
                                ns,
                                "singularity.installAugmentations",
                ...
                //Chongqing has 3 unique augs, we have 40 NFG now, we want another 2 from here.
                    ...
                    !(await hasAllAugs(ns, "Chongqing")) ||
                    neuroAmount < 42
                ) {
                    if (
                        !(await hasAllAugs(ns, "Chongqing")) &&
                        player.factions.includes("Chongqing") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "Chongqing",
                        )) < (await maxRepNeeded(ns, "Chongqing"))
                    )
                        await factionWork(
                            ns,
                            "05/18",
                ...
                //NiteSec has 6 unitue augs to purchase.  We have 42NFG, We want at least 0 more NFG, bringing us up to 42
                    ...
                    if (
                        ...
                        await factionWork(
                            ...
                            false,
                            true,
                            { hackLvl: 205, hashBuy: 1e20, hashType: "money" },
                        );
                    }
                    if (
                        player.factions.includes("NiteSec") &&
                ...
                //The Black Hand has 4 augs
                    ...
                    if (
                        cctCost < 4200 &&
                        !(await hasAllAugs(ns, "The Black Hand")) &&
                        player.factions.includes("The Black Hand") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                        ...
                        await factionWork(
                            ...
                            false,
                            true,
                            { homeRam: 64, hashType: "money", hackLvl: 345 },
                        );
                    if (
                        player.factions.includes("The Black Hand") &&
                        (await hasAllAugs(ns, "The Black Hand"))
                    ...
                    if (
                        ...
                        (await proxy(
                            ...
                            "singularity.getFactionRep",
                            "Slum Snakes",
                        )) < (await maxRepNeeded(ns, "Slum Snakes"))
                    )
                        await factionWork(
                            ns,
                            "09/18",
                            ...
                            false,
                            true,
                            { job: "security", cstats: 30, karma: -9 },
                        );
                    else
                        await factionWork(
                            ns,
                    ...
                    if (
                        cctCost < 5200 &&
                        player.factions.includes("Tetrads") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "Tetrads",
                        )) < (await maxRepNeeded(ns, "Tetrads"))
                    )
                        await factionWork(
                            ns,
                            "10/18",
                    ...
                    if (
                        ...
                        await factionWork(
                            ...
                            false,
                            true,
                            { homeRam: 128, hashType: "money", hackLvl: 530 },
                        );
                }
                //Farm for a while
                    ...
                    if (maxHomeRam < 32768) {
                        ...
                        await factionWork(
                            ...
                            80,
                            false,
                            { hashBuy: 11e20, hashType: hashMode, hackLvl: 80 },
                        );
                    ...
                    if (
                        ...
                        await factionWork(
                            ...
                            false,
                            true,
                            { hashBuy: 1e21, augsAtOnce: 3, filterNFG: true },
                        );
                    else
                        await factionWork(
                            ns,
                    ...
                    if (
                        cctCost < 5400 &&
                        player.factions.includes("The Syndicate") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "The Syndicate",
                        )) < (await maxRepNeeded(ns, "The Syndicate"))
                    )
                        await factionWork(
                            ns,
                            "14/18",
                    ...
                    if (
                        cctCost < 5400 &&
                        player.factions.includes("Speakers for the Dead") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "Speakers for the Dead",
                        )) < (await maxRepNeeded(ns, "Speakers for the Dead"))
                    )
                        await factionWork(
                            ns,
                            "15/18",
                    ...
                    if (
                        cctCost < 5400 &&
                        player.factions.includes("The Covenant") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "The Covenant",
                        )) < (await maxRepNeeded(ns, "The Covenant"))
                    )
                        await factionWork(
                            ns,
                            "16/18",
                    ...
                    if (
                        cctCost < 5400 &&
                        player.factions.includes("Illuminati") &&
                        (await proxy(
                            ns,
                            "singularity.getFactionRep",
                            "Illuminati",
                        )) < (await maxRepNeeded(ns, "Illuminati"))
                    )
                        await factionWork(
                            ns,
                            "17/18",
                        ...
                        await factionWork(
                            ...
                            false,
                            false,
                            { cstats: 1200, hashBuy: 1e21, hashType: "train" },
                        );
        ...
        case 10:
            while (true) {
                ...
                await prime(ns);
                await getCommands(ns);
                const newestOwnedAugs = await proxy(
                    ns,
                    "singularity.getOwnedAugmentations",
                );
                if (HASBN9 && !(await hasAllAugs(ns, "Netburners")))
                    ...
                    if (
                        (await maxSleeves(ns)) === 8 &&
                        !(await allSleevesUpgraded(ns))
                    ) {
                        //
                        if (neuroAmount < 140)
                            await farmFaction(
                            ...
                            await factionWork(
                                ...
                                false,
                                true,
                                { cstats: 200, city: "Sector-12", karma: -90 },
                            );
                        else if (
                            !(await hasAllAugs(ns, "Speakers for the Dead"))
                        )
                            await factionWork(
                                ns,
                                "11/15",
                        ...
                        else if (!(await allSleevesUpgraded(ns))) {
                            //Covenant until we get all sleeves
                            const player = await proxy(ns, "getPlayer");
                            if (
                                player.factions.includes("The Covenant") &&
                                (await proxy(
                                ...
                                const pidof = await runIt(
                                    ns,
                                    "SphyxOS/sleeves/buyUpgradeSleeves.js",
                                    false,
                                    [],
                                );
                                if (pidof) {
                        ...
                        if (neuroAmount < 100)
                            ...
                            if (player.city !== "New Tokyo") {
                                ...
                                printLogs(
                                    ns,
                                    "14/15 - Trying to travel to New Tokyo!  In " +
                                        player.city,
                                );
                                await proxy(
                                    ns,
                            ...
                            printLogs(
                                ns,
                                "14/15 - Grafting QLink.  Please wait.",
                            );
                            await ns.sleep(1000);
                        } else
                            await farmFaction(
                ...
                if (!augments.includes("Nanofiber Weave")) {
                    ...
                    if (
                        ...
                        await factionWork(
                            ...
                            false,
                            true,
                            { hashBuy: 12e12, hashType: "None", buyRep: false },
                        );
                    else {
                        await factionWork(
                            ns,
                    ...
                    await factionWork(
                        ...
                        false,
                        false,
                    ); //, 0, "None", 1e12, 64, 0, 0, "none", 0, false)
                } else if (!(await hasAllAugs(ns, "BitRunners"))) {
                    if (MONEYCHANGE3) {
                        ns.writePort(12, "silent");
                        ns.writePort(12, "nomoney");
                    ...
                    if (
                        (await proxy(
                            ...
                            "singularity.getFactionRep",
                            "BitRunners",
                        )) < (await maxRepNeeded(ns, "BitRunners", false))
                    )
                        await factionWork(
                            ns,
                            "11/14",
        ...
        case 12:
            while (true) {
                ...
                await prime(ns);
                await getCommands(ns);
                if (!(await hasAllAugs(ns, "Netburners")))
                    //Netburners
                    await factionWork(
                        ns,
                        "1/8",
        ...
        case 13:
            while (true) {
                ...
                if (HASBN9 && !(await hasAllAugs(ns, "Netburners")))
                ...
                } else if (neuroAmount < 42) {
                    //CyberSec should be donatable after
                    ...
                    await factionWork(
                        ...
                        false,
                        false,
                        { cstats: 850, augsAtOnce: 3, filterNFG: true },
                    );
                else if (!(await hasAllAugs(ns, "Illuminati")))
                    await factionWork(
                        ns,
            ...
            while (true) {
                ...
                //await farmFaction(ns, "xx/xx", "CyberSec", true, false)
                    ...
                    await factionWork(
                        ...
                        false,
                        true,
                        { cstats: 300, hashBuy: 50e21, karma: -45, killed: 30 },
                    );
                } else if (!(await hasAllAugs(ns, "The Covenant"))) {
                    if (MONEYCHANGE2) {
                        ns.writePort(12, "silent");
...
async function prime(ns, purchaseServersOn = true, stocks = false) {
    const moneySources = await proxy(ns, "getMoneySources");
    if (HASBN9 && moneySources?.sinceInstall["hacknet"] < 1000000)
        await hashes(ns, 1e30, "money");
    if (!UPGRADED && moneySources?.sinceInstall.casino >= 10000000000) {
        if (
            (await proxy(ns, "singularity.getUpgradeHomeRamCost")) <= PRIMEMONEY
        )
            while (await proxy(ns, "singularity.upgradeHomeRam")) {
                UPGRADED = true;
            }
        if (stocks) {
            if (ns.ui.getGameInfo()?.versionNumber >= 44) {
                if (
                    !(await proxy(ns, "stock.hasWseAccount")) &&
                    (await proxy(ns, "stock.purchaseWseAccount"))
                )
                    UPGRADED = true;
                if (
                    !(await proxy(ns, "stock.hasTixApiAccess")) &&
                    (await proxy(ns, "stock.purchaseTixApi"))
                )
                    UPGRADED = true;
            } else {
                if (
...
async function hashes(ns, moneyCeiling, type = "coding") {
    if (
        HASBN9 &&
        (await proxy(ns, "getServerMoneyAvailable", "home")) >= moneyCeiling
    ) {
        const pidof = await runIt(
            ns,
            "SphyxOS/bins/hacknetPurchaser.js",
    ...
    if (HASBN9) await runIt(ns, "SphyxOS/extras/hashIt.js", false, [type]);
}
async function casino(ns) {
    ...
    if (CASINO === "Need" && ns.peek(10) === "NULL PORT DATA") {
        ...
        if (
            player.city !== "Aevum" &&
            (await proxy(ns, "singularity.travelToCity", "Aevum"))
        ) {
            if ((await proxy(ns, "getServerMoneyAvailable", "home")) >= 10000) {
                const pidof = await runIt(
                    ns,
                    "SphyxOS/cheats/casino.js",
                    false,
                    [],
                );
                if (pidof) {
            ...
            player.city === "Aevum" &&
            (await proxy(ns, "getServerMoneyAvailable", "home")) > 32000
        ) {
            const pidof = await runIt(
                ns,
                "SphyxOS/cheats/casino.js",
...
async function farmFaction(
    ...
    if (!player.factions.includes(faction)) {
        ...
        if (!player.factions.includes(faction)) {
            if (backdoor) {
                const pidof = await runIt(
                    ns,
                    "SphyxOS/bins/singularityBackdoor.js",
                    false,
                    ["quiet", "autopilot"],
                );
                if (pidof) {
            ...
            if (city !== "None") {
                if (
                    player.city !== city &&
                    !(await proxy(ns, "singularity.travelToCity", city))
                ) {
                    clearLogs(ns);
            ...
            if (player.factions.includes(faction))
                printLogs(
                    ns,
                    step + " Working for " + faction + " for it's NeuroFlux",
                );
            else {
        ...
        const pidof = await runIt(
            ns,
            "SphyxOS/bins/codingContracts.js",
            false,
            ["quiet"],
        );
        if (pidof) {
    ...
    if (
        (await proxy(ns, "singularity.getFactionFavor", faction)) <
            (FAVOR * 3) / 4 &&
        (await proxy(ns, "singularity.getFactionFavorGain", faction)) +
            (await proxy(ns, "singularity.getFactionFavor", faction)) >
            (FAVOR * 3) / 4
    ) {
        //Reset for favor
        ...
        (await proxy(ns, "singularity.getFactionFavor", faction)) >=
            (FAVOR * 3) / 4 &&
        (await proxy(ns, "singularity.getFactionFavor", faction)) < FAVOR &&
        (await proxy(ns, "singularity.getFactionFavorGain", faction)) +
            (await proxy(ns, "singularity.getFactionFavor", faction)) >
            FAVOR
    ) {
...
async function factionWork(
    ...
    if (!player.factions.includes(faction)) {
        ...
        if (moneySources?.sinceInstall.casino >= 10000000000) {
            ...
            if (skills.hacking < hacknetPurchase || skills.hacking < hackLvl) {
                if (
                    ...
                    !(await proxy(ns, "singularity.travelToCity", "Sector-12"))
                ) {
                    clearLogs(ns);
                ...
                clearLogs(ns);
                const highest =
                    hacknetPurchase > hackLvl ? hacknetPurchase : hackLvl;
            ...
            if (
                cstats > 0 &&
                (skills.agility < cstats ||
                    skills.strength < cstats ||
                    skills.defense < cstats ||
                    skills.dexterity < cstats)
            ) {
                if (
                    player.city !== "Sector-12" &&
                    !(await proxy(ns, "singularity.travelToCity", "Sector-12"))
            ...
            if (karma !== 0 && karma < 0 && ns.heart.break() > karma) {
                //One day there may be positive karma.  I know I tried once.
                clearLogs(ns);
        ...
        if (
            HASBN3 &&
            (await proxy(ns, "singularity.getFactionRep", faction)) <
                (await maxRepNeeded(ns, faction))
        ) {
        ...
        if (
            buyAugs &&
            !(await hasAllAugs(ns, faction)) &&
            (await proxy(ns, "singularity.getFactionRep", faction)) >=
                (await maxRepNeeded(ns, faction)) &&
            (await proxy(ns, "getServerMoneyAvailable", "home")) >
                (await maxMoneyNeeded(ns, faction))
        ) {
            let augsFromFaction = await proxy(
                ns,
                "singularity.getAugmentationsFromFaction",
                faction,
            );
            augsFromFaction = augsFromFaction.filter(
                (f) => f !== "NeuroFlux Governor",
            );
    ...
    if (
        ...
        if ((await proxy(ns, "singularity.getFactionRep", faction)) < maxRep) {
            ...
            const moneyAvailable = await proxy(
                ns,
                "getServerMoneyAvailable",
                "home",
            );
            await proxy(
                ns,
                "singularity.donateToFaction",
                faction,
                Math.min(moneyAvailable - moneySaved, maxDonate),
            );
        }
    }
...
async function gangCheck(ns) {
    ...
    const maxMoney = Math.min(
        await currentMaxUnlockCost(ns, "Slum Snakes"),
        8e9,
    );
    const hasMoney = myMoney >= maxMoney;
    const waiting = await augsWaiting(ns);
    if (
        ((await totalAugs(ns, "Slum Snakes")) === total &&
            total !== 0 &&
            hasMoney) ||
        (total + waiting > 11 && waiting === 0 && hasMoney) ||
        (total + waiting > 11 && waiting > 0)
    ) {
        await dump(ns);
    }
...
async function totalAugs(ns, faction, onlyUnlocked = false) {
    const allFacAugs = await proxy(
        ns,
        "singularity.getAugmentationsFromFaction",
        faction,
    );
    const allAugs = await proxy(ns, "singularity.getOwnedAugmentations", true);
    const factionAugs = allFacAugs.filter(
        (f) => f !== "NeuroFlux Governor" && !allAugs.includes(f),
    );
    let count = 0;
    const rep = await proxy(ns, "singularity.getFactionRep", faction);
...
async function endIt(ns) {
    ...
    if (maxRam < 256 && (await getServerAvailRam(ns, "home")) < 256) {
        ...
        if (MOVEON)
            await destroyWD(ns, nextBN, "SphyxOS/singularity/restart.js");
        else {
...
async function loadStanek(ns) {
    ...
    const files = homeFiles.map((m) => [
        m.substring(defaultFileLocation.length - 1),
        defaultFileLocation,
    ]);
    ...
    for (const testFile of files) {
        ...
        const [width, hight, ...fileName] = testFile[0]
            .substring(0, testFile[0].length - 4)
            .split("x");
        if (
            width <= (await proxy(ns, "stanek.giftWidth")) &&
            hight <= (await proxy(ns, "stanek.giftHeight")) &&
            fileName.includes("autoPilot")
        )
            usableFiles.push([
                width + "x" + hight + "x" + fileName.join("x"),
                testFile[1],
            ]);
    }
    ...
    if (chosen === "") {
        ...
        await proxy(
            ns,
            "scp",
            defaultFileLocation + chosen + ".txt",
            ns.self().server,
            "home",
        );
...
async function doCrime(ns, type, slv = null) {
    ...
    const me =
        slv === null
            ? await proxy(ns, "getPlayer")
            : await proxy(ns, "sleeve.getSleeve", slv);
    let bestRatio = 0;
    let bestCrime = "Mug";
    ...
    if (slv === null) {
        ...
        if (task?.crimeType !== bestCrime) {
            ...
            if (
                maxRam <
                ns.getFunctionRamCost("singularity.commitCrime") + 1.6
            ) {
...
function getChance(crimestats, person) {
    ...
    if (HASBN5)
        chance *= 1 + (1 * Math.pow(person.skills.intelligence, 0.8)) / 600;
    chance *= 100;
