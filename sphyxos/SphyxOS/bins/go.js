...
export async function main(ns) {
    ...
    while (true) {
        ...
        if (turn <= 1) results = await movePiece(ns, getOpeningMove(ns));
        else {
            switch (STYLE) {
                case 0: //Netburners
                case 1: //The Black Hand
                    if ((results = await movePiece(ns, getRandomCounterLib())))
                        break;
                    if ((results = await movePiece(ns, getRandomLibAttack(88))))
                        break;
                    if ((results = await movePiece(ns, getRandomLibDefend())))
                        break;
                    if ((results = await moveSnakeEyes(ns, getSnakeEyes(8))))
                        break;
                    if (
                        (results = await movePiece(ns, getAggroAttack(2, 2, 2)))
                    )
                        break;
                    ...
                    if (
                        ...
                        break;
                    if ((results = await movePiece(ns, getRandomBolster(2, 1))))
                        break;
                    if (
                        (results = await movePiece(
                            ns,
                        ...
                        break;
                    if ((results = await movePiece(ns, attackGrowDragon(1))))
                        break;
                    if ((results = await movePiece(ns, getDefAttack(8, 20, 2))))
                        break;
                    if ((results = await movePiece(ns, getRandomExpand())))
                        break;
                    if (
                        (results = await movePiece(
                            ns,
                        ...
                        break;
                    if ((results = await movePiece(ns, getRandomLibAttack())))
                        break;
                    if ((results = await movePiece(ns, getRandomStrat())))
                        break;
                    if ((results = await moveWallBreaker(ns, getWallBreaker())))
                        break;
                    if (
                        (results = await moveWallBreaker(ns, getWallBreaker(4)))
                    )
                        break;
                    if (
                        (results = await moveWallBreaker(
                            ns,
                            getWallBreaker(-1),
                        ))
                    )
                ...
                case 4: //Tetrads
                    ...
                    if ((results = await movePiece(ns, getDefPattern()))) break;
                    if (
                        (results = await movePiece(ns, getAggroAttack(3, 4, 3)))
                    )
                        break;
                    if ((results = await movePiece(ns, getRandomBolster(2, 1))))
                        break;
                    if (
                        (results = await movePiece(ns, getAggroAttack(5, 7, 3)))
                    )
                        break;
                    if ((results = await movePiece(ns, attackGrowDragon(1))))
                        break;
                ...
                case 6: //??????
                    ...
                    if ((results = await movePiece(ns, getRandomBolster(2, 1))))
                        break;
                    if ((results = await movePiece(ns, getDefAttack(5, 7, 3))))
                        break;
                    if ((results = await movePiece(ns, attackGrowDragon(1))))
                        break;
                    if ((results = await movePiece(ns, getRandomExpand())))
    ...
    for (let x = startx - depth; x <= startx + depth; x++)
        for (let y = starty - depth; y <= starty + depth; y++)
            if (
                ...
                count +=
                    board[x][y] === player ? 1.5 : board[x][y] === "." ? 1 : 0;
    return count;
}
/** @param {NS} ns */
...
function getDefAttack(
    ...
    for (const [x, y] of moves) {
        ...
        const atk =
            ((((friendlyLibs * chainAtk) / eyeValue) * getHeatMap(x, y, me)) /
                lowestLibs) *
