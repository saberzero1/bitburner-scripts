...
export async function main(ns) {
    ...
    //optionsDB = []
    if (globalThis["document"].autopilot)
        optionsDB["AutoPilotMoveOn"] = globalThis["document"].autopilot;
    await setOptionsDB(ns);

    if (ns.args.includes("BBRestart")) {
        //Restart from BB
        if (hasBN(resetInfo, sourceFiles, 10)) {
            //Sleeves
    ...
    if (ns.args.includes("autoPilot")) {
        //Restart from autoPilot
        if (hasBN(resetInfo, sourceFiles, 4, 2)) {
            ns.writePort(22, "silent");
            ns.writePort(
                22,
                optionsDB["AutoPilotMoveOn"] ? "moveon" : "nomoveon",
            );
    ...
    while (true) {
        ...
        const batcherUseHacknet = hasBN(resetInfo, sourceFiles, 9, 1) ? (
            <button
                style={optionsDB["BatcherUseHacknet"] ? greenStyle : redStyle}
                onClick={() => buttonBatcherUseHacknet(ns)}
            >
                {"Use Hacknet"}
            </button>
        ) : (
        ...
        const batcherAutoHash = hasBN(resetInfo, sourceFiles, 9, 1) ? (
            <button
                style={optionsDB["BatcherAutoHash"] ? greenStyle : redStyle}
                onClick={() => buttonBatcherAutoHash(ns)}
            >
                {"Auto Hash"}
            </button>
        ) : (
        ...
        const batcherChargeStanek = hasBN(resetInfo, sourceFiles, 13) ? (
            <button
                style={optionsDB["BatcherStanek"] ? greenStyle : redStyle}
                onClick={() => buttonBatcherToggleStanek(ns)}
            >
                {"Charge Stanek"}
            </button>
        ) : (
        ...
        const batcherAutoBuyHacknet = hasBN(resetInfo, sourceFiles, 9, 1) ? (
            <button
                style={
                    optionsDB["BatcherAutoBuyHacknet"] ? greenStyle : redStyle
                }
                onClick={() => buttonBatcherAutoBuyHacknet(ns)}
            >
                {"Batcher: AutoBuy"}
            </button>
        ) : (
        ...
        const miscBackdoorBasic = !hasBN(resetInfo, sourceFiles, 4, 2) ? (
            <button
                style={greenStyle}
                onClick={() => buttonMiscBackdoorBasic(ns)}
            >
                {"Backdoor"}
            </button>
        ) : (
        ...
        const miscBackdoorSingBasic = hasBN(resetInfo, sourceFiles, 4, 2) ? (
            <button
                style={greenStyle}
                onClick={() => buttonMiscBackdoorSing(ns)}
            >
                {"Backdoor Basic"}
            </button>
        ) : (
        ...
        const miscBackdoorSingAll = hasBN(resetInfo, sourceFiles, 4, 2) ? (
            <button
                style={greenStyle}
                onClick={() => buttonMiscBackdoorSing(ns, true)}
            >
                {"Backdoor All"}
            </button>
        ) : (
        ...
        const gangSleeves = hasBN(resetInfo, sourceFiles, 10, 1) ? (
            <button
                style={
                    optionsDB["SleeveMode"] === "Gangs" ? greenStyle : redStyle
                }
                onClick={() => buttonSleevesToggle(ns, "Gangs")}
            >
                {"Sleeves"}
            </button>
        ) : (
        ...
        const bbSleeves = hasBN(resetInfo, sourceFiles, 10, 1) ? (
            <button
                style={optionsDB["SleeveMode"] === "BB" ? greenStyle : redStyle}
                onClick={() => buttonSleevesToggle(ns, "BB")}
            >
                {"Sleeves"}
            </button>
        ) : (
        ...
        const bbInfilOnly = hasBN(resetInfo, sourceFiles, 10, 1) ? (
            <button
                style={optionsDB["BBInfilOnly"] ? greenStyle : redStyle}
                onClick={() => buttonBBInfilOnly(ns)}
            >
                {"Infil Only"}
            </button>
        ) : (
        ...
        const keepAlive =
            bbPlatform !== "Steam" ? (
                <button
                    style={wnd.keepAlive ? greenStyle : redStyle}
                    onClick={() => buttonMiscKeepAlive(ns)}
                >
                    {"Keep Tab Alive"}
                </button>
            ) : (
        ...
        const displayButtons = (
            <span>
                <button
                    style={
                        optionsDB["DisplayToggleHelper"] ? greenStyle : redStyle
                    }
                    onClick={() => buttonDisplayToggleHelper(ns)}
                >
                    {"Helper Text"}
                </button>
                <button
                    style={
                        optionsDB["DisplayToggleCollapse"]
                            ? greenStyle
                            : redStyle
                    }
                    onClick={() => buttonDisplayToggleCollapse(ns)}
                >
                    {"Collapsible"}
                </button>
                <button
                    style={greenStyle}
                    onClick={() => buttonDisplayOpenLogs(ns)}
                >
                    {"Open Logs"}
                </button>
                <button
                    style={greenStyle}
                    onClick={() => buttonDisplayClearActive(ns)}
                >
                    {"Clear Active"}
                </button>
                <button
                    style={greenStyle}
                    onClick={() => buttonDisplayRefresh(ns)}
                >
                    {"Refresh"}
                </button>
                <br></br>
                <button
                    style={greenStyle}
                    onClick={() => buttonDisplayUpdate(ns)}
                >
                    {"Update"}
                </button>
                <button style={greenStyle} onClick={() => buttonChangeLog(ns)}>
                    {"Change Log"}
                </button>
                <button
                    style={greenStyle}
                    onClick={() => buttonDisplayRemove(ns)}
                >
                    {"REMOVE PROGRAM"}
                </button>
            </span>
        ...
        const corp = hasCorp
            ? await proxy(ns, "corporation.getCorporation")
            : false;
        const player = await proxy(ns, "getPlayer");
        const batcherHelp = hasBN(resetInfo, sourceFiles, 13)
            ? "Get Money and/or XP  Charge Stanek between cycles  Pop out the log"
            : "Get Money and/or XP  Pop out the log";
        let corpBribeName = "Bribe Unavailable";
        if (
            corp &&
            corp.valuation >= 100000000000000 &&
        ...
        const rows = [
            [
                ...
                <span>
                    <button
                        style={
                            ns.peek(2) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonBatcherStart(ns)}
                    >
                        {ns.peek(2) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    <button
                        style={
                            optionsDB["BatcherAutoBuyServers"]
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonBatcherAutoBuyServers(ns)}
                    >
                        {"Auto-Buy Servers"}
                    </button>
                    {batcherUseHacknet}
                    {batcherAutoHash}{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Activate/Deactivate the batcher and it's helpers"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={
                            optionsDB["BatcherMoney"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonBatcherToggleMoney(ns)}
                    >
                        {"Money Mode"}
                    </button>
                    <button
                        style={optionsDB["BatcherXP"] ? greenStyle : redStyle}
                        onClick={() => buttonBatcherToggleXP(ns)}
                    >
                        {"XP Mode"}
                    </button>
                    {batcherChargeStanek}
                    <button
                        style={optionsDB["BatcherPad"] ? greenStyle : redStyle}
                        onClick={() => buttonBatcherPad(ns)}
                    >
                        {"Pad Grows"}
                    </button>
                    <button
                        style={optionsDB["BatcherLog"] ? greenStyle : redStyle}
                        onClick={() => buttonBatcherLog(ns)}
                    >
                        {"LogErrors"}
                    </button>
                    <button
                        style={
                            optionsDB["BatcherPopout"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonPopout(ns, "Batcher")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] && batcherHelp}
                </span>,
            ],
            [
                "Hacknet",
                ...
                <span>
                    <button
                        style={greenStyle}
                        onClick={() => buttonHacknetBuyHacknet(ns)}
                    >
                        {"Buy Hacknet"}
                    </button>
                    {batcherAutoBuyHacknet}{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Buy Hacknet or allow batcher to auto buy if unlocked"}
                </span>,
            ],
            [
                "Hashing",
                ...
                <span>
                    <button
                        style={greenStyle}
                        onClick={() => buttonHashing(ns, "money")}
                    >
                        {"Money"}
                    </button>
                    <button
                        style={
                            ns.peek(3) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonHashing(ns, "min")}
                    >
                        {"Reduce Min Sec"}
                    </button>
                    <button
                        ...
                        onClick={() => buttonHashing(ns, "max")}
                    >
                        {"Boost Max Money"}
                    </button>
                    <button
                        style={greenStyle}
                        onClick={() => buttonHashing(ns, "coding")}
                    >
                        {"Generate Contract"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Sec and Money require a batcher target"}{" "}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={hasCorp ? greenStyle : redStyle}
                        onClick={() => buttonHashing(ns, "corp")}
                    >
                        {"Corp Money"}
                    </button>
                    <button
                        style={hasCorp ? greenStyle : redStyle}
                        onClick={() => buttonHashing(ns, "research")}
                    >
                        {"Corp Research"}
                    </button>
                    <button
                        style={hasBB ? greenStyle : redStyle}
                        onClick={() => buttonHashing(ns, "bbrank")}
                    >
                        {"Boost BB Rank"}
                    </button>
                    <button
                        style={hasBB ? greenStyle : redStyle}
                        onClick={() => buttonHashing(ns, "bbsp")}
                    >
                        {"Boost BB SP"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] && "Targets corp and BB"}{" "}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={greenStyle}
                        onClick={() => buttonHashing(ns, study)}
                    >
                        {"Boost Study"}
                    </button>
                    <button
                        style={greenStyle}
                        onClick={() => buttonHashing(ns, "train")}
                    >
                        {"Boost Train"}
                    </button>
                    <button
                        style={greenStyle}
                        onClick={() => buttonHashing(ns, "favor")}
                    >
                        {"Boost Job Favor"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Targets training, study and jobs"}
                </span>,
            ],
            [
                "Stocks",
                ...
                <span>
                    <button
                        style={
                            ns.peek(4) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonStocksStart(ns)}
                    >
                        {ns.peek(4) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    <button
                        ...
                        onClick={() => buttonStocksSell(ns)}
                    >
                        {"Sell"}
                    </button>
                    <button
                        style={
                            optionsDB["StocksToggleAutoBuy"]
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonStocksToggleAutoBuy(ns)}
                    >
                        {"Toggle AutoBuy"}
                    </button>
                    <button
                        style={greenStyle}
                        onClick={() => buttonStocksReset(ns)}
                    >
                        {"Reset Stats"}
                    </button>
                    <button
                        style={
                            optionsDB["StocksPopOut"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonPopout(ns, "Stocks")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Activate, buy and sell stocks. Set it to auto buy. Pop out the log."}
                </span>,
            ],
            [
                "Misc",
                ...
                <span>
                    ...
                    <button
                        style={greenStyle}
                        onClick={() => buttonMiscTeleport(ns)}
                    >
                        {"Teleport"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Backdoor servers, or move to a server"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={greenStyle}
                        onClick={() => buttonMiscSolveContracts(ns)}
                    >
                        {"Solve Contracts"}
                    </button>
                    <button
                        style={optionsDB["ShareMode"] ? greenStyle : redStyle}
                        onClick={() => buttonMiscShareRam(ns)}
                    >
                        {optionsDB["ShareMode"] ? "Unshare Ram" : "Share Ram"}
                    </button>
                    {keepAlive}{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Contracts solve, Share all ram and keep tab alive"}
                </span>,
            ],
            [
                "Singularity",
                ...
                <span>
                    <button
                        style={greenStyle}
                        onClick={() => buttonSingDumpMoney(ns)}
                    >
                        {"Dump Money"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Spend all your money on augments and home upgrades"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={
                            ns.peek(21) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonAutoPilot(ns)}
                    >
                        {"AutoPilot V1"}
                    </button>
                    <button
                        style={
                            optionsDB["AutoPilotMoveOn"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonAutoPilotMoveOn(ns)}
                    >
                        {"Start On Next"}
                    </button>
                    <button
                        style={
                            optionsDB["AutoPilotPopOut"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonPopout(ns, "AutoPilot")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Takes full control of the game and runs you through the node   Start scripts on the next node  Pop out the log window"}
                </span>,
            ],
            [
                "IPvGo",
                ...
                <span>
                    <button
                        style={
                            ns.peek(5) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonIPvGoStart(ns)}
                    >
                        {ns.peek(5) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    <button
                        style={
                            optionsDB["IPvGoPlayAsWhite"]
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonIPvGoPlayWhite(ns)}
                    >
                        {"Play White"}
                    </button>
                    <button
                        style={optionsDB["IPvGoRepeat"] ? greenStyle : redStyle}
                        onClick={() => buttonIPvGoRepeat(ns)}
                    >
                        {"Repeat"}
                    </button>
                    <button
                        style={optionsDB["IPvGoCheats"] ? greenStyle : redStyle}
                        onClick={() => buttonIPvGoCheats(ns)}
                    >
                        {"Cheats"}
                    </button>
                    <button
                        style={
                            optionsDB["IPvGoLogging"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoLogging(ns)}
                    >
                        {"Logging"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Start and toggle IPvGo settings"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={
                            optionsDB["IPvGoNetburners"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoNetburners(ns)}
                    >
                        {"Netburners"}
                    </button>
                    <button
                        style={
                            optionsDB["IPvGoSlumSnakes"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoSlumSnakes(ns)}
                    >
                        {"Slum Snakes"}
                    </button>
                    <button
                        ...
                        onClick={() => buttonIPvGoTheBlackHand(ns)}
                    >
                        {"The Black Hand"}
                    </button>
                    <button
                        style={
                            optionsDB["IPvGoTetrads"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoTetrads(ns)}
                    >
                        {"Tetrads"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] && "Toggleable opponents"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={
                            optionsDB["IPvGoDaedalus"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoDaedalus(ns)}
                    >
                        {"Daedalus"}
                    </button>
                    <button
                        style={
                            optionsDB["IPvGoIlluminati"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoIlluminati(ns)}
                    >
                        {"Illuminati"}
                    </button>
                    <button
                        style={
                            optionsDB["IPvGoUnknown"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoUnknown(ns)}
                    >
                        {"????????"}
                    </button>
                    <button
                        style={optionsDB["IPvGoNoAI"] ? greenStyle : redStyle}
                        onClick={() => buttonIPvGoNoAI(ns)}
                    >
                        {"No AI"}
                    </button>
                    <button
                        style={
                            optionsDB["IPvGoSlowMode"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonIPvGoSlowMode(ns)}
                    >
                        {"SlowMode"}
                    </button>
                    <button
                        style={optionsDB["IPvGoPopOut"] ? greenStyle : redStyle}
                        onClick={() => buttonPopout(ns, "IPvGo")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Toggleable opponents. ??? is after fl1ght. No AI will only play on practice board.  Create a delay before making moves.  Popout the log file."}
                </span>,
            ],
            [
                "Gangs",
                ...
                <span>
                    <button
                        style={
                            ns.peek(6) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonGangStart(ns)}
                    >
                        {ns.peek(6) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    <button
                        style={
                            optionsDB["GangAutoAscend"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonGangAutoAscend(ns)}
                    >
                        {"Auto-Ascend"}
                    </button>
                    <button
                        style={optionsDB["GangAutoEQ"] ? greenStyle : redStyle}
                        onClick={() => buttonGangAutoEQ(ns)}
                    >
                        {"Auto-EQ"}
                    </button>
                    {gangSleeves}{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Start and toggle Automatic Gang settings"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={
                        ...
                        onClick={() => buttonGangMode(ns, "AutoMode")}
                    >
                        {"AutoMode"}
                    </button>
                    <button
                        ...
                        onClick={() => buttonGangMode(ns, "Respect")}
                    >
                        {"Respect"}
                    </button>
                    <button
                        ...
                        onClick={() => buttonGangMode(ns, "Money")}
                    >
                        {"Money"}
                    </button>
                    {optionsDB["DisplayToggleHelper"] &&
                        "Set gang to earn Respect, make money or auto"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={greenStyle}
                        onClick={() => buttonGangBuyEQ(ns)}
                    >
                        {"Buy EQ All"}
                    </button>
                    <button
                        style={greenStyle}
                        onClick={() => buttonGangAscend(ns)}
                    >
                        {"Ascend All"}
                    </button>
                    <button
                        style={optionsDB["GangPopOut"] ? greenStyle : redStyle}
                        onClick={() => buttonPopout(ns, "Gang")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Buy gear for / or ascend everyone"}
                </span>,
            ],
            [
                "Corps",
                ...
                <span>
                    <button
                        style={
                            ns.peek(9) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonCorpStart(ns)}
                    >
                        {ns.peek(9) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    <button
                        style={greenStyle}
                        onClick={() => buttonCorpResetTAII(ns)}
                    >
                        {"Reset TAII"}
                    </button>
                    <button
                        style={
                            corpBribeName === "Bribe" ? greenStyle : redStyle
                        }
                        onClick={() => buttonCorpBribe(ns)}
                    >
                        {corpBribeName}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Activate Corps, Reset TAII DB, Bribe factions"}
                </span>,
            ],
            [
                "BladeBurner",
                ...
                <span>
                    <button
                        style={
                            ns.peek(8) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonBBStart(ns)}
                    >
                        {ns.peek(8) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    <button
                        style={optionsDB["BBFinisher"] ? greenStyle : redStyle}
                        onClick={() => buttonBBFinisher(ns)}
                    >
                        {"Finisher"}
                    </button>
                    <button
                        style={optionsDB["BBIntMode"] ? greenStyle : redStyle}
                        onClick={() => buttonBBIntMode(ns)}
                    >
                        {"Int Mode"}
                    </button>
                    {bbSleeves}
                    {bbInfilOnly}
                    <button
                        style={optionsDB["BBPopOut"] ? greenStyle : redStyle}
                        onClick={() => buttonPopout(ns, "BB")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Start BB and sets modes. Finisher will end BN and start a new one, int trains int,  PopOut the log"}
                </span>,
            ],
            [
                "Stanek",
                ...
                <span>
                    <button
                        style={
                            ns.peek(11) !== "NULL PORT DATA"
                                ? redStyle
                                : greenStyle
                        }
                        onClick={() => buttonStanekStart(ns)}
                    >
                        {ns.peek(11) === "NULL PORT DATA"
                            ? "Charge"
                            : "Pls Wait"}
                    ...
                    <button
                        style={greenStyle}
                        onClick={() => buttonStanekSaveConfig(ns)}
                    >
                        {"Save Config"}
                    </button>
                    <button
                        style={greenStyle}
                        onClick={() => buttonStanekLoadConfig(ns)}
                    >
                        {"Load Config"}
                    </button>
                    <button
                        style={
                            optionsDB["StanekDefault"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonStanekUseDefault(ns)}
                    >
                        {"Defaults"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Save/Load your config. Allow defaults to show when loading"}
                </span>,
            ],
            [
                "Sleeves",
                ...
                <span>
                    <button
                        style={
                            ns.peek(7) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonSleeveStart(ns)}
                    >
                        {ns.peek(7) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    <button
                        style={
                            optionsDB["SleeveMode"] === "Recovery"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonSleevesToggle(ns, "Recovery")}
                    >
                        {"Recovery"}
                    </button>
                    <button
                        style={
                            optionsDB["SleeveMode"] === "Sync"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonSleevesToggle(ns, "Sync")}
                    >
                        {"Sync"}
                    </button>
                    <button
                        style={
                            optionsDB["SleeveMode"] === "Training"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonSleevesToggle(ns, "Training")}
                    >
                        {"Train"}
                    </button>
                    <button
                        style={
                            optionsDB["SleeveInstall"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonSleevesInstallAugments(ns)}
                    >
                        {"Install Augments"}
                    </button>
                    {optionsDB["DisplayToggleHelper"] &&
                        "Start sleeve manager and toggle modes.  Install augments for all sleeves"}
                    <br></br>
                    {!optionsDB["DisplayToggleCollapse"] && mainBuffer}
                    <button
                        style={
                            optionsDB["SleeveMode"] === "Money"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonSleevesToggle(ns, "Money")}
                    >
                        {"Money"}
                    </button>
                    <button
                        style={
                            optionsDB["SleeveMode"] === "Karma"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonSleevesToggle(ns, "Karma")}
                    >
                        {"Karma"}
                    </button>
                    <button
                        ...
                        onClick={() => buttonSleevesToggle(ns, "Idle")}
                    >
                        {"Idle"}
                    </button>
                    <button
                        style={
                            optionsDB["SleeveMode"] === "Int"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonSleevesToggle(ns, "Int")}
                    >
                        {"Int"}
                    </button>
                    <button
                        style={
                            optionsDB["SleevePopOut"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonPopout(ns, "Sleeve")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Toggle more sleeve training options"}
                </span>,
            ],
            [
                "Grafting",
                ...
                <span>
                    <button
                        style={greenStyle}
                        onClick={() => buttonGrafting(ns)}
                    >
                        {"Grafting"}
                    </button>
                    <button
                        style={
                            optionsDB["GraftingPopOut"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonPopout(ns, "Grafting")}
                    >
                        {"Pop Out"}
                    </button>{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Start the Auto Grafter"}
                </span>,
            ],
            [
                "Cheats",
                ...
                <span>
                    {optionsDB["DisplayToggleCollapse"] &&
                        "Dev/Unlocks" + getBuffer(11)}
                    <button
                        style={greenStyle}
                        onClick={() => buttonDevMenu(ns)}
                    >
                        {"Dev Menu"}
                    </button>
                    <button
                        style={greenStyle}
                        onClick={() => buttonUnlockAll(ns)}
                    >
                        {"Unlock All Achievements"}
                    </button>
                    {optionsDB["DisplayToggleHelper"] &&
                        "Open the Dev Menu or give yourself all achievements"}
                    <br></br>
                    {"Casino" + getBuffer(6)}
                    <button
                        style={
                            ns.peek(10) !== "NULL PORT DATA"
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonCasinoStart(ns)}
                    >
                        {ns.peek(10) === "NULL PORT DATA"
                            ? "Activate"
                            : "De-Activate"}
                    ...
                    {optionsDB["DisplayToggleHelper"] &&
                        "Move to Aevum, start script, it will end. Move into Casino, restart script while in casino"}
                    <br></br>
                    {"AutoInfil" + getBuffer(9)}
                    <button
                        style={wnd.tmrAutoInf ? greenStyle : redStyle}
                        onClick={() => buttonAutoInfilStart(ns)}
                    >
                        {!wnd.tmrAutoInf ? "Activate" : "De-Activate"}
                    </button>
                    <button
                        style={
                            optionsDB["AutoInfilAuto"] ? greenStyle : redStyle
                        }
                        onClick={() => buttonAutoInfilAuto(ns)}
                    >
                        {"Auto"}
                    </button>
                    <button
                        style={
                            optionsDB["AutoInfilMoneyMode"]
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonAutoInfilMoney(ns)}
                    >
                        {"Money"}
                    </button>
                    <button
                        style={
                            optionsDB["AutoInfilFactionMode"]
                                ? greenStyle
                                : redStyle
                        }
                        onClick={() => buttonAutoInfilFaction(ns)}
                    >
                        {"Faction"}
                    </button>
                    {optionsDB["AutoInfilFaction"]}{" "}
                    {optionsDB["DisplayToggleHelper"] &&
                        "Start Auto Infil, switch between modes.  Enter a companies Infiltrate screen to start"}
                </span>,
            ],
        ];
        ns.printRaw(
        ...
        for (const [sendTitle, bn, lvl, buttonsList] of rows) {
            if (
                hasBN(resetInfo, sourceFiles, bn, lvl) ||
                (bn === 6 && hasBN(resetInfo, sourceFiles, 7, lvl))
            )
                ns.printRaw(
                    <Row
                        title={sendTitle.toString()}
                        buttons={buttonsList}
                    ></Row>,
                );
        }
...
function processCommands(ns) {
    while (ns.peek(1) !== "NULL PORT DATA") {
        ...
        if (result !== 1 && result !== true) {
            // 1 and true are used to just cycle the display, anythign else is state communication
            //ns.tprintf(result)
            switch (result) {
                ...
                default:
                    ns.tprintf(
                        "Invalid response received in autopilot: %s",
                        result,
                    );
                    break;
            }
...
function getBuffer(startValue, endValue = 12) {
    let buffer = "";
    for (let i = startValue; i < endValue; i++) buffer += " ";
    buffer += ":";
    return buffer;
}
...
async function buttonDisplayClearActive(ns) {
    if (ns.peek(2) !== "NULL PORT DATA" && ns.peek(2) > 0)
        await proxy(ns, "kill", ns.peek(2)); // Puppet
    if (ns.peek(4) !== "NULL PORT DATA" && ns.peek(4) > 0)
        await proxy(ns, "kill", ns.peek(4)); // Stocks
    if (ns.peek(5) !== "NULL PORT DATA" && ns.peek(5) > 0)
        await proxy(ns, "kill", ns.peek(5)); // IPvGo
    if (ns.peek(6) !== "NULL PORT DATA" && ns.peek(6) > 0)
        await proxy(ns, "kill", ns.peek(6)); // Gangs
    if (ns.peek(7) !== "NULL PORT DATA" && ns.peek(7) > 0)
        await proxy(ns, "kill", ns.peek(7)); // Sleeves
    if (ns.peek(8) !== "NULL PORT DATA" && ns.peek(8) > 0)
        await proxy(ns, "kill", ns.peek(8)); // BB
    if (ns.peek(9) !== "NULL PORT DATA" && ns.peek(9) > 0)
        await proxy(ns, "kill", ns.peek(9)); // Corps
    if (ns.peek(10) !== "NULL PORT DATA" && ns.peek(10) > 0)
        await proxy(ns, "kill", ns.peek(10)); // Casino
    if (ns.peek(20) !== "NULL PORT DATA" && ns.peek(20) > 0)
        await proxy(ns, "kill", ns.peek(20)); // Casino
    if (ns.peek(30) !== "NULL PORT DATA" && ns.peek(30) > 0)
...
function buttonChangeLog(ns) {
    ...
    if (updatePid === 0)
        ns.tprintf("Error:  Not enough RAM to open change log.");
}
async function buttonDisplayRemove(ns) {
    const result = await ns.prompt("Are you sure?", { type: "boolean" });
    if (result === true) {
        const localStorage = !!(await ns.prompt(
            "Local Storage(Stanek loadouts, etc) too?",
            { type: "boolean" },
        ));
        ...
        ns.toast(
            "Not enough free RAM to run the removal script.",
            "error",
            3000,
        );
    }
    ...
    switch (program) {
        case "Batcher":
            ...
            if (ns.peek(2) !== "NULL PORT DATA")
                optionsDB["BatcherPopout"] === true
                    ? ns.writePort(12, "popout")
                    : ns.writePort(12, "nopopout");
            break;
        case "AutoPilot":
            ...
            if (ns.peek(21) !== "NULL PORT DATA")
                ns.writePort(
                    22,
                    optionsDB["AutoPilotPopOut"] ? "popout" : "nopopout",
                );
            break;
        case "Stocks":
            ...
            if (ns.peek(4) !== "NULL PORT DATA")
                ns.writePort(
                    13,
                    optionsDB["StocksPopOut"] ? "popout" : "nopopout",
                );
            break;
        case "IPvGo":
            ...
            if (ns.peek(5) !== "NULL PORT DATA")
                ns.writePort(
                    15,
                    optionsDB["IPvGoPopOut"] ? "popout" : "nopopout",
                );
            break;
        case "BB":
        ...
        case "Gang":
            ...
            if (ns.peek(6) !== "NULL PORT DATA")
                ns.writePort(
                    16,
                    optionsDB["GangPopOut"] ? "popout" : "nopopout",
                );
            break;
        case "Grafting":
            ...
            if (ns.peek(20) !== "NULL PORT DATA")
                ns.writePort(
                    23,
                    optionsDB["GraftingPopOut"] ? "popout" : "nopopout",
                );
            break;
        case "Sleeve":
            ...
            if (ns.peek(7) !== "NULL PORT DATA")
                ns.writePort(
                    17,
                    optionsDB["SleevePopOut"] ? "popout" : "nopopout",
                );
            break;
        default:
...
async function buttonMiscBackdoorSing(ns, all = false) {
    if (all)
        await runIt(ns, "SphyxOS/bins/singularityBackdoor.js", true, ["all"]);
...
async function buttonMiscShareRam(ns) {
    ...
    if (optionsDB["ShareMode"])
        await runIt(ns, "SphyxOS/bins/startShare.js", false, []);
...
async function buttonIPvGoStart(ns) {
    if (ns.peek(5) !== "NULL PORT DATA") {
        ...
        ns.writePort(
            15,
            optionsDB["IPvGoLogging"] ? "Logging On" : "Logging Off",
        );
        ...
        ns.writePort(
            15,
            optionsDB["IPvGoTetrads"] ? "Tetrad On" : "Tetrad Off",
        );
        ns.writePort(15, optionsDB["IPvGoDaedalus"] ? "Daed On" : "Daed Off");
        ns.writePort(
            15,
            optionsDB["IPvGoIlluminati"] ? "Illum On" : "Illum Off",
        );
...
function buttonIPvGoPlayWhite(ns) {
    ...
    if (!optionsDB["IPvGoPlayAsWhite"]) {
        ...
        optionsDB["IPvGoNoAI"] = true;
        if (ns.peek(5) !== "NULL PORT DATA")
            ns.writePort(15, "Play as White On");
...
function buttonIPvGoSlowMode(ns) {
    ...
    if (ns.peek(5) !== "NULL PORT DATA")
        ns.writePort(
            15,
            optionsDB["IPvGoSlowMode"] ? "SlowMode On" : "SlowMode Off",
        );
    ns.writePort(1, true);
}
async function buttonGangStart(ns) {
    if (ns.peek(6) !== "NULL PORT DATA") {
        ...
        ns.writePort(
            16,
            optionsDB["GangAutoAscend"] ? "AutoAscend On" : "AutoAscend Off",
        );
        ns.writePort(16, optionsDB["GangAutoEQ"] ? "AutoEQ On" : "AutoEQ Off");
        ns.writePort(16, optionsDB["GangMode"]);
        ns.writePort(
            16,
            optionsDB["SleeveMode"] === "Gangs" ? "Sleeves On" : "Sleeves Off",
        );
...
async function buttonCorpBribe(ns) {
    ...
    if (corp && corp.valuation >= 100000000000000) {
        const faction = await ns.prompt("Choose a faction to bribe:", {
            type: "select",
            choices: player.factions,
        });
        if (faction === "") return;
...
async function buttonSleeveStart(ns) {
    if (ns.peek(7) !== "NULL PORT DATA") {
        ...
        ns.writePort(
            17,
            optionsDB["SleeveInstall"] ? "Install On" : "Install Off",
        );
...
async function buttonSleevesToggle(ns, mode) {
    switch (
        optionsDB["SleeveMode"] // Turn off the one that's on since it's changing
    ) {
        case "Gangs":
            if (ns.peek(6) !== "NULL PORT DATA")
    ...
    if (optionsDB["SleeveMode"] === mode || mode === "Idle") {
        //turn it off and set to idle if it's the same one
        ...
        switch (
            mode // Turn on the new one
        ) {
            case "Gangs":
                if (ns.peek(6) !== "NULL PORT DATA")
                    ns.writePort(16, "Sleeves On");
                break;
            case "BB":
                if (ns.peek(8) !== "NULL PORT DATA")
                    ns.writePort(18, "sleeves on");
                break;
            case "Training":
            case "Recovery":
            case "Sync":
            ...
            case "Karma":
            case "Int":
                if (ns.peek(7) !== "NULL PORT DATA") ns.writePort(17, mode);
                break;
            case "Idle":
...
async function buttonBBStart(ns) {
    if (ns.peek(8) !== "NULL PORT DATA") {
        ...
        ns.writePort(
            18,
            optionsDB["BBFinisher"] ? "finisher on" : "finisher off",
        );
        ns.writePort(
            18,
            optionsDB["BBIntMode"] ? "int mode on" : "int mode off",
        );
        ns.writePort(
            18,
            optionsDB["SleeveMode"] === "BB" ? "sleeves on" : "sleeves off",
        );
        ns.writePort(
            18,
            optionsDB["BBInfilOnly"] ? "sleeve infil on" : "sleeve infil off",
        );
...
function buttonStanekLoadConfig(ns) {
    ...
    else ns.exec("SphyxOS/stanek/loadStanek.js", "home", 1);
    ns.writePort(1, true);
}
...
async function buttonAutoInfilStart(ns) {
    if (wnd.tmrAutoInf) {
        ...
        //Start it
        if (!optionsDB["AutoInfilAuto"])
            ns.writePort(
                30,
                await runIt(ns, "SphyxOS/cheats/autoInfil.js", false, []),
            );
        else if (optionsDB["AutoInfilMoneyMode"])
            ns.writePort(
                30,
                await runIt(ns, "SphyxOS/cheats/autoInfil.js", false, [
                    "--auto",
...
async function buttonAutoInfilMoney(ns) {
    ...
    if (optionsDB["AutoInfilMoneyMode"]) {
        //Already on, just abort
        return;
    } else {
        //Turn it on
...
async function buttonAutoInfilFaction(ns) {
    ...
    const factions = player.factions.filter(
        (f) =>
            ![
                gangFac,
                "Bladeburners",
    ...
    if (factions.length === 0) {
        ...
        optionsDB["AutoInfilFaction"] = await ns.prompt("Select Faction", {
            type: "select",
            choices: factions,
        });
    else optionsDB["AutoInfilFaction"] = factions.pop();
...
async function setOptionsDB(ns) {
    ...
    if (optionsDB["BatcherMoney"] === undefined)
        optionsDB["BatcherMoney"] = true;
    if (optionsDB["BatcherXP"] === undefined) optionsDB["BatcherXP"] = true;
    if (optionsDB["BatcherStanek"] === undefined)
        optionsDB["BatcherStanek"] = false;
    if (optionsDB["BatcherPad"] === undefined) optionsDB["BatcherPad"] = false;
    if (optionsDB["BatcherLog"] === undefined) optionsDB["BatcherLog"] = false;
    ...
    if (optionsDB["IPvGoPlayAsWhite"] === undefined)
        optionsDB["IPvGoPlayAsWhite"] = false;
    if (optionsDB["IPvGoRepeat"] === undefined) optionsDB["IPvGoRepeat"] = true;
    if (optionsDB["IPvGoCheats"] === undefined) optionsDB["IPvGoCheats"] = true;
    ...
    if (optionsDB["IPvGoUnknown"] === undefined)
        optionsDB["IPvGoUnknown"] = true;
    if (optionsDB["IPvGoNoAI"] === undefined) optionsDB["IPvGoNoAI"] = false;
    ...
    if (optionsDB["GangAutoAscend"] === undefined)
        optionsDB["GangAutoAscend"] = true;
    if (optionsDB["GangAutoEQ"] === undefined) optionsDB["GangAutoEQ"] = true;
    if (optionsDB["GangMode"] === undefined) optionsDB["GangMode"] = "AutoMode";
    if (optionsDB["GangPopOut"] === undefined) optionsDB["GangPopOut"] = false;
    ...
    if (optionsDB["BBFinisher"] === undefined) optionsDB["BBFinisher"] = false;
    if (optionsDB["BBIntMode"] === undefined) optionsDB["BBIntMode"] = false;
    if (optionsDB["BBInfilOnly"] === undefined)
        optionsDB["BBInfilOnly"] = false;
    if (optionsDB["BBPopOut"] === undefined) optionsDB["BBPopOut"] = false;
    ...
    if (optionsDB["AutoInfilFaction"] === undefined)
        optionsDB["AutoInfilFaction"] = "";
    if (optionsDB["ShareMode"] === undefined) optionsDB["ShareMode"] = false;
...
function hasBN(resetInfo, sourceFiles, bn, sfLvl = 1) {
    ...
    try {
        for (const sf of sourceFiles)
            if (sf.n === bn && sf.lvl >= sfLvl) return true;
        return false;
    } catch {
        return false;
    }
        ...
        return (
            <div>
                <details
                    open
                    onToggle={(e) => {
                        if (e.currentTarget.open) openDB.add(title);
