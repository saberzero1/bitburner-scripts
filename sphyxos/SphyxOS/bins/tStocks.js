import { hasWSEAcct, hasTIXAPIAccs, getSyms, getSalesGain, shortEnabled, has4SAPI, getmaxshares } from "SphyxOS/util.js"
...
async function printLogs(ns, stocks) {
  ...
  update(ns, ns.sprintf("├───────────────┴─────────┼─────────┴─────────┼──────────┴─────────┼───────────────┘"))
  if (ns.ui.getGameInfo()?.versionNumber >= 44) update(ns, ns.sprintf("│Sales: %18s│Fees: %13s│Banked: %12s│", "$" + ns.format.number(RUNNINGTOTAL, 2), "$" + ns.format.number(RUNNINGCOST, 2), "$" + ns.format.number(RUNNINGBANKED, 2)))
  else update(ns, ns.sprintf("│Sales: %18s│Fees: %13s│Banked: %12s│", "$" + ns.formatNumber(RUNNINGTOTAL, 2), "$" + ns.formatNumber(RUNNINGCOST, 2), "$" + ns.formatNumber(RUNNINGBANKED, 2)))
