...
export async function main(ns) {
    ...
    let SPECIFIC = await ns.prompt("Select Augment:", {
        type: "select",
        choices: graftable,
    });

    if (SPECIFIC === "" || SPECIFIC === "Done") ns.exit();
    queue.push(SPECIFIC);
    while (SPECIFIC !== "" && SPECIFIC !== "Done") {
        SPECIFIC = await ns.prompt("Select Next Queued Augment:", {
            type: "select",
            choices: graftable.filter((f) => !queue.includes(f)),
        });
    ...
    while (!ready) {
        ...
        update(ns, ns.sprintf("Neuroreceptor Management Implant: %s", !FOCUS));
        if (FOCUS)
            update(ns, "Please remain focused or your time will increase.");
        else
            update(
                ns,
                "You can unfocus and do other things without penalty now.",
            );
        update(ns, "Grafting: Unknown");
        update(ns, "Will take a min of Unknown.");
        update(ns, "Script will continue once this graft is finished.");
    ...
    while (queue.length > 0) {
        ...
        if (city !== "New Tokyo") {
            while (city !== "New Tokyo") {
                ...
                update(
                    ns,
                    ns.sprintf(
                        "You need to travel to New Tokyo!  Currently in %s",
                        city,
                    ),
                );
        ...
        if (FOCUS)
            update(
                ns,
                ns.sprintf("Please remain focused or your time will increase."),
            );
        else
            update(
                ns,
        ...
        const graftTime = await proxyTry(
            ns,
        ...
        if (graftTime) {
            if (ns.ui.getGameInfo()?.versionNumber >= 44)
                update(
                    ns,
                    ns.sprintf(
                        "Will take a min of %s",
                        ns.format.time(graftTime),
                    ),
                );
            else
                update(
                    ns,
                    ns.sprintf("Will take a min of %s", ns.tFormat(graftTime)),
                );
        }
        if (queue.length > 0) {
            const price = await proxyTry(
                ns,
        ...
        while (!ready) {
            ...
            if (FOCUS)
                update(
                    ns,
                    ns.sprintf(
                        "Please remain focused or your time will increase.",
                    ),
                );
            else
                update(
            ...
            if (queue.length > 0) {
                ...
                update(
                    ns,
                    ns.sprintf("#Queued After above: %s", queue.length - 1),
                );
            }
            await ns.asleep(200);
        }
        if (performance.now() - time < 1000) {
            const augPrice = await proxyTry(
                ns,
                "grafting.getAugmentationGraftPrice",
                SPECIFIC,
            );
            if (!augPrice) update(ns, "You have already installed this.");
                ...
                update(
                    ns,
                    ns.sprintf(
                        "Need to install a pre-req for %s first",
                        SPECIFIC,
                    ),
                );
        }
