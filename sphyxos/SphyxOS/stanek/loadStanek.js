...
export async function main(ns) {
    ...
    const files = ns
        .ls("home", fileLocation)
        .map((m) => [m.substring(fileLocation.length - 1), fileLocation]);
    if (useDefaults) {
        const defaultFiles = ns
            .ls("home", defaultFileLocation)
            .map((m) => [
    ...
    const chosen = await ns.prompt("Choose your loadout", {
        type: "select",
        choices: selectable.sort((a, b) => b[0] > a[0]),
    });
    if (chosen === "") {
        ...
        ns.toast(
            "SUCCESS: Stanek loaded from " + prefix + chosen + ".txt",
            "success",
            3000,
        );
    }
