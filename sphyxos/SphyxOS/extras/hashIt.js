...
export async function main(ns) {
    ...
    switch (ns.args[0]) {
        ...
        case "corp":
            while (
                ns.hacknet.spendHashes("Sell for Corporation Funds", "home")
            ) {}
            break;
        case "min":
            ...
            while (
                ns.hacknet.spendHashes("Reduce Minimum Security", hackTarget)
            ) {
                if (ns.getServerMinSecurityLevel(hackTarget) === 1) break;
            }
            break;
        case "max":
            if (hackTarget === "NULL PORT DATA") return;
            while (
                ns.hacknet.spendHashes("Increase Maximum Money", hackTarget)
            ) {}
            break;
        case "study":
        ...
        case "coding":
            while (
                ns.hacknet.spendHashes("Generate Coding Contract", "home")
            ) {}
            break;
        case "favor":
            let favorTarget = "home";
            try {
                favorTarget = ns.singularity.getCurrentWork().companyName;
            } catch {
                return;
            }
            if (favorTarget)
                while (ns.hacknet.spendHashes("Company Favor", favorTarget)) {}
            break;
        default:
            break;
    }
