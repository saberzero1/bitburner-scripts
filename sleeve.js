...
let factionWorkCache = [],
    factionWorkCacheExpiry = 0,
    factionRepCache = {},
    factionWorkTypesCache = {},
    factionWorkTypesCacheExpiry = 0;
...
export async function main(ns) {
    ...
    if (!(10 in ownedSourceFiles))
        return ns.tprint(
            "WARNING: You cannot run sleeve.js until you do BN10.",
        );
    // Start the main loop
    while (true) {
        try {
...
async function manageSleeveAugs(ns, i, budget) {
    ...
    const cooldownLeft = Math.max(
        0,
        options["buy-cooldown"] - (Date.now() - (lastPurchaseTime[i] || 0)),
    );
...
async function getFactionWorkTargets(ns, playerInfo) {
    ...
    const ownedAugs = await getNsDataThroughFile(
        ns,
    ...
    factionWorkCache = Object.keys(factionAugMap)
        .map((name) => ({
            name,
            unownedCount: factionAugMap[name].filter(
                (aug) => !ownedAugs.includes(aug),
            ).length,
            rep: factionRepCache[name] ?? 0,
            supportedWorks: factionWorkTypesCache[name] ?? works,
            supportedWorksLower: (factionWorkTypesCache[name] ?? works).map(
                (w) => w.toLowerCase(),
            ),
        }))
        .filter(
            (entry) =>
                entry.unownedCount > 0 &&
                !(unsupportedFactionWork[entry.name] > Date.now()),
        )
        .filter((entry) =>
            entry.supportedWorksLower.some((w) => works.includes(w)),
        )
        .sort(
            (a, b) =>
                b.unownedCount - a.unownedCount ||
                a.rep - b.rep ||
...
function getPreferredFactionWorkIndex(sleeve) {
    const physicalAvg =
        ...
        4;
    if (
        sleeve.skills.hacking >= physicalAvg &&
        sleeve.skills.hacking >= sleeve.skills.charisma
    )
        return works.indexOf("hacking");
...
async function mainLoop(ns) {
    ...
    if (!options["disable-bladeburner"] && !playerInBladeburner)
        playerInBladeburner = await getNsDataThroughFile(
            ns,
            "ns.bladeburner.inBladeburner()",
        );
    const playerWorkInfo = await getCurrentWorkInfo(ns);
    if (!playerInGang)
        playerInGang = !(2 in ownedSourceFiles)
            ? false
            : await getNsDataThroughFile(ns, "ns.gang.inGang()");
    if (playerInGang && !gangFactionName) {
        try {
            gangFactionName = (
                await getNsDataThroughFile(ns, "ns.gang.getGangInformation()")
            ).faction;
        } catch {}
    }
    let globalReserve = Number(ns.read("reserve.txt") || 0);
    let budget =
        (playerInfo.money - (options["reserve"] || globalReserve)) *
        options["aug-budget"];
    ...
    const costByNextLoop =
        (interval / 1000) *
        task.filter((t) => t.startsWith("train")).length *
        2400;
    // Get time in current bitnode (to cap how long we'll train sleeves)
    const timeInBitnode =
        Date.now() -
        (await getNsDataThroughFile(ns, "ns.getResetInfo()")).lastNodeReset;
    let canTrain =
        ...
        playerInfo.money - costByNextLoop >
            (options["training-reserve"] ||
                (promptedForTrainingBudget
                    ? ns.read(trainingReserveFile)
                    : undefined) ||
                globalReserve);
    // If any sleeve is training at the gym, see if we can purchase a gym upgrade to help them
    if (
        canTrain &&
        task.some((t) => t?.startsWith("train")) &&
        !options["disable-spending-hashes-for-gym-upgrades"]
    )
        if (
            await getNsDataThroughFile(
                ns,
            ...
            log(
                ns,
                `SUCCESS: Bought "Improve Gym Training" to speed up Sleeve training.`,
                false,
                "success",
            );
    if (
        canTrain &&
        task.some((t) => t?.startsWith("study")) &&
        ...
        if (
            ...
            log(
                ns,
                `SUCCESS: Bought "Improve Studying" to speed up Sleeve studying.`,
                false,
                "success",
            );
    if (playerInBladeburner && 7 in ownedSourceFiles) {
        const bladeburnerCity = await getNsDataThroughFile(
            ns,
            `ns.bladeburner.getCity()`,
        );
        bladeburnerCityChaos = await getNsDataThroughFile(
            ns,
    ...
    for (
        ...
        if (
            sleeveInfo[i].shock == 0 &&
            (i < i || i > 3 || !playerInBladeburner)
        )
            followPlayerSleeve ??= i; // Skips assignment if previously assigned
    followPlayerSleeve ??= 0; // If all have shock, use the first sleeve

    const currentSleeveTasks = await getNsDataThroughFile(
        ns,
        `ns.args.map(i => ns.sleeve.getTask(i))`,
        `/Temp/sleeve-getTask-all.txt`,
        [...Array(numSleeves).keys()],
    );
    ...
    for (let i = 0; i < numSleeves; i++) {
        ...
        if (
            typeof designatedTask === "string" &&
            designatedTask.startsWith("work for faction '")
        ) {
            const factionMatch = designatedTask.match(
                /work for faction '([^']+)'/,
            );
        ...
        [lastSleeveHp[i], lastSleeveShock[i]] = [
            sleeve.hp.current,
            sleeve.shock,
        ];
        ...
        if (task[i] != designatedTask)
            assignSuccess = await setSleeveTask(
                ns,
                i,
                designatedTask,
                command,
        ...
        if (
            ...
            (assignSuccess === true ||
                (assignSuccess === undefined &&
                    Date.now() - (lastStatusUpdateTime[i] ?? 0) >
                        statusUpdateInterval))
        ) {
            log(
                ns,
                `INFO: Sleeve ${i} is ${assignSuccess === undefined ? "(still) " : ""}${statusUpdate} `,
            );
...
async function pickSleeveTask(
    ...
    if (canTrain) {
        ...
        let untrainedStats = trainStats.filter(
            (stat) => sleeve.skills[stat] < options[`train-to-${stat}`],
        );
        let untrainedSmarts = trainSmarts.filter(
            (smart) => sleeve.skills[smart] < options[`study-to-${smart}`],
        );
        ...
        if (untrainedStats.length > 0) {
            ...
            if (sleeve.city != ns.enums.CityName.Sector12) {
                ...
                await getNsDataThroughFile(
                    ns,
                    "ns.sleeve.travel(ns.args[0], ns.args[1])",
                    null,
                    [i, ns.enums.CityName.Sector12],
                );
            }
            ...
            if (sleeve.city != ns.enums.CityName.Volhaven) {
                log(
                    ns,
                    `Moving Sleeve ${i} from ${sleeve.city} to Volhaven so that they can study at ZB Institute.`,
                );
                await getNsDataThroughFile(
                    ns,
                    "ns.sleeve.travel(ns.args[0], ns.args[1])",
                    null,
                    [i, ns.enums.CityName.Volhaven],
                );
            }
    ...
    if (i == followPlayerSleeve && playerWorkInfo.type == "FACTION") {
        ...
        if (!assignedFactions.has(faction)) {
            if (gangFactionName && faction === gangFactionName)
                unsupportedFactionWork[faction] =
                    Date.now() + unsupportedFactionWorkCooldown;
            else if (!(unsupportedFactionWork[faction] > Date.now())) {
                const supportedWorksRaw =
                    factionWorkTypesCache[faction] ?? works;
                const supportedWorks = supportedWorksRaw.map((w) =>
                    w.toLowerCase(),
                );
                const allowedWorks = supportedWorks.filter((w) =>
                    works.includes(w),
                );
                if (allowedWorks.length == 0) {
                    ...
                    const workRaw =
                        supportedWorksRaw.find(
                            (w) => w.toLowerCase() == work,
                        ) ?? supportedWorksRaw[0];
                    return [
    ...
    if (i != followPlayerSleeve) {
        ...
        const nextFaction = factionTargets.find(
            (f) => !assignedFactions.has(f.name),
        );
        if (nextFaction) {
            ...
            if (gangFactionName && faction === gangFactionName)
                ...
                const supportedWorksLower =
                    nextFaction.supportedWorksLower ??
                    supportedWorksRaw.map((w) => w.toLowerCase());
                const allowedWorks = supportedWorksLower.filter((w) =>
                    works.includes(w),
                );
                if (allowedWorks.length == 0)
                    unsupportedFactionWork[faction] =
    ...
    if (playerInBladeburner) {
        ...
        const bbTasks = [
            ...
            /*0*/ options["enable-bladeburner-team-building"]
                ? ["Support main sleeve"]
            ...
            /*5*/ ["Diplomacy"],
            /*6*/ ["Field Analysis"],
            /*7*/ options["enable-bladeburner-team-building"]
                ? ["Recruitment"]
                : ["Infiltrate Synthoids"],
        ];
        ...
        const contractCount =
            bladeburnerContractCounts[contractName] ?? Infinity;
        ...
        if (
            sleeve.hp.current < lastSleeveHp[i] ||
            sleeve.shock > lastSleeveShock[i]
        ) {
            bladeburnerCooldown[i] =
                Date.now() + options["failed-bladeburner-contract-cooldown"];
            log(
                ns,
            ...
            !onCooldown() &&
            (contractChance <= minBbProbability ||
        ...
        // If the sleeve is on cooldown ,do not perform their designated bladeburner task
            ...
            if (sleeve.shock > 0)
                return shockRecoveryTask(
                    sleeve,
                    i,
                    `bladeburner task is on cooldown`,
                );
    ...
    var crime =
        options.crime ||
        (await calculateCrimeChance(ns, sleeve, "Homicide")) >=
            options["homicide-chance-threshold"]
            ? "Homicide"
            : "Mug";
    return await crimeTask(
        ns,
        crime,
        i,
        sleeve,
    ...
    const crimeStats =
        ...
        (cachedCrimeStats[crimeName] =
            4 in ownedSourceFiles
                ? await getNsDataThroughFile(
                      ns,
                      `ns.singularity.getCrimeStats(ns.args[0])`,
                      null,
                      [crimeName],
                  )
