...
const usage =
    "Usage: run grep.js [<filename.ext>] <search_string>\n" +
...
export async function main(ns) {
    ...
    const search =
        args.length == 1 ? args[0] : args.slice(1, args.length).join(" ");
    // Two or more arguments, treat the first argument as a file name
    if (args.length > 1) {
        const fileName = args[0];
        const contents = ns.read(fileName);
        if (!contents)
            return ns.tprint(`ERROR: File not found: "${fileName}".\n${usage}`);
    ...
    const allOutput = files.flatMap((fileName) =>
        searchRows(ns.read(fileName), search, fileName),
    );
    ns.tprint(
        allOutput.length > 0
            ? allOutput.join("\n")
...
function searchRows(text, search, fileName) {
    ...
    if (output.length > 0 && fileName)
        output.unshift(
            `Found ${output.length} occurrences of the string "${search}" in file ${fileName}:`,
        );
    return output;
}
