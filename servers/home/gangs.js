...
let allTaskStats = /**@returns{{[taskName: string]: GangTaskStats;}}*/ (() =>
    undefined)();
let assignedTasks =
    /**@returns{{[gangMemberName: string]: string;}}*/ (() => ({}))(); // Each member will independently attempt to scale up the crime they perform until they are ineffective or we start generating wanted levels
...
let equipments =
    /**@returns{{name: string;type: string;cost: number;stats: EquipmentStats;}[]};*/ (() => [])();
let importantStats = [];
...
async function initialize(ns) {
    ...
    while (!haveJoinedAGang) {
        try {
            haveJoinedAGang = await getNsDataThroughFile(
                ns,
                "ns.gang.inGang()",
            );
    ...
    const myGangInfo = await getNsDataThroughFile(
        ns,
    ...
    if (sf4Level == 0)
        ...
        try {
            ...
            log(
                ns,
                `Highest augmentation reputation cost is ${formatNumberShort(requiredRep)}`,
            );
        } catch {
            log(
                ns,
    ...
    const dictEquipmentTypes =
        await /**@returns{Promise<{[gangMember: string]: string;}>}*/ (() =>
            getGangInfoDict(ns, equipmentNames, "getEquipmentType"))();
    const dictEquipmentCosts =
        await /**@returns{Promise<{[gangMember: string]: number;}>}*/ (() =>
            getGangInfoDict(ns, equipmentNames, "getEquipmentCost"))();
    const dictEquipmentStats =
        await /**@returns{Promise<{[gangMember: string]: EquipmentStats;}>}*/ (() =>
    ...
    const dictMembers =
        await /**@returns{Promise<{[gangMember: string]: GangMemberInfo;}>}*/ (() =>
...
async function mainLoop(ns) {
    ...
    if (!territoryTickDetected) {
        ...
        if (
            ...
            JSON.stringify(otherGangInfo) != JSON.stringify(lastOtherGangInfo)
        ) {
    ...
    if (
        !warfareFinished &&
        !isReadyForNextTerritoryTick &&
        thisLoopStart + updateInterval + territoryTickWaitPadding >=
            territoryNextTick
    ) {
    ...
    if (
        (isReadyForNextTerritoryTick &&
            myGangInfo.power != lastTerritoryPower) ||
        thisLoopStart >
            territoryNextTick +
                5000 /* Wait up to 5 additional seconds in case time was wonkey */
    ) {
...
async function onTerritoryTick(ns, myGangInfo) {
    territoryNextTick =
        lastLoopTime + territoryTickTime / (ns.gang.getBonusTime() > 0 ? 5 : 1); // Reset the time the next tick will occur
    if (lastTerritoryPower != myGangInfo.power || lastTerritoryPower == null) {
        ...
        if (
            consecutiveTerritoryDetections > 5 &&
            territoryTickWaitPadding > updateInterval
        )
            territoryTickWaitPadding = Math.max(
                updateInterval,
                territoryTickWaitPadding - updateInterval,
            );
        ...
        territoryTickWaitPadding = Math.min(
            2000,
            territoryTickWaitPadding + updateInterval,
        ); // Start waiting earlier to account for observed lag.
    ...
    myGangMembers = await getNsDataThroughFile(ns, "ns.gang.getMemberNames()");
    const canRecruit = await getNsDataThroughFile(
        ns,
...
async function updateMemberActivities(
    ...
    const workOrders = [];
    const maxMemberDefense = Math.max(
        ...Object.values(dictMembers).map((m) => m.def),
    );
    for (const member of Object.values(dictMembers)) {
        ...
        if (
            ...
            (member.def < 100 ||
                member.def < Math.min(10000, maxMemberDefense * 0.1))
        )
...
async function optimizeGangCrime(ns, myGangInfo) {
    ...
    const memberTaskRates = Object.fromEntries(
        Object.values(dictMembers).map((m) => [
            ...
            allTaskNames
                ...
                .filter(
                    (task) =>
                        task.wanted <= 0 || task.money > 0 || task.respect > 0,
                ),
        ]),
    ...
    for (let shuffle = 0; shuffle < 100; shuffle++) {
        ...
        shuffleArray(myGangMembers.slice()).forEach((member, index) => {
            ...
            const sustainableTasks =
                index < myGangMembers.length - 2
                    ? taskRates
                    : taskRates.filter(
                          (c) => totalWanted + c.wanted <= wantedGainTolerance,
                      );
        ...
        while (
            totalWanted > wantedGainTolerance &&
            Object.values(proposedTasks).some(
                (t) => t.name !== strWantedReduction,
            )
        ) {
            const mostWanted = Object.keys(proposedTasks).reduce(
                (t, c) =>
                    ...
                    (t == null ||
                        proposedTasks[t].wanted < proposedTasks[c].wanted)
                        ? c
                        : t,
                null,
            );
            const nextBestTask =
                ...
                memberTaskRates[mostWanted].find(
                    (t) => t.name === strWantedReduction,
                );
            [proposedTasks[mostWanted], totalWanted, totalGain] = [
                ...
                totalWanted +
                    nextBestTask.wanted -
                    proposedTasks[mostWanted].wanted,
                totalGain +
                    nextBestTask[optStat] -
                    proposedTasks[mostWanted][optStat],
            ];
    ...
    if (
        ...
        myGangMembers.some(
            (m) => assignedTasks[m] !== bestTaskAssignments[m].name,
        )
    ) {
        myGangMembers.forEach(
            (m) => (assignedTasks[m] = bestTaskAssignments[m].name),
        ...
        const [optWanted, optRespect, optMoney] = myGangMembers
            ...
            .reduce(
                ([w, r, m], t) => [w + t.wanted, r + t.respect, m + t.money],
                [0, 0, 0],
            );
        if (
        ...
        if (
            Math.abs(myGangInfo.wantedLevelGainRate - optWanted) / optWanted >
                0.01 ||
            Math.abs(myGangInfo.respectGainRate - optRespect) / optRespect >
                0.01 ||
...
async function fixWantedGainRate(ns, myGangInfo, wantedGainTolerance = 0) {
    ...
    log(
        ns,
        `WARNING: Generating wanted levels (${lastWantedLevelGainRate.toPrecision(3)}/sec > ${wantedGainTolerance.toPrecision(3)}/sec), reassigning members to ${strWantedReduction}...`,
        false,
        "warning",
    );
    const dictMembers = await getGangInfoDict(
    ...
    const candidateMembers = Object.values(dictMembers)
        .filter(
            (member) =>
                crimes.includes(assignedTasks[member.name]) &&
                assignedTasks[member.name] !== strWantedReduction,
        )
        .map((member) => {
            const currentTask = assignedTasks[member.name];
            const currentWanted = computeWantedGains(
                myGangInfo,
                currentTask,
                member,
            );
            const reductionWanted = computeWantedGains(
                myGangInfo,
                strWantedReduction,
                member,
            );
            const currentRep = computeRepGains(myGangInfo, currentTask, member);
            const reductionRep = computeRepGains(
                myGangInfo,
                strWantedReduction,
                member,
            );
            ...
            const score =
                repLoss <= 0
                    ...
                    : wantedReduction / repLoss;
            return { name: member.name, wantedReduction, repLoss, score };
        })
        .filter((entry) => entry.wantedReduction > 0);
    candidateMembers.sort(
        (a, b) => b.score - a.score || b.wantedReduction - a.wantedReduction,
    );
    for (const candidate of candidateMembers) {
        ...
        if (lastWantedLevelGainRate == wantedLevelGainRate)
            log(
                ns,
                `Warning: Attempt to rollback crime of ${candidate.name} to ${assignedTasks[candidate.name]} resulted in no change in wanted level gain rate ` +
...
async function tryAscendMembers(ns) {
    ...
    for (let i = 0; i < myGangMembers.length; i++) {
        ...
        const ascMultiThreshold =
            options["ascend-multi-threshold"] +
            (11 - i) * options["ascend-multi-threshold-spacing"];
        ...
        if (
            ...
            (await getNsDataThroughFile(
                ns,
                `ns.gang.ascendMember(ns.args[0])`,
                null,
                [member],
            ))
        ) {
...
async function tryUpgradeMembers(ns, dictMembers) {
    ...
    let budget =
        Math.min(
            maxBudget,
            options["equipment-budget"] ||
                defaultMaxSpendPerTickTransientEquipment,
        ) * homeMoney;
    let augBudget =
        Math.min(
            maxBudget,
            options["augmentations-budget"] ||
                defaultMaxSpendPerTickPermanentEquipment,
        ) * homeMoney;
    if (!is4sBought)
        is4sBought = await getNsDataThroughFile(
            ns,
    ...
    for (const equip of equipments) {
        ...
        for (const member of Object.values(dictMembers)) {
            ...
            if (equip.type != "Augmentation" && percievedCost > budget)
                continue;
            if (
...
async function waitForGameUpdate(ns, oldGangInfo) {
    if (
        !myGangMembers.some(
            (member) => !assignedTasks[member].includes("Train"),
        )
    )
    ...
    while (Date.now() < start + maxWaitTime) {
        var latestGangInfo = await getNsDataThroughFile(
            ns,
            "ns.gang.getGangInformation()",
        );
        ...
        await ns.sleep(
            Math.min(waitInterval, start + maxWaitTime - Date.now()),
        );
    }
    sequentialMisfires++;
    log(
...
async function enableOrDisableWarfare(ns, myGangInfo) {
    ...
    if (shouldEngage != myGangInfo.territoryWarfareEngaged) {
        ...
        await runCommand(ns, `ns.gang.setTerritoryWarfare(ns.args[0])`, null, [
            shouldEngage,
        ]);
    }
}
...
const getGangInfoDict =
    /**@returns{Promise<{[gangMember: string]: any;}>}*/ async (
        ns,
        elements,
        gangFunction,
    ) =>
...
const getDict = /**@returns{Promise<{[key: string]: any;}>}*/ async (
    ns,
    elements,
    nsFunction,
    fileName,
