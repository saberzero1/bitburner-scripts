...
const factionsWithoutDonation = [
    "Bladeburners",
    "Church of the Machine God",
    "Shadows of Anarchy",
]; // Not allowed to donate to these factions for rep
...
let playerData = /**@returns{Player}*/ (() => null)(),
    bitNode = 0,
    gangFaction = "";
let numAugsAwaitingInstall = 0,
    nfLevelPurchased = 0,
...
let ownedAugmentations = [""],
    ...
    allAugStats = [""],
    priorityAugs = [""];
let effectiveSourceFiles =
    /**@returns {{[bitNode: number]: number}}*/ (() => ({}))();
let factionData =
    /**@returns {{[factionName: string]: FactionData}}*/ (() => ({}))();
let augmentationData =
    /**@returns {{[augmentationName: string]: AugmentationData}}*/ (() => ({}))();
...
const statPlayer = [
    ...
    "agility",
    "charisma",
]; // Since these are substrings of other stats, we can specifically request this stat with e.g. "hacking_level"
const allFactions = [
    "Illuminati",
    "Daedalus",
    "The Covenant",
...
export function autocomplete(data, args) {
    ...
    if (
        lastFlag == "--sort" ||
        lastFlag == "--stat-desired" ||
        lastFlag == "--hide-stat"
    )
        return statShortcuts
            .concat(stat_multis)
            .concat(statPlayer.map((s) => `${s}_level`));
    ...
    if (
        lastFlag == "--omit-aug" ||
        lastFlag == "--aug-desired" ||
        lastFlag == "--priority-aug"
    )
...
export async function main(ns) {
    ...
    printToTerminal =
        (options.v || options.verbose === true || options.verbose === null) &&
        !options["join-only"];
    ...
    if (sf4Level == 0)
        return log(
            ns,
            `ERROR: This script requires SF4 (singularity) functions to work.`,
            true,
            "error",
        );
    else if (sf4Level < 3)
    ...
    numAugsAwaitingInstall =
        ownedAugmentations.length - installedAugmentations.length;
    if (options["neuroflux-disabled"]) omitAugs.push(strNF);
    simulatedOwnedAugmentations = ignorePlayerData
        ? []
        : ownedAugmentations.filter((a) => a != strNF);
    // Clear "priority" / "desired" lists of any augs we already own
    priorityAugs = priorityAugs.filter(
        (name) => !simulatedOwnedAugmentations.includes(name),
    );
    desiredAugs = desiredAugs.filter(
        (name) => !simulatedOwnedAugmentations.includes(name),
    );
    ...
    if ((desiredStatsFilters?.length ?? 0) == 0) {
        ...
        const willTakeAnyAug =
            ownedAugmentations.length > 40 || // Once we have more than N augs, switch to buying up anything and everything
            bitNode == 6 ||
            bitNode == 7 ||
            playerData.factions.includes("Bladeburners") || // If doing bladeburners, combat augs matter too, so just get everything
        ...
        desiredStatsFilters = willTakeAnyAug
            ? ["*"] // Take any aug if one of the above criteria is met
            : bitNode == 8
              ? ["hacking_level", "hacking_exp"] // In BN8, we only want to install if we will be boosting our hack level (to unlock Daedalus). We don't need rep as much, since it can be purchased immediately.
              : [
                    "hacking",
                    "faction_rep",
                    "company_rep",
                    ...
                    "hacknet",
                    "crime_money",
                ]; // Otherwise get hacking + rep boosting, etc. for unlocking augs more quickly
    }
    ...
    if (ignorePlayerData)
        log(
            ns,
            "INFO: Skipping joining available factions due to the --ignore-player-data flag set.",
        );
    else {
    ...
    if (
        ...
        // Don't do this next part if we were "mocking" the player for this run
        // Write a file that summarizes what augs we could afford if we could ascend right now. (used by autopilot.js)
        const augsAwaitingInstall = ownedAugmentations.slice(
            installedAugmentations.length,
        ); // Assumes augs are returned in purchased order
        // Infer the number of nf we have installed based on the current nf purchase level, minus the pending nf installs
        const nfInstalled =
            nfLevelPurchased -
            augsAwaitingInstall.filter((a) => a == strNF).length;
        ns.write(
            output_file,
            JSON.stringify(
                {
                    ...
                    installed_count_ex_nf: installedAugmentations.filter(
                        (a) => a != strNF,
                    ).length, // Count of non-NeuroFlux augs installed
                    ...
                    purchased_count_ex_nf: ownedAugmentations.filter(
                        (a) => a != strNF,
                    ).length, // Count augmentations we have purchased (some of which may not be installed)
                    ...
                    awaiting_install_count_nf: augsAwaitingInstall.filter(
                        (a) => a == strNF,
                    ).length, // Count of NeuroFlux levels awaiting installation
                    awaiting_install_count_ex_nf: augsAwaitingInstall.filter(
                        (a) => a != strNF,
                    ).length, // Count of non-NeuroFlux awaiting installation
                    ...
                    affordable_count_nf: purchaseableAugs.filter(
                        (a) => a.name == strNF,
                    ).length, // Count of NeuroFlux levels we can currently afford to buy
                    affordable_count_ex_nf: purchaseableAugs.filter(
                        (a) => a.name != strNF,
                    ).length, // Count of unique augs we can currently afford to buy, ignoring NeuroFlux
                    total_rep_cost: Object.values(
                        purchaseFactionDonations,
                    ).reduce((t, r) => t + r, 0), // Total money needed to buy the reputation needed for all affordable augs
                    total_aug_cost: getTotalCost(purchaseableAugs), // Total money needed to buy all affordable augs (excludes the rep cost above)
                    // Unpurchased augs
                    unpurchased_count: Object.values(augmentationData).filter(
                        (a) => !a.owned,
                    ).length, // Number of augs are we have not yet purchased (note: depending on config, may not include all augs in the game)
                },
                undefined,
                2,
            ),
...
function unshorten(strMult) {
    ...
    if (stat_multis.includes(strMult.replace("_mult", "")))
        return strMult.replace("_mult", ""); // _mult suffix no longer appears
    if (stat_multis.includes(strMult.replace("_level", "")))
        return strMult.replace("_level", ""); // Users can explicitly request just the base mult (and not all mults that include it) by specifying the _level suffix
...
async function updateFactionData(ns, factionsToOmit) {
    ...
    factionNames.push(
        ...(options["hide-locked-factions"]
            ? easyAccessFactions
            : allFactions
        ).filter((f) => !factionNames.includes(f)),
    );
    // Unless "all factions" is requested, omit factions that are in no way accessible on this reset
    if (!(options.a || options.all)) {
        if (!(13 in effectiveSourceFiles))
            factionsToOmit.push("Church of the Machine God");
        if (!(6 in effectiveSourceFiles || 7 in effectiveSourceFiles))
            factionsToOmit.push("Bladeburners");
    }
    // Finally, remove all factions marked as omitted
    log(
        ns,
        `We "know" about ${factionNames.length} factions, and will omit ${factionsToOmit.length} of them.`,
    );
    ...
    const dictFactionAugs =
        /**@returns {{[factionName: string]: string[]}}*/ (() => null)() ??
        (await getSingularityDict(
            ns,
            "getAugmentationsFromFaction",
            factionNames,
        ));
    const dictFactionReps =
        /**@returns {{[factionName: string]: number}}*/ (() => null)() ??
        (await getSingularityDict(ns, "getFactionRep", factionNames));
    const dictFactionFavors =
        /**@returns {{[factionName: string]: number}}*/ (() => null)() ??
    ...
    factionData = Object.fromEntries(
        factionNames.map((faction) => [
            ...
            new FactionData(
                faction,
                invitations.includes(faction),
                joinedFactions.includes(faction),
                dictFactionReps[faction],
                dictFactionFavors[faction],
                dictFactionAugs[faction],
            ),
        ]),
    );
}
...
async function updateAugmentationData(ns) {
    ...
    const dictAugRepReqs =
        /**@returns {{[augmentationName: string]: number}}*/ (() => null)() ??
        (await getSingularityDict(
            ns,
    ...
    const dictAugStats =
        /**@returns {{[augmentationName: string]: Multipliers}}*/ (() =>
            null)() ??
        (await getSingularityDict(
            ns,
    ...
    const dictAugPrereqs =
        /**@returns {{[augmentationName: string]: string[]}}*/ (() => null)() ??
        (await getSingularityDict(
            ns,
...
function isStatDesired(stat_name) {
    return (
        ...
        desiredStatsFilters.includes("_") || // Wildcards - if all stats are desired, always return true (_ is for backwards compatibility when all stat names ended with '_mult')
        desiredStatsFilters.some(
            (filter) =>
                stat_name.includes(filter) || // A stat is desired if any "desired stat" string appears anywhere in the stat name
                stat_name == filter.replace("_level", ""),
        )
    ); // Users can explicitly request just the base mult (and not all mults that include it as a substring) by specifying the _level suffix
}
...
class AugmentationData {
    ...
    constructor(
        ...
        this.stats = Object.fromEntries(
            Object.entries(augmentationStats).filter(([k, v]) => v != 1),
        );
        ...
        this.getFromAny =
            factionNames
                ...
                .filter((f) => f.augmentations.includes(aug))[0]?.name ??
            "(unknown)";
    }
    ...
    getFromJoined() {
        ...
        if (this.name != strNF)
            return (
                augFactions.filter((f) => f.reputation >= this.reputation)[0] || // Any faction we can buy it from
                augFactions
                    ...
                    )[0] || // Cheapest donation for aug
                augFactions.sort((a, b) => b.reputation - a.reputation)[0] || // Faction we are closest to being able to get it from (most rep)
                augFactions[0]
            )?.name; // First faction in our faction list order (which should be ordered by priority)

        // The "Neuroflux" augmentation uses a different approach.
        const repReady = augFactions.filter(
            (f) => f.reputation >= this.reputation,
        );
        if (repReady.length > 0)
            return repReady.sort((a, b) => b.reputation - a.reputation)[0]
                ?.name;
        const donationReady = augFactions.filter((f) => f.donationsUnlocked);
        return (donationReady.length > 0 ? donationReady : augFactions).sort(
    ...
    toString() {
            ...
            (statKeys.length == 0
                ...
                : ` { ` +
                  ...
                  statKeys
                      .sort(
                          (a, b) =>
                              isStatDesired(b) - isStatDesired(a) ||
                              this.stats[b] - this.stats[a],
                      )
                      .map(
                          (prop) =>
                              shorten(prop) +
                              ": " +
                              Math.round(
                                  (this.stats[prop] + Number.EPSILON) * 100,
                              ) /
                                  100,
                      )
                      .join(", ") +
        ...
        return (
            `${this.desired ? "*" : " "} Price: ${formatMoney(this.price, 4).padEnd(7)} ${this.price <= budget ? "✓" : "✗"}  ` +
...
async function joinFactions(ns, forceJoinFactions) {
    ...
    for (const faction of invitations.sort(factionSortOrder)) {
        ...
        let newAugs = unownedAugs.filter(
            (aug) => !accessibleAugmentations.has(aug),
        ); //  Filter out augmentations we can purchase from another faction we've already joined
        let desiredAugs = newAugs.filter(
            (aug) => augmentationData[aug].desired,
        ); //  Filter out augmentations we have no interest in
        log(
            ns,
            `${faction.name} has ${faction.augmentations.length} augs, ${unownedAugs.length} unpurchased, ${newAugs.length} not offered by joined factions, ` +
        ...
        if (
            desiredAugs.length == 0 &&
            !forceJoinFactions.includes(faction.name)
        )
            continue;
        if (
            ...
            if (
                ...
                faction.joined = true;
                faction.augmentations.forEach((aug) =>
                    accessibleAugmentations.add(aug),
                );
                joinedFactions.push(faction.name);
                log(
                    ns,
                    `SUCCESS: Joined faction ${faction.name} (Response: ${response})`,
                    true,
                    "success",
                );
                joined++;
                ...
                log(
                    ns,
                    `ERROR: Error joining faction ${faction.name}. Response: ${response}`,
                    false,
                    "error",
                );
        }
 ...
 * @param {number} rep */
let getCostOfReputation = (rep) =>
    Math.ceil(
        (1e6 * rep) /
            playerData.mults.faction_rep /
            bitNodeMults.FactionWorkRepGain,
    );
...
let getReqDonationForRep = (rep_needed, factionOrFactionName) =>
    getCostOfReputation(
        Math.max(
            0,
            rep_needed -
                (factionOrFactionName.name
                    ? factionOrFactionName
                    : factionData[factionOrFactionName]
                ).reputation,
        ),
    );
...
let getReqDonationForAug = (aug, factionOrFactionName) =>
    getReqDonationForRep(
        aug.reputation,
        factionOrFactionName || aug.getFromJoined(),
    );
...
function sortAugs(ns, augs = []) {
    ...
    for (let i = 0; i < augs.length; i++) {
        for (let j = 0; j < augs[i].prereqs.length; j++) {
            const prereqIndex = augs.findIndex(
                (a) => a.name == augs[i].prereqs[j],
            );
            if (
    ...
    for (let i = augs.length - 1; i > 0; i--) {
        ...
        while (
            i - batchLengh >= 0 &&
            augs[i].price > augs[i - batchLengh].price &&
            isRelated(augs[i], augs[i - batchLengh])
        )
            batchLengh++;
        ...
        while (i + moved + 1 < augs.length) {
            ...
            testOrder.splice(
                insertionIndex,
                0,
                testOrder.splice(moveIndex, 1)[0],
            ); // Try moving it above the batch
    ...
    if (totalMoves > 0)
        log(
            ns,
            `Cost reduced by ${formatMoney(initialCost - finalCost)} (from ${formatMoney(initialCost)} to ${formatMoney(finalCost)}) by bubbling ${totalMoves} augs up above batches of dependencies.`,
        );
    return augs;
}
...
async function manageUnownedAugmentations(ns, ignoredAugs) {
    ...
    const unownedAugs = Object.values(augmentationData).filter(
        (aug) =>
            (!aug.owned || aug.name == strNF) &&
            !ignoredAugs.includes(aug.name),
    );
    ...
    let unavailableAugs = unownedAugs.filter(
    ...
    if (
        ...
        (unavailableAugs.length > 0 &&
            options["show-unavailable-aug-purchase-order"])
    )
    ...
    if (countAvailable > 0) {
        ...
        let accessibleAugs = await manageFilteredSubset(
            ns,
            outputRows,
            "Desired Within Rep",
    ...
    if (purchaseableAugs.length > 0)
        log(
            ...
            `INFO: The above ${purchaseableAugs.length} augmentations ${options.purchase ? "will" : "can"} be purchased ` +
                ...
                (options.purchase
                    ? ""
                    : " Run with the --purchase flag to make the purchase."),
            printToTerminal,
        );
}
...
function computeCosts(sortedAugs) {
    ...
    const totalRepCost = Object.values(repCostByFaction).reduce(
        (t, r) => t + r,
        0,
    );
...
function estimateAffordableNfLevels(
    augNf,
    factionName,
    budget,
    totalAugCost,
    ...
    while (levels < 200) {
        ...
        const nextNfCost =
            augNf.price *
            nfCountMult ** nfIndex *
            augCountMult ** (baseAugCount + levels);
        const nextNfRep = augNf.reputation * nfCountMult ** nfIndex;
        const totalDonation =
            nextNfRep <= faction.reputation
                ? 0
                : getReqDonationForRep(nextNfRep, faction);
        const nextRepCost = Math.max(0, totalDonation - currentDonation);
        const totalCost =
            localAugCost + localRepCost + nextNfCost + nextRepCost;
...
function selectBestNfFaction(
    ...
    const candidates = Object.values(factionData).filter(
        (f) => f.joined && f.augmentations.includes(augNf.name),
    );
    let best = null;
    let bestLevels = -1;
    for (const faction of candidates) {
        const levels = estimateAffordableNfLevels(
            augNf,
            faction.name,
            budget,
            totalAugCost,
        ...
        if (
            ...
            (levels === bestLevels &&
                best &&
                faction.reputation > best.reputation)
        ) {
            best = faction;
            bestLevels = levels;
        }
...
function getCostString(augCost, repCost) {
    return (
        ...
        (repCost == 0
            ? ""
            : ` (Augs: ${formatMoney(augCost, 4)} + Rep: ${formatMoney(repCost, 4)})`)
    );
}
    ...
    do {
        ...
        for (const aug of subset) {
            const missingPreqs = aug.prereqs.filter(
                (prereq) =>
                    !(
                        simulatedOwnedAugmentations.includes(prereq) ||
...
async function manageFilteredSubset(
    ...
    let [repCostByFaction, totalRepCost, totalAugCost] =
        computeCosts(subsetSorted);
    // By default, if the purchase order is unchanged after filtering out augmentations, don't bother reprinting the full list
    if (
        printList === true ||
        (printList !== false &&
            options["show-all-purchase-lists"] &&
            !subset.every((v, i) => v == subsetSorted[i]))
    )
        outputRows.push(
        ...
        `Total Cost of ${subset.length} ${subsetName}:`.padEnd(37) +
            ...
            (totalRepCost == 0
                ...
                : `  Donate: {${Object.keys(repCostByFaction)
                      .map(
                          (f) =>
                              `"${f}":${formatNumberShort(repCostByFaction[f], 4)}`,
                      )
                      .join(", ")}}`),
    );
    return subsetSorted;
...
async function managePurchaseableAugs(ns, outputRows, accessibleAugs) {
    ...
    const inaccessiblePriorityAugs = priorityAugs.filter((name) => {
        ...
        return (
            !aug ||
            !accessibleAugs.includes(aug) ||
            aug.price + getReqDonationForAug(aug) > budget
        );
    });
    ...
    do {
        ...
        purchaseableAugs = filterMissingPrereqs(
            ...
            accessibleAugs
                ...
                .filter(
                    (a) =>
                        !droppedPriorityAugs.includes(a.name) &&
                        a.price + getReqDonationForAug(a) <= budget,
                ),
        );
        [purchaseFactionDonations, totalRepCost, totalAugCost] =
            computeCosts(purchaseableAugs);
        // Remove the most expensive augmentation until we can afford all that remain
        while (
            totalAugCost + totalRepCost > budget &&
            purchaseableAugs.length > 0
        ) {
            ...
            if (!mostExpensiveAug) {
                ...
                const aPa = priorityAugs.filter((name) =>
                    purchaseableAugs.some((a) => a.name == name),
                );
            ...
            log(
                ns,
                `Dropping aug from the purchase order: \"${mostExpensiveAug.name}\". New total cost: ${costAfter}`,
            );
        }
    } while (restart);
    ...
    if (numAugsAwaitingInstall > 0)
        outputRows.push(
            `WARNING: Prices all have a x ${formatNumberShort(augCountMult ** numAugsAwaitingInstall)} cost penalty, because ` +
    ...
    if (inaccessiblePriorityAugs.length > 0)
        outputRows.push(
            `INFO: ${inaccessiblePriorityAugs.length} 'priority' augs are not yet accessible: ${inaccessiblePriorityAugs.map((n) => `"${n}"`).join(", ")}`,
        );
    const additionalDroppedPri = droppedPriorityAugs.filter(
        (n) => !inaccessiblePriorityAugs.includes(n),
    );
    if (additionalDroppedPri.length > 0)
        outputRows.push(
            `INFO: ${additionalDroppedPri.length} 'priority' augs had to be droped: ${additionalDroppedPri.map((n) => `"${n}"`).join(", ")}`,
        );
    ...
    nfLevelPurchased = Math.round(
        Math.log(
            augNf.price /
                (augCountMult ** numAugsAwaitingInstall *
                    750000 *
                    bitNodeMults.AugmentationMoneyCost),
        ) / Math.log(1.14),
    );
    let nextNfLevel = nfLevelPurchased + 1;
    const baseAugCount = purchaseableAugs.length;
    let nfPurchased = purchaseableAugs.filter(
        (a) => a.name === augNf.name,
    ).length;
    let getFrom = augNf.getFromJoined();
    const bestNfFaction = selectBestNfFaction(
        augNf,
        budget,
        totalAugCost,
        totalRepCost,
    ...
    if (!augNf.canAfford() && !augNf.canAffordWithDonation()) {
        outputRows.push(
            `Cannot purchase any ${strNF}. The next level (${nextNfLevel}) requires ${formatNumberShort(augNf.reputation)} reputation, but ` +
                (!getFrom
                    ? `it isn't being offered by any of our factions`
                    : `the best faction (${getFrom}) has insufficient rep (${formatNumberShort(factionData[getFrom].reputation)}).`),
        );
        // Prefer factions that support donating for reputation, otherwise grinding rep takes a long time.
        const factionSort = (a, b) =>
            (b.donationsUnlocked ? 1 : 0) - (a.donationsUnlocked ? 1 : 0) ||
            b.favor - a.favor;
        ...
        if (
            ...
            !getFrom ||
            (factionData[getFrom].favor < factionWithMostFavor.favor &&
            ...
            if (!joinedFactions.includes(factionWithMostFavor.name)) {
                ...
                if (invitedFactionsWithDonation.length > 0) {
                    outputRows.push(
                        `Failed to join ${factionWithMostFavor.name}. Attempting to join any factions with whom we have enough favour to donate: ${invitedFactionsWithDonation.join(", ")}.`,
                    );
                    joined = await joinFactions(
                        ns,
                        invitedFactionsWithDonation,
                    );
                } else
                    outputRows.push(
                        `Failed to join ${factionWithMostFavor.name}. NeuroFlux will not be accessible.`,
                    );
            }
            ...
            if (!getFrom)
                return log(
                    ns,
                    "Cannot buy any NF due to no joined or joinable factions offering it.",
                );
        }
        if (!augNf.canAfford() && !augNf.canAffordWithDonation())
            log(
            ...
            outputRows.push(
                `SUCCESS: Joined ${joined} factions just to gain access to additional NeuroFlux levels.`,
            );
    }
    if (
        getFrom &&
        !factionData[getFrom].donationsUnlocked &&
        factionData[getFrom].favor >= favorToDonate
    )
        outputRows.push(
    ...
    for (
        let i = purchaseableAugs.length - 1;
        i >= 0 && purchaseableAugs[i].price < augNf.price;
        i--
    )
        nfAppendPosition--;
    ...
    if (augNfFaction && (augNf.canAfford() || augNf.canAffordWithDonation()))
        log(
            ns,
            `Getting NF from faction ${augNfFaction.name} (rep: ${formatNumberShort(augNfFaction.reputation)}). Price of next NF (Level ${nextNfLevel}) is ` +
    ...
    while (augNfFaction && nfPurchased < 200) {
        ...
        const candidateFaction = selectBestNfFaction(
            augNf,
            budget,
            totalAugCost,
            totalRepCost,
        ...
        const nextNfRepCost = Math.max(
            0,
            nextNfTotalRepDonation - currentNfFactionDonation,
        ); // Compute the incremental cost of donating for rep
        const totalCostWithNextNf =
            totalAugCost + nextNfCost + totalRepCost + nextNfRepCost;
        log(
            ns,
            `Adding ${nfPurchased + 1} NF (Level ${nextNfLevel}) Requires ${formatNumberShort(nextNfRep, 4)} reputation, ` +
        ...
        if (
            totalCostWithNextNf > budget ||
            (nextNfRep > augNfFaction.reputation &&
                !augNfFaction.donationsUnlocked)
        ) {
            nextUpNf =
                `Next NF (L${nextNfLevel}) will be available at:`.padEnd(37) +
        ...
        const nfClone = new AugmentationData(
            augNf.name,
            nextNfRep,
            nextNfPrice,
            augNf.stats,
        ...
        purchaseableAugs.splice(
            purchaseableAugs.length + nfAppendPosition,
            0,
            nfClone,
        );
        totalAugCost += nextNfCost;
        const newDonationForRep = Math.max(
            currentNfFactionDonation,
            nextNfTotalRepDonation,
        );
        if (newDonationForRep > 0)
            purchaseFactionDonations[augNfFaction.name] = newDonationForRep;
    ...
    log(
        ns,
        `With ${formatMoney(budget)}, can afford to purchase ${nfPurchased} level${nfPurchased == 1 ? "" : "s"} of ${strNF}.` +
...
function computeAugsRepReqDonationByFaction(augmentations) {
    const repCostByFaction =
        /**@returns {{[factionName: string]: number}}*/ (() => ({}))();
    for (const aug of augmentations) {
        if (!(aug instanceof AugmentationData))
            throw new Error(
                "Wrong type: " + aug.constructor.name + JSON.stringify(aug),
            );
...
async function purchaseDesiredAugs(ns) {
    ...
    if (stockValue > 0)
        return log(
            ns,
            `ERROR: For your own protection, --purchase will not run while you are holding stocks (current stock value: ${formatMoney(stockValue)}). ` +
    ...
    if (
        ...
        totalAugCost + totalRepCost > playerData.money * 1.1
    )
    ...
    if (
        ...
        if (
            ...
            log(
                ns,
                `SUCCESS: Donated to ${Object.keys(purchaseFactionDonations).length} factions to gain access to desired augmentations.`,
                printToTerminal,
                "success",
            );
        else
    ...
    if (purchased == purchaseableAugs.length)
        log(
            ns,
            `SUCCESS: Purchased ${purchased} desired augmentations in optimal order!`,
            printToTerminal,
            "success",
        );
    else
        log(
            ...
            `ERROR: We were only able to purchase ${purchased} of our ${purchaseableAugs.length} augmentations. ` +
                ...
                `is now ${formatMoney((await getPlayerInfo(ns)).money)}`,
            printToTerminal,
            "error",
        );
}
...
function displayJoinedFactionSummary(ns) {
    ...
    let noaugs = joinedFactions.filter(
...
function displayFactionSummary(
    ...
    for (const faction of overrideFinishedFactions)
        simulatedOwnedAugmentations.push(
            ...(factionData[faction]?.unownedAugmentations() || []),
        );
            ...
            undefined !==
                summaryFactions.find((f) =>
                    f
                        ...
                        .find(
                            (aug) => 1 != (augmentationData[aug].stats[s] || 1),
                        ),
                ),
    );
    ...
    let getFactionSummary = (faction) => {
        ...
        return (
            `\n ${faction.joined ? "✓" : faction.invited ? "✉" : "✗"} ${faction.name} `.padEnd(
                32,
            ) +
    ...
    const unownedAugCount =
        Object.values(augmentationData).length -
        simulatedOwnedAugmentations.length;
    if (!unique)
