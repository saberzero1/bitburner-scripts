...
export function autocomplete() {
    return crimes.concat(argFastCrimesOnly);
}
...
export async function main(ns) {
    ...
    tail(ns);
    if (!crime || ns.args.includes(argFastCrimesOnly))
        ...
        await crimeForKillsKarmaStats(
            ...
            ns.args.includes(argFastCrimesOnly),
        );
...
async function legacyAutoCrime(ns, crime = "Mug") {
    ...
    while (true) {
        ...
        while (
            (await getNsDataThroughFile(ns, `ns.singularity.isBusy()`)) &&
            maxBusyLoops-- > 0
        ) {
        ...
        if (maxBusyLoops <= 0) {
            ns.tprint(
                "User have been busy for too long. auto-crime.js exiting...",
            );
            return;
        }
        ...
        let wait =
            ...
            (await getNsDataThroughFile(
                ns,
                "ns.singularity.commitCrime(ns.args[0])",
                null,
                [crime],
            ));
        ns.print(
            `Karma: ${formatNumberShort(ns.heart.break())} Committing crime \"${crime}\" and sleeping for ${formatDuration(wait)}...`,
        );
        await ns.sleep(wait);
    }
}
