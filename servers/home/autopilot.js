...
export async function main(ns) {
    ...
    const defaultBnOrder = [
        // The order in which we intend to play bitnodes
        ...
        3.3, // Hard.   Corporations. Now automated with corporation.js. Provides massive income potential and corp API access.
    ...
    let playerInGang = false,
        rushGang = false; // Tells us whether we're should be trying to work towards getting into a gang
    ...
    let alreadyJoinedDaedalus = false,
        autoJoinDaedalusUnavailable = false,
        reservingMoneyForDaedalus = false,
        disableStockmasterForDaedalus = false; // Flags to indicate that we should be keeping 100b cash on hand to earn an invite to Daedalus
    let prioritizeHackForDaedalus = false,
        prioritizeHackForWd = false;
    ...
    let dictOwnedSourceFiles =
        /**@returns{{[k: number]: number;}}*/ (() => [])(); // Player owned source files
    let dictServerHackReqs =
        /**@returns{{[serverName: string]: number;}}*/ (() => undefined)(); // Hacking requirement for each server
    let unlockedSFs = [],
        nextBn = 0; // Info for the current bitnode
    ...
    function getTimeInAug() {
        return Date.now() - resetInfo.lastAugReset;
    }
    ...
    async function main_start(ns) {
        ...
        for (const flag of flagsSet)
            log(
                ns,
                `WARNING: You have previously enabled the flag "--${flag}". Because of the way this script saves its run settings, the ` +
                    `only way to now turn this back off will be to manually edit or delete the file ${ns.getScriptName()}.config.txt`,
                true,
            );
        ...
        while (keepRunning) {
            try {
                ...
                log(
                    ns,
                    `WARNING: autopilot.js Caught (and suppressed) an unexpected error:` +
                        `\n${getErrorInfo(err)}`,
                    false,
                    "warning",
                );
    ...
    async function startUp(ns) {
        ...
        homeRam = await getNsDataThroughFile(
            ns,
        ...
        try {
            if (!(4 in unlockedSFs)) {
                log(
                    ns,
                    `WARNING: This script requires SF4 (singularity) functions to assess purchasable augmentations ascend automatically. ` +
                        `Some functionality will be disabled and you'll have to manage working for factions, purchasing, and installing augmentations yourself.`,
                    true,
                );
                installedAugmentations = [];
                playerInstalledAugCount = null; // 'null' is treated as 'Unknown'
            } else {
                installedAugmentations = await getNsDataThroughFile(
                    ns,
            ...
            log(
                ns,
                `WARNING: You only have SF4 level ${unlockedSFs[4]}. Without level 3, some singularity functions will be ` +
                    `too expensive to run until you have bought a lot of home RAM.`,
                true,
            );
        }
        ...
        const getSFLevel = (bn) =>
            Number(
                bn +
                    "." +
                    ((dictOwnedSourceFiles[bn] || 0) +
                        (resetInfo.currentNode == bn ? 1 : 0)),
            );
        const nextSfEarned = getSFLevel(resetInfo.currentNode);
        const nextRecommendedSf = defaultBnOrder.find(
            (v) =>
                v - Math.floor(v) > getSFLevel(Math.floor(v)) - Math.floor(v),
        );
        const nextRecommendedBn = Math.floor(nextRecommendedSf);
        nextBn = options["next-bn"] || nextRecommendedBn;
        log(
            ns,
            `INFO: After the current BN (${nextSfEarned}), the next recommended BN is ${nextRecommendedBn} until you have SF ${nextRecommendedSf}.` +
                `\nYou are currently earning SF${nextSfEarned}, and you already own the following source files: ` +
                Object.keys(dictOwnedSourceFiles)
                    .map((bn) => `${bn}.${dictOwnedSourceFiles[bn]}`)
                    .join(", "),
        );
        if (nextBn != nextRecommendedBn)
            log(
                ns,
                `WARN: The next recommended BN is ${nextRecommendedBn}, but the --next-bn parameter is set to override this with ${nextBn}.`,
                true,
                "warning",
            );
    ...
    async function persistConfigChanges(ns) {
        ...
        const changedArgs = argsSchema
            .filter(
                (a) =>
                    JSON.stringify(options[a[0]], jsonReplacer) !=
                    JSON.stringify(a[1]),
                jsonReplacer,
            )
        ...
        for (const [key, finalValue] of changedArgs) {
            ...
            log(
                ns,
                `INFO: Default config has been modified: ${key}=${strFinalValue} (type="${typeof finalValue})" ` +
            ...
            if (
                ...
                log(
                    ns,
                    `WARNING: A configuration value provided (${key}=${strFinalValue} - ` +
        ...
        if (
            (strConfigChanges.length > 2 || currentConfig) &&
            strConfigChanges != currentConfig
        ) {
            ns.write(configPath, strConfigChanges, "w");
            log(
                ns,
                `INFO: Updated "${configPath}" to persist the most recent run args through resets: ${strConfigChanges}`,
                true,
                "info",
            );
        }
    ...
    async function mainLoop(ns) {
        ...
        let stocksValue = 0;
        try {
            stocksValue = await getStocksValue(ns);
        } catch {
            /* Assume if this fails (insufficient ram) we also have no stocks */
        }
    ...
    async function updateCachedData(ns) {
        ...
        if (4 in unlockedSFs) {
            // Note: Installed augmentations can also be obtained from getResetInfo() (without SF4), but this seems unintended and will probably be removed from the game.
            try {
                installedAugmentations = await getNsDataThroughFile(
                    ns,
                    "ns.singularity.getOwnedAugmentations()",
                ...
                log(
                    ns,
                    `WARNING: failed to update owned augmentations (low RAM?)`,
                    true,
                );
            }
        }
    ...
    async function checkOnDaedalusStatus(ns, player, stocksValue) {
        ...
        if (
            installedAugmentations.includes(augTRP) ||
        ...
        if (
            ...
            if (!(10 in unlockedSFs)) autoJoinDaedalusUnavailable = true; // Won't be able to unlock daedalus this ascend if we can't graft augs and have to install for them
        ...
        if (player.factions.includes("Daedalus")) {
            ...
            if (prioritizeHackForDaedalus || reservingMoneyForDaedalus) {
                ...
                if (reservingMoneyForDaedalus) {
                    ...
                    reason =
                        (reason ? reason + " and" : "by") +
                        " saving up our money";
                }
                log(
                    ns,
                    `SUCCESS: We sped along joining the faction 'Daedalus' ${reason}. ` + // Pat ourselves on the back
                        `Restarting work-for-factions.js to speed along earn rep.`,
                    false,
                    "success",
                );
        ...
        if (reservingMoneyForDaedalus && player.money >= moneyReq)
            // If our cash has dipped below the threshold again, we may need to take action below
            return await getNsDataThroughFile(
                ns,
                "ns.singularity.joinFaction(ns.args[0])",
                null,
                ["Daedalus"],
            ); // Note, we should have already checked that we have SF4 access before reserving money
        ...
        if (
            ...
            totalWorth >
                moneyReq *
                    1.001 /* slight buffer to account for timing issues */
        ) {
            ...
            if (!(4 in unlockedSFs)) {
                log(
                    ns,
                    `SUCCESS: ${player.money < moneyReq ? "If you sell your stocks, y" : "Y"}ou should have enough money ` +
                        ...
                        `augmentations installed (scripts cannot check this without SF4).`,
                    true,
                    "success",
                );
            ...
            if (player.money < moneyReq) {
                // Only liquidate stocks if we don't have enough cash lying around.
                disableStockmasterForDaedalus = true; // Flag to keep stockmaster offline until we've gotten a daedalus invite
                log(
                    ns,
                    "INFO: Temporarily liquidating stocks to earn an invite to Daedalus...",
                    true,
                    "info",
                );
            ...
            reservingMoneyForDaedalus &&
            totalWorth <
                moneyReq * 0.999 /* slight buffer to let cash recover */
        ) {
            ...
            log(
                ns,
                `WARN: We previously had sufficient wealth to earn a Daedalus invite (>=${formatMoney(moneyReq)}), ` +
                    `but our wealth somehow decreased (to ${formatMoney(totalWorth)}) before the invite was recieved, ` +
                    `so we'll need to wait for it to recover and try again later.`,
                false,
                "warning",
            );
        }
    ...
    async function checkIfBnIsComplete(ns, player) {
        ...
        if (wdHack === null) {
            // If we haven't checked yet, see if w0r1d_d43m0n (server) has been unlocked and get its required hack level
            wdHack = await getNsDataThroughFile(
                ns,
                'ns.scan("The-Cave").includes("w0r1d_d43m0n") ? ' +
        ...
        if (bnComplete) {
            const caveRooted = await getNsDataThroughFile(
                ns,
                "ns.hasRootAccess(ns.args[0])",
                null,
                ["w0r1d_d43m0n"],
            );
            if (!caveRooted) bnComplete = false;
        }

        // Detect the BB win condition (requires SF7 (bladeburner API) or being in BN6)
        if (7 in unlockedSFs)
            // No point making this async check if bladeburner API is unavailable
            playerInBladeburner =
                playerInBladeburner ||
        ...
        if (!bnComplete && playerInBladeburner)
            bnComplete = await getNsDataThroughFile(
                ns,
        ...
        if (!loggedBnCompletion) {
            const text =
                `BN ${resetInfo.currentNode}.${(dictOwnedSourceFiles[resetInfo.currentNode] || 0) + 1} completed at ` +
        ...
        if (options["on-completion-script"]) {
            const pid = launchScriptHelper(
                ns,
                options["on-completion-script"],
                unEscapeArrayArgs(options["on-completion-script-args"]),
                false,
            );
        ...
        if (resetInfo.currentNode == 10) {
            // Suggest the user doesn't reset until they buy all sleeves and max memory
            const shouldHaveSleeveCount = Math.min(
                8,
                6 + (dictOwnedSourceFiles[10] || 0),
            );
            const numSleeves = await getNsDataThroughFile(
                ns,
            ...
            if (numSleeves < shouldHaveSleeveCount)
                ...
                sleeveInfo = await getNsDataThroughFile(
                    ns,
                ...
                if (sleeveInfo.some((s) => s.memory < 100))
                    reasonToStay =
                        `Detected that you have ${numSleeves}/${shouldHaveSleeveCount} sleeves, but they do not all have the maximum memory of 100:\n  ` +
                        sleeveInfo
                            .map(
                                (s, i) =>
                                    `- Sleeve ${i} has ${s.memory}/100 memory`,
                            )
                            .join("\n  ");
                else sleevesMaxedOut = true; // Flag is used elsewhere to allow continued installs
            }
            if (reasonToStay) {
                log_once(
                    ns,
                    `WARNING: ${reasonToStay}\nTry not to leave BN10 before buying all you can from the faction "The Covenant", especially sleeve memory!` +
        ...
        if (options["disable-auto-destroy-bn"]) {
            log(
                ns,
                `--disable-auto-destroy-bn is set, you can manually exit the bitnode when ready.`,
                true,
            );
        ...
        if (!(4 in unlockedSFs)) {
            log(
                ns,
                `You do not own SF4, so you must manually exit the bitnode (` +
                    `${player.skills.hacking >= wdHack ? "by hacking W0r1dD43m0n" : "on the bladeburner BlackOps tab"}).`,
                true,
            );
            return (bnCompletionSuppressed = true);
        }
        ...
        pid = await runCommand(
            ns,
            `ns.singularity.destroyW0r1dD43m0n(ns.args[0], ns.args[1]` +
                `, { sourceFileOverrides: new Map() }` + // Explicit empty map for safety
                `)`,
            "/Temp/singularity-destroyW0r1dD43m0n.js",
            [nextBn, ns.getScriptName()],
        );
        if (pid) {
            log(
                ns,
                `SUCCESS: Initiated process ${pid} to execute 'singularity.destroyW0r1dD43m0n' with args: [${nextBn}, ${ns.getScriptName()}]`,
                true,
                "success",
            );
            ...
            log(
                ns,
                `WARNING: Process is done running, why am I still here? Sleeping 10 seconds...`,
                true,
                "error",
            );
        ...
        persist_log(
            ...
            log(
                ns,
                `ERROR: Tried destroy the bitnode (pid=${pid}), but we're still here...`,
                true,
                "error",
            ),
        );
    ...
    async function getRunningScripts(ns) {
        return await getNsDataThroughFile(ns, "ns.ps(ns.args[0])", null, [
            "home",
        ]);
    }
    ...
    function findScriptHelper(baseScriptName, runningScripts, filter = null) {
        return runningScripts.filter(
            (s) =>
                s.filename == getFilePath(baseScriptName) &&
                (!filter || filter(s)),
        )[0];
    }
    ...
    async function killScript(
        ...
        processInfo =
            ...
            findScriptHelper(
                baseScriptName,
                runningScripts || (await getRunningScripts(ns)),
            );
        if (processInfo) {
            log(
                ns,
                `INFO: Killing script ${baseScriptName} with pid ${processInfo.pid} and args: [${processInfo.args.join(", ")}].`,
                false,
                "info",
            );
            return await getNsDataThroughFile(ns, "ns.kill(ns.args[0])", null, [
                processInfo.pid,
            ]);
        }
        log(
            ns,
            `INFO: Skipping request to kill script ${baseScriptName}, no running instance was found...`,
            false,
            "warning",
        );
        return false;
    ...
    async function checkOnRunningScripts(ns, player) {
        if (lastScriptsCheck > Date.now() - options["interval-check-scripts"])
            return;
        ...
        const findScript =
            /** @param {(value: ProcessInfo, index: number, array: ProcessInfo[]) => unknown} filter @returns {ProcessInfo} */
            (baseScriptName, filter = null) =>
                findScriptHelper(baseScriptName, runningScripts, filter);
        ...
        if (
            ...
            !disableStockmasterForDaedalus &&
            homeRam >= 32
        )
            launchScriptHelper(ns, "stockmaster.js", [
                "--fracH",
                resetInfo.currentNode == 8 ? 0.001 : 0.1, // Fraction of wealth to keep as cash (10% - unless in BN8)
                "--reserve",
                0, // Override to ignore the global reserve.txt. Any money we reserve can more or less safely live as stocks
            ]);
        ...
        let existingSpendHashesProc = findScript(
            "spend-hacknet-hashes.js",
            (s) => s.args.includes("--spend-on-server"),
        );
        if (
            9 in unlockedSFs &&
            getTimeInAug() >=
                options["time-before-boosting-best-hack-server"] &&
            0 != bitNodeMults.ScriptHackMoney * bitNodeMults.ScriptHackMoneyGain
        ) {
            // No point in boosting hack income if it's scaled to 0 in the current BN
            ...
            if (strServerIncomeInfo) {
                ...
                dictServerHackReqs ??= await getNsDataThroughFile(
                    ns,
                    "Object.fromEntries(ns.args.map(server => [server, ns.getServerRequiredHackingLevel(server)]))",
                    "/Temp/getServerRequiredHackingLevel-all.txt",
                    incomeByServer.map((s) => s.hostname),
                );
                const [bestServer, gain] = incomeByServer
                    .filter(
                        (s) =>
                            dictServerHackReqs[s.hostname] <=
                            player.skills.hacking,
                    )
                    .reduce(
                        ([bestServer, bestIncome], target) =>
                            target.gainRate > bestIncome
                                ? [target.hostname, target.gainRate]
                                : [bestServer, bestIncome],
                        [null, -1],
                    );
                const spendHashesMultThreshold =
                    options["spend-hashes-on-server-hacking-threshold"];
                ...
                if (
                    bestServer &&
                    (gain > spendHashesMultThreshold ||
                        resetInfo.currentNode == 9)
                ) {
                    // Check whether we should be spending hashes to reduce minimum security
                    const serverMinSecurity = await getNsDataThroughFile(
                        ns,
                    ...
                    if (existingSpendHashesProc) {
                        const currentBoostTarget =
                            existingSpendHashesProc.args[
                                1 +
                                    existingSpendHashesProc.args.indexOf(
                                        "--spend-on-server",
                                    )
                            ];
                        const isReducingSecurity =
                            existingSpendHashesProc.args.includes(
                                "Reduce_Minimum_Security",
                            );
                        if (
                            currentBoostTarget != bestServer ||
                            isReducingSecurity != shouldReduceMinSecurity
                        ) {
                            log(
                                ns,
                                `Killing a prior spend-hacknet-hashes.js process targetting ${currentBoostTarget} because ` +
                                    (currentBoostTarget != bestServer
                                        ? `The new best income server is ${bestServer}.`
                                        : "We no longer need to reduce minimum security."),
                                true,
                            );
                            await killScript(
                                ns,
                    ...
                    if (!existingSpendHashesProc) {
                        ...
                        log(
                            ns,
                            `Identified that the best hack income server is ${bestServer} worth ${formatMoney(gain)}/sec.`,
                        );
                        const spendHashesArgs = [
                            "--liquidate",
                            "--spend-on-server",
                            bestServer,
                            "--spend-on",
                            "Increase_Maximum_Money",
                        ];
                        if (shouldReduceMinSecurity)
                            spendHashesArgs.push(
                                "--spend-on",
                                "Reduce_Minimum_Security",
                            );
                        launchScriptHelper(
                    ...
                    log_once(
                        ns,
                        `INFO: The best server (${bestServer})'s hack income multiplier (${formatNumber(gain)}) is currently too severely penalized ` +
                    ...
                    log(
                        ns,
                        `WARNING: strServerIncomeInfo was not empty, but could not determine best server:\n${strServerIncomeInfo}`,
                    );
            }
        }
        ...
        if (player.skills.hacking >= hackThreshold) {
            daemonArgs = [
                ...
                Math.min(5.0, player.skills.hacking / hackThreshold),
            ]; // Use more recovery thread padding as our hack level increases
            // Log a special notice if we're going to be relaunching daemon.js for this reason
            if (
                !existingDaemon ||
                !existingDaemon.args.includes("--looping-mode")
            )
            ...
            // If we're in early BN 1.1 (i.e. with < 32GB home RAM), avoid squandering RAM
            daemonArgs.push("--no-share", "--initial-max-targets", 1);
        } else {
            // XP-ONLY MODE: We can shift daemon.js to this when we want to prioritize earning hack exp rather than money
            let useXpOnlyMode =
                prioritizeHackForDaedalus ||
                prioritizeHackForWd ||
                ...
                (bitNodeMults.ScriptHackMoney *
                    ...
                    0 &&
                    resetInfo.currentNode != 8);
            if (!useXpOnlyMode) {
                // Otherwise, respect the configured interval / duration
                ...
                if (
                    xpInterval > 0 &&
                    xpDuration > 0 &&
                    minutesInAug % (xpInterval + xpDuration) <= xpDuration
                )
            ...
            if (useXpOnlyMode) {
                ...
                if (!existingDaemon?.args.includes("--xp-only"))
                    daemonRelaunchMessage = prioritizeHackForWd
                        ? `We're close to the required hack level destroy the BN.`
                        : prioritizeHackForDaedalus
        ...
        let launchDaemon =
            ...
            daemonArgs.some(
                (arg) =>
                    !existingDaemon.args.includes(arg) && !Number.isFinite(arg),
            ) ||
            // Special cases: We also must relaunch daemon if it is running with certain flags we wish to remove
            ["--xp-only"].some(
                (arg) =>
                    !daemonArgs.includes(arg) &&
                    existingDaemon.args.includes(arg),
            );
        if (launchDaemon) {
            if (existingDaemon) {
        ...
        if (options["no-tail-windows"])
            workForFactionsArgs.push("--no-tail-windows");
        if (options["disable-bladeburner"])
            workForFactionsArgs.push("--no-bladeburner-check");
        // The following args are ideal when running 'work-for-factions.js' to rush unlocking gangs (earn karma)
        const rushGangsArgs = workForFactionsArgs.concat(
            ...[
                ...
                200, // Be willing to spend more time grinding for stats rather than skipping a faction
                "--prioritize-invites",
            ],
        ...
        if (2 in unlockedSFs) {
            ...
            if (!playerInGang)
                playerInGang = await getNsDataThroughFile(
                    ns,
                    "ns.gang.inGang()",
                );
            ...
            const wrongWork = findScript(
                "work-for-factions.js",
                !rushGang
                    ? (s) => s.args.includes("--crime-focus")
                    : (s) => !rushGangsArgs.every((a) => s.args.includes(a)),
            ); // Require all rushGangsArgs if we're not in a gang yet.
            // If running with the wrong args, kill it so we can start it with the desired args
            if (wrongWork)
                await killScript(ns, "work-for-factions.js", null, wrongWork);
            ...
            if (playerInGang) {
                ...
                if (
                    existingGang &&
                    gangArgs.length > 0 &&
                    !gangArgs.every((a) => existingGang.args.includes(a))
                )
        ...
        if (
            4 in unlockedSFs &&
            !findScript("work-for-factions.js") &&
    ...
    async function maybeAcceptStaneksGift(ns, player) {
        ...
        if (
            bitNodeMults.StaneksGiftExtraSize == 0 &&
            bitNodeMults.StaneksGiftPowerMultiplier < 1
        )
            return;
        ...
        if (installedAugmentations.length > 1)
            log(
                ns,
                `WARNING: We think it's a good idea to accept Stanek's Gift, but it appears to be too late - other augmentations have been installed. Trying Anyway...`,
            );
        ...
        if (await getNsDataThroughFile(ns, "ns.stanek.acceptGift()")) {
            ...
            log(
                ns,
                `WARNING: autopilot.js tried to accepted Stanek's Gift, but was denied.`,
                true,
                "warning",
            );
    ...
    async function maybeUpgradeSleeveMemory(ns, player) {
        ...
        try {
            ...
            const buttons = [...doc.querySelectorAll("button")].filter(
                (b) =>
                    /memory/i.test(b.innerText) &&
                    /upgrade|purchase|buy/i.test(b.innerText),
            );
            let clicked = false;
            ...
            if (clicked)
                log(
                    ns,
                    "INFO: Attempted to purchase sleeve memory via UI.",
                    false,
                    "info",
                );
        } catch (err) {
            log(
                ns,
                `WARNING: Unable to automate sleeve memory upgrades: ${getErrorInfo(err)}`,
                false,
                "warning",
            );
        }
    ...
    async function maybeDoCasino(ns, player) {
        ...
        if (casinoEarnings >= 1e10) {
            log(
                ns,
                `INFO: Skipping running casino.js, as we've previously earned ${formatMoney(casinoEarnings)} and been kicked out.`,
            );
        ...
        if (playerWealth >= 1e12) {
            log(
                ns,
                `INFO: Skipping running casino.js, since we're already ridiculously wealthy (${formatMoney(playerWealth)} > 1t).`,
            );
            return (ranCasino = true);
        }
        ...
        if (resetInfo.currentNode != 8) {
            ...
            if (getTimeInAug() < 60000)
                return log_once(
                    ns,
                    `INFO: Waiting a minute to establish player income before deciding whether casino.js is needed.`,
                );
            // Since it's possible that the CashRoot Startker Kit could give a false income velocity, account for that.
            const cashRootBought =
                installedAugmentations.includes(`CashRoot Starter Kit`);
            const incomePerMs =
                (playerWealth - (cashRootBought ? 1e6 : 0)) / getTimeInAug();
            const incomePerMinute = incomePerMs * 60_000;
            if (incomePerMinute >= 5e9) {
                log(
                    ns,
                    `INFO: Skipping running casino.js this augmentation, since our income (${formatMoney(incomePerMinute)}/min) >= 5b/min`,
                );
                return (ranCasino = true);
            }
        }
        ...
        if (player.city != "Aevum" && player.money < 300000)
            return log_once(
                ns,
                `INFO: Waiting until we have ${formatMoney(300000)} to travel to Aevum and run casino.js`,
            );
        ...
        const pid = launchScriptHelper(ns, "casino.js", [
            "--kill-all-scripts",
            true,
            "--on-completion-script",
            ns.getScriptName(),
        ...
        if (pid) {
            ...
            log(
                ns,
                `ERROR: Something went wrong. casino.js was run, but we haven't been killed. It must have run into a problem...`,
            );
        }
    }
    ...
    async function maybeInstallAugmentations(ns, player) {
        if (!(4 in unlockedSFs))
            // Cannot automate augmentations or installs without singularity
            return setStatus(
                ns,
                `No singularity access, so you're on your own. You should manually work for factions and install augmentations!`,
            );
        ...
        if (reservedPurchase && installCountdown <= Date.now()) {
            log(
                ns,
                "INFO: Manually running faction-manager.js to ensure previously reserved purchase is still obtainable.",
            );
        ...
        if (!facman) {
            setStatus(
                ns,
                `Faction manager output not available. Will try again later.`,
            );
        ...
        const inFirstBn9Aug =
            resetInfo.currentNode == 9 &&
            Math.abs(resetInfo.lastNodeReset - resetInfo.lastAugReset) < 1000;
        ...
        let reducedAugReq = Math.floor(
            (options["reduced-aug-requirement-per-hour"] * getTimeInAug()) /
                3.6e6,
        );
        ...
        if (inFirstBn9Aug || resetInfo.currentNode == 8)
            // In BN8, no reset bonuses are possible, and we'd lose our stock progress
        ...
        const augsNeeded = Math.max(
            1,
            options["install-at-aug-count"] + sf11Level - reducedAugReq,
        );
        const augsNeededInclNf = Math.max(
            1,
            options["install-at-aug-plus-nf-count"] + sf11Level - reducedAugReq,
        );
        ...
        const pendingAugCount =
            facman.affordable_count_ex_nf + facman.awaiting_install_count_ex_nf; // Excludes neuroflux levels
        const pendingNfCount =
            facman.affordable_count_nf + facman.awaiting_install_count_nf; // Only neuroflux levels
        ...
        let augsToInstall = facman.awaiting_install_augs
            .filter((aug) => aug != strNF)
        ...
        const augSummary =
            `${pendingAugCount} of ${facman.unpurchased_count - 1} remaining augmentations` + // Unowned - 1 because we can always buy more Neuroflux
            (pendingNfCount > 0
                ? ` + ${pendingNfCount} levels of NeuroFlux.`
                : ".") +
            (pendingAugCount > 0
        ...
        let shouldReset =
            options["install-for-augs"].some((a) =>
                facman.affordable_augs.includes(a),
            ) ||
            pendingAugCount >= augsNeeded ||
            pendingAugInclNfCount >= augsNeededInclNf;
        ...
        if (
            !inFirstBn9Aug &&
            ((getTimeInAug() < 20 * 60 * 1000 &&
                pendingAugInclNfCount >= quickInstallThreshold) ||
        ...
        if (!shouldReset) {
            setStatus(
                ns,
                `Currently at ${formatDuration(getTimeInAug())} since last aug. ` +
        ...
        if (await shouldDelayInstall(ns, player, facman))
            // If we're currently in a state where we should not be resetting, skip reset logic
            return (reservedPurchase = 0);
        ...
        if (reservedPurchase < totalCost) {
            ...
            if (reservedPurchase == 0)
                ...
                // If we were already reserving for a purchase and the number went up, log a notice of the timer being reset.
                ...
                if (!augsToInstall.includes(augTRP)) {
                    ...
                    const newCountDown =
                        Date.now() +
                        Math.max(
                            10 * 1000, // At a bare minimum, wait 10 more seconds
                            // Heuristic: Linearly reduce the cooldown until we have doubled the aug count needed.
                            options["install-countdown"] *
                                (1 - installCountdownResets / augsNeededInclNf),
                        );
                    if (newCountDown > installCountdown) {
                        // If the existing countdown remaining was longer than this, leave it be
                        installCountdown = newCountDown;
                        purchaseChangeLog =
                            purchaseChangeLog +
                            " Resetting the timer before we install augmentations.";
                    }
                }
        ...
        if (installCountdown > Date.now()) {
            ...
            ns.toast(
                `Heads up: Autopilot plans to reset in ${formatDuration(installCountdown - Date.now())}`,
                "info",
            );
        ...
        if (pendingAugInclNfCount == 0)
            // If we know we would install 0 augs, but still wish to reset, we must enable soft resetting
    ...
    async function shouldDelayInstall(ns, player, facmanOutput) {
        ...
        if (await checkIfGrafting(ns)) return true;
        // Are we close to being able to afford 4S TIX data?
        if (!have4STixApi)
            have4STixApi = await getNsDataThroughFile(
                ns,
                `ns.stock.has4SDataTixApi()`,
            );
        if (!options["disable-wait-for-4s"] && !have4STixApi) {
            if (!have4SData)
                have4SData = await getNsDataThroughFile(
                    ns,
                    `ns.stock.has4SData()`,
                );
            const totalWorth = player.money + (await getStocksValue(ns));
            const totalCost =
                25e9 * bitNodeMults.FourSigmaMarketDataApiCost +
            ...
            if (ratio >= options["wait-for-4s-threshold"]) {
                setStatus(
                    ns,
                    `Not installing until scripts purchase the 4SDataTixApi because we have ` +
        ...
        if (resetInfo.currentNode == 8) {
            // Many special rules for this special Bitnode
            if (player.factions.includes("Daedalus")) {
                // If we've already joined Daedalus
                ...
                if (
                    !installedAugmentations.includes(augTRP) &&
                    !facmanOutput.affordable_augs.includes(augTRP) &&
                    !facmanOutput.awaiting_install_augs.includes(augTRP)
                ) {
                    setStatus(
                        ns,
                        `We're in Daedalus, so we won't install until we can afford to purchase "${augTRP}".`,
                    );
                    return true;
                }
            } else if (
                playerInstalledAugCount >=
                    bitNodeMults.DaedalusAugsRequirement &&
                player.skills.hacking >= 2500 * 0.9
            ) {
                // If we meet the Daedalus aug count requirement and at least 90% of the required hack level, wait to earn the invite
                setStatus(
                    ns,
                    `Not installing because we're in BN8 and we have enough augs and ` +
                        (player.skills.hacking < 2500 ? "nearly " : "") +
                        "enough hack level to get invited to Daedalus once we hit $100b.",
                );
                return true;
            } else if (getTimeInAug() > 4 * 60 * 60 * 1000) {
                // 4 hours = 4hrs/min * 60mins/sec * 60secs/ms * 1000ms
                ...
                const augsReadyToInstall =
                    facmanOutput.awaiting_install_count_ex_nf +
                    facmanOutput.affordable_count_ex_nf;
                if (augsReadyToInstall < 10) {
                    // Heuristic: 10 augs per install means max 3 installs before we meet the Daedalus aug requirement
                    setStatus(
                        ns,
                        `Not installing because we've in BN8 for more than 4 hours (~${Math.round(getTimeInAug() / 1000 / 60 / 60)}h) and aren't in Daedalus yet, ` +
        ...
        if (reservingMoneyForDaedalus) {
            setStatus(
                ns,
                `Not installing since we are close to earning an invite from Daedalus.`,
            );
            return true;
        }
        ...
        if (
            resetInfo.currentNode == 10 &&
            player.money >= 10e15 &&
            !sleevesMaxedOut
        ) {
            // Heuristic: If we hit 10q (10% the cost of the last sleeve) before an install, we can probably go all the way
            setStatus(
                ns,
                `Not installing anymore since we are nearing the 100q needed to purchase the 6th sleeve from the Covenant.`,
            );
            return true;
        }
        ...
        if (
            ...
            if (currentAction?.type == "BlackOp") {
                setStatus(
                    ns,
                    `Not installing because a Bladeburner BlackOp is in progress (${currentAction.name}).`,
                );
                return true;
            }
        }
        const favorToDonate = await getNsDataThroughFile(
            ns,
            "ns.getFavorToDonate()",
        );
        const factions = player.factions;
        if (factions.length > 0) {
            const dictFavors = await getNsDataThroughFile(
                ns,
                "Object.fromEntries(ns.args.map(f => [f, ns.singularity.getFactionFavor(f)]))",
                "/Temp/faction-favor.txt",
                factions,
            );
            const dictReps = await getNsDataThroughFile(
                ns,
                "Object.fromEntries(ns.args.map(f => [f, ns.singularity.getFactionRep(f)]))",
                "/Temp/faction-rep.txt",
                factions,
            );
            const repToFavour = (rep) =>
                Math.ceil(25500 * 1.02 ** (rep - 1) - 25000);
            for (const fac of factions) {
                ...
                if (
                    ...
                    setStatus(
                        ns,
                        `Not installing because we are close to unlocking donations with ${fac}.`,
                    );
                    return true;
                }
            }
    ...
    async function checkIfGrafting(ns) {
        ...
        currentWork = await getNsDataThroughFile(
            ns,
        ...
        if (currentWork?.type == "GRAFTING") {
            if (!wasGrafting)
                // Only log the first time we detect we've started grafting
                log(
                    ns,
                    "Grafting in progress. autopilot.js will make sure to not install augmentations or otherwise interrupt it.",
                );
            return (wasGrafting = true);
        } else return (wasGrafting = false);
    }
    ...
    function manageReservedMoney(ns, player, stocksValue) {
        ...
        if (reservingMoneyForDaedalus)
            // Reserve 100b to get the daedalus invite
            return currentReserve == 100e9
                ? true
                : ns.write("reserve.txt", 100e9, "w");
        ...
        const minStockPercent = Math.max(
            0,
            0.8 - (0.1 * getTimeInBitnode()) / 3.6e6,
        ); // Reduce by 10% per hour in the BN
        ...
        const reserve = Math.min(
            ...
            Math.max(
                0,
                player.money * minStockPercent,
                minStockValue - stocksValue,
            ),
        );
        return currentReserve == reserve
            ? true
            : ns.write("reserve.txt", reserve, "w"); // Reserve for stocks
    ...
    function shouldWeKeepRunning(ns) {
        if (4 in unlockedSFs) return true; // If we have SF4 - run always
        // If we've gotten daemon.js launched, but only have 8GB ram, we must shut down for now
        if (homeRam == 8 && daemonStartTime > 0) {
            log(
                ns,
                `WARN: (not an actual warning, just trying to make this message stand out.)` +
                    `\n` +
                    "-".repeat(100) +
    ...
    function launchScriptHelper(
        ...
        if (!options["no-tail-windows"]) tail(ns); // If we're going to be launching scripts, show our tail window so that we can easily be killed if the user wants to interrupt.
        let pid, err;
        try {
            pid = ns.run(
    ...
    function persist_log(ns, text) {
        ns.write(
            persistentLog,
            `${new Date().toISOString().substring(0, 19)} ${text}\n`,
            "a",
        );
    }
    ...
    function log_once(ns, message, alsoPrintToTerminal, toastStyle) {
        if (logged_once.has(message)) return;
