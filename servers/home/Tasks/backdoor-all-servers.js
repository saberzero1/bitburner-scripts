...
export async function main(ns) {
    ...
    try {
        ...
        if ((await instanceCount(ns, "home", false, false)) > 1)
            return log(
                ns,
                "Another instance is already running. Shutting down...",
            );
        ...
        const myHackingLevel = await getNsDataThroughFile(
            ns,
            "ns.getHackingLevel()",
        );
        ...
        for (let i = 0, j; i < servers.length; i++)
            for (j of await getNsDataThroughFile(
                ns,
                `ns.scan(ns.args[0])`,
                null,
                [servers[i]],
        ...
        const hackableServers = servers.filter(
            (s) =>
                s != "home" && !s.includes("hacknet-") && !s.includes("daemon"),
        ); /*or whatever you name your purchased servers*/

        // Get the required hacking level of each server
        const dictRequiredHackingLevels = await getNsDataThroughFile(
            ns,
        ...
        const dictRootAccess = await getNsDataThroughFile(
            ns,
            `Object.fromEntries(ns.args.map(server => [server, ns.hasRootAccess(server)]))`,
            "/Temp/hasRootAccess-all.txt",
            hackableServers,
        );

        ns.print(`${hackableServers.length} not-owned servers on the network.`);
        ns.print(
            `${hackableServers.filter((s) => dictRootAccess[s]).length} servers are currently rooted.`,
        );
        ns.print(
            `${hackableServers.filter((s) => myHackingLevel > dictRequiredHackingLevels[s]).length} servers are within our hack level (${myHackingLevel}).`,
        );
        ns.print(
            `${hackableServers.filter((s) => myHackingLevel > dictRequiredHackingLevels[s] && dictRootAccess[s]).length} rooted servers are within our hack level (${myHackingLevel})`,
        );
        ...
        let toBackdoor = await getNsDataThroughFile(
            ...
            "/Temp/getServers-where-not-backdoorInstalled.txt",
            hackableServers,
        );
        ...
        ns.print(
            `${toBackdoor.filter((s) => dictRootAccess[s]).length} of ${count} servers to backdoor are currently rooted.`,
        );
        toBackdoor = toBackdoor.filter(
            (s) => myHackingLevel > dictRequiredHackingLevels[s],
        );
        ns.print(
        ...
        toBackdoor.sort(
            (a, b) =>
                dictRequiredHackingLevels[a] - dictRequiredHackingLevels[b],
        );
        ns.print(
            `Servers will be backdoored in the following order:\n` +
                toBackdoor.join(", "),
        );

        // Collect information about any servers still being backdoored (from a prior run), so we can skip them
        let scriptPath = getFilePath(
            "/Tasks/backdoor-all-servers.js.backdoor-one.js",
        );
        let serversBeingBackdoored = await getNsDataThroughFile(
            ns,
            "ns.ps().filter(script => script.filename == ns.args[0]).map(script => script.args[0])",
            "/Temp/servers-being-backdoored.txt",
            [scriptPath],
        );
        ...
        for (const server of toBackdoor) {
            ...
            const homeFreeRam = await getNsDataThroughFile(
                ns,
                "ns.getServerMaxRam(ns.args[0]) - ns.getServerUsedRam(ns.args[0])",
                "/Temp/getServerFreeRam.txt",
                ["home"],
            );
            if (homeFreeRam < options["reserved-home-ram"])
                return log(
                    ns,
                    `WARNING: Home is low on RAM, will skip backdooring remaining servers.`,
                );
            ...
            const success = await getNsDataThroughFile(
                ...
                "/Temp/singularity-connect-hop-to-server.txt",
                routes[server],
            );
            if (!success)
                log(
                    ns,
                    `ERROR: Failed to hop to server ${server}. Backdoor probably won't work...`,
                    true,
                    "error",
                );
            if (server === "w0r1d_d43m0n") {
                ...
                log(
                    ns,
                    "INFO: Sleeping forever to avoid multiple instances navigating to w0r1d_d43m0n.",
                );
            ...
            if (pid === 0)
                return log(
                    ns,
                    `WARN: Couldn't initiate a new backdoor of "${server}" (insufficient RAM?). Will try again later.`,
                    false,
                    "warning",
                );
            ...
            const backAtHome = await getNsDataThroughFile(
                ns,
        ...
        log(
            ns,
            `ERROR: ${ns.getScriptName()} Caught an unexpected error:\n${getErrorInfo(err)}`,
            false,
            "error",
        );
    } finally {
