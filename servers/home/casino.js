...
export async function main(ns) {
    ...
    async function start() {
        ...
        function showDialog(onCancel) {
            ...
            dlg.style.cssText =
                "position:fixed;top:20px;right:20px;z-index:9999;padding:12px 16px;background:#111;color:#ddd;border:1px solid #444;border-radius:6px;font:12px monospace;max-width:280px;";
            dlg.innerHTML =
                `<div>casino.js is running until it wins \$10b.</div>` +
        ...
        async function checkStillAtCasino(throwError = true, silent = false) {
            ...
            const focusWasStolen = await checkForStolenFocus(
                throwError,
                silent ? 3 : 1,
            );
            ...
            try {
                ...
                if (ns.getPlayer().city != "Aevum") {
                    if (ns.getPlayer().money < 200000)
                        throw new Error(
                            "Sorry, you need at least 200k to travel to the casino.",
                        );
                    let travelled = false;
                    try {
                ...
                try {
                    ...
                    let success = false,
                        err2;
                    try {
                        success = await getNsDataThroughFile(
                            ns,
                ...
                if (ns.ls("home", "Temp/").length > 0) {
                    ...
                    const moneySources = await getNsDataThroughFile(
                        ns,
                        "ns.getMoneySources()",
                    ); // NEW (2022): We can use money sources to see what our casino earnings have been
                    const priorCasinoEarnings =
                        moneySources.sinceInstall.casino;
                    ...
                    if (
                        ...
                        if (await checkForKickedOut())
                            return await onCompletion(false); // We appear to have previously been kicked out
                        throw new Error(
        ...
        try {
            ...
            while (true) {
                ...
                const bet = Math.min(
                    1e8,
                    ns.getPlayer().money *
                        0.9 /* Avoid timing issues with other scripts spending money */,
                );
                ...
                if (!gameStarted) {
                    ...
                    if (winLoseTie == null) {
                        ...
                        if (startGameRetries++ >= 5)
                            ...
                            throw new Error(
                                errMessage +
                                    ` Gave up after 5 retry attempts.\n${supportMsg}`,
                            );
                        tail(ns); // Since we're having difficulty, pop open a tail window so the user is aware and can monitor.
                        verbose = true; // Switch on verbose logs
                        log(
                            ns,
                            `WARNING: ${errMessage} Trying again...`,
                            false,
                            "warning",
                        );
                    ...
                    try {
                        ...
                        if (options["use-basic-strategy"]) {
                            ...
                            const highCount = Number(
                                allCounts[allCounts.length - 1].innerText,
                            ); // The larger value (with Ace=11) - used in basic-strategy mode
                            shouldHit = highCount < 17; // Basic strategy, hit on 16 or less, stay on 17 or over (whether hard or soft)
                            if (verbose)
                                log(
                                    ns,
                                    `INFO: Count is ${highCount}, we will ${shouldHit ? "Hit" : "Stay"}`,
                                );
                        } // Advanced strategy will also look at the dealer card
                        else shouldHit = await shouldHitAdvanced(txtCount);
                        ...
                        if (midGameRetries++ >= 5)
                            // Retry up to 5 times before giving up and crashing out.
                            throw new Error(
                                `After ${priorAttempts} attempts, casino.js continues to catch ${errMessage}`,
                            );
                        tail(ns); // Since we're having difficulty, pop open a tail window so the user is aware and can monitor.
                        verbose = true; // Switch on verbose logs
                        log(
                ...
                switch (winLoseTie) {
                    ...
                    case "lose":
                        ...
                        if (
                            ns.getPlayer().money < 1e8 ||
                            netWinnings <= peakWinnings - 10 * 1e8
                        )
                ...
                throw new Error(
                    "This code should be unreachable - did someone break the logic above?",
                );
            }
        } catch (err) {
    ...
    async function reload() {
        ...
        while (attempts++ <= 5) {
            ...
            else if (attempts < 4) win.location.reload(true);
    ...
    async function onCompletion(kickedOutAfterPlaying = true) {
        ...
        try {
            const terminalNav = await tryfindElement(
                "//div[(@role = 'button') and (contains(., 'Terminal'))]",
            );
            if (terminalNav) await click(terminalNav);
        } catch (err) {
            log(
                ns,
                `WARNING: Failed to route to the terminal: ${getErrorInfo(err)}`,
                false,
            );
        }
        ...
        if (ns.run(completionScript, 1, ...completionArgs))
            log(
                ns,
                `INFO: casino.js shutting down and launching ${completionScript}...`,
                false,
                "info",
            );
        else
            log(
                ns,
                `WARNING: casino.js shutting down, but failed to launch ${completionScript}...`,
                false,
                "warning",
            );
    }
    ...
    async function findRequiredElement(
        xpath,
        retries = 15,
        customErrorMessage = null,
    ) {
        return await internalfindWithRetry(
            xpath,
            false,
            retries,
            customErrorMessage,
        ...
        try {
            ...
            if (expectFailure) {
                if (verbose)
                    log(
                        ns,
                        `INFO: Element doesn't appear to be present, moving on...`,
                        false,
                    );
            } else {
                const errMessage =
    ...
    function makeLogSafeXPath(xpath) {
        ...
        const converted = xpath
            .split("")
            .map((ch) => map[ch] ?? ch)
            .join("");
        return converted.length > 60
            ? converted.slice(0, 60) + "..."
            : converted;
    }
    ...
    async function shouldHitAdvanced(playerCountElem) {
        ...
        if (verbose)
            log(
                ns,
                `Player Count Text: ${txtPlayerCount}, Player: ${player}, Dealer: ${dealer}`,
            );
