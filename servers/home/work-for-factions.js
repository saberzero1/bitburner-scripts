...
const jobs = [
    // Job stat requirements for a company with a base stat modifier of +224 (modifier of all megacorps except the ones above which are 25 higher)
    {
        ...
        reqCha: [0, 0, 275, 300], // [0,  0, 51,  76] + 224
        repMult: [0.9, 1.1, 1.3, 1.4],
    },
    {
        name: "Software",
        reqRep: [0, 8e3, 4e4, 2e5, 4e5, 8e5, 16e5, 32e5],
        reqHck: [225, 275, 475, 625, 725, 725, 825, 975], // [1, 51, 251, 401, 501, 501, 601, 751] + 224
        reqCha: [0, 0, 275, 375, 475, 475, 625, 725], // [0,  0,  51, 151, 251, 251, 401, 501] + 224
...
let dictSourceFiles,
    ...
    lastTravel,
    crimeCount,
    lastStockLiquidation;
let firstFactions,
    skipFactions,
    completedFactions,
    softCompletedFactions,
...
const breakToMainLoop = () =>
    Date.now() > mainLoopStart + checkForNewPrioritiesInterval;
...
export async function main(ns) {
    ...
    lastTravel = crimeCount = currentBitnode = lastStockLiquidation = 0;
    ...
    if (options["desired-stats"].length == 0)
        options["desired-stats"] = options["crime-focus"]
            ? ["str", "def", "dex", "agi", "faction_rep", "hacknet", "crime"]
            : [
                  "hacking",
                  "faction_rep",
                  "company_rep",
    ...
    ns.print(
        `--desired-stats matching: ${options["desired-stats"].join(", ")}`,
    );
    ...
    if (options.skip.length > 0)
        ns.print(`--skip factions: ${options.skip.join(", ")}`);
    ...
    while (!loadingComplete) {
        try {
            ...
            log(
                ns,
                "WARNING: work-for-factions.js caught an unhandled error while starting up. Trying again in 5 seconds...\n" +
                    getErrorInfo(err),
                false,
                "warning",
            );
    ...
    while (true) {
        ...
        try {
            ...
            log(
                ns,
                "WARNING: work-for-factions.js caught an unhandled error in its main loop. Trying again in 5 seconds...\n" +
                    getErrorInfo(err),
                false,
                "warning",
            );
...
async function loadStartupData(ns) {
    ...
    const allKnownFactions = factions.concat(
        playerInfo.factions.filter((f) => !factions.includes(f)),
    );
    ...
    const augmentationNames = [
        ...new Set(Object.values(dictFactionAugs).flat()),
    ];
    ...
    hasFocusPenalty = !installedAugmentations.includes(
        "Neuroreceptor Management Implant",
    ...
    if (
        playerGang &&
        !options["disable-treating-gang-as-sole-provider-of-its-augs"]
    ) {
        ...
        ns.print(
            `Your gang ${playerGang} provides easy access to ${gangAugs.length} augs. Ignoring these augs from the original factions that provide them.`,
        );
        for (const faction of allKnownFactions.filter((f) => f != playerGang))
            dictFactionAugs[faction] = dictFactionAugs[faction].filter(
                (a) => !gangAugs.includes(a),
            );
    }

    const orderedFactionList = [
        ...new Set(
            firstFactions.concat(
                preferredEarlyFactionOrder,
                preferredCompanyFactionOrder,
                allKnownFactions,
            ),
        ),
    ];
    const augPrimaryFaction = {};
    for (const aug of augmentationNames) {
        const owner = orderedFactionList.find((f) =>
            dictFactionAugs[f]?.includes(aug),
        );
        if (owner) augPrimaryFaction[aug] = owner;
    }
    const dedupedFactionAugs = Object.fromEntries(
        allKnownFactions.map((f) => [
            f,
            dictFactionAugs[f].filter((aug) => augPrimaryFaction[aug] === f),
        ]),
    );
    mostExpensiveAugByFaction = Object.fromEntries(
        allKnownFactions.map((f) => [
            f,
            dedupedFactionAugs[f]
                ...
                .filter(
                    (aug) =>
                        ...
                        (options["desired-augs"].includes(aug) ||
                            ...
                            options["desired-stats"].length == 0 ||
                            Object.keys(dictAugStats[aug]).some((key) =>
                                options["desired-stats"].some((stat) =>
                                    key.includes(stat),
                                ),
                            )),
                )
    ...
    const isPrecludedByCity = (factionName) =>
        (["Aevum", "Sector-12"].includes(factionName) &&
            ["Chongqing", "New Tokyo", "Ishima", "Volhaven"].some((f) =>
                joinedFactions.includes(f),
            )) ||
        (["Chongqing", "New Tokyo", "Ishima"].includes(factionName) &&
            ["Aevum", "Sector-12", "Volhaven"].some((f) =>
                joinedFactions.includes(f),
            )) ||
        (["Volhaven"].includes(factionName) &&
            ["Aevum", "Sector-12", "Chongqing", "New Tokyo", "Ishima"].some(
                (f) => joinedFactions.includes(f),
            ));
    const filterableFactions = allKnownFactions.filter(
        (f) => !isPrecludedByCity(f),
    );
    // Unless otherwise configured, we will skip factions with no remaining augmentations
    completedFactions = filterableFactions.filter(
        (fac) => mostExpensiveAugByFaction[fac] == -1,
    );
    softCompletedFactions = filterableFactions.filter(
        (fac) =>
            mostExpensiveDesiredAugByFaction[fac] == -1 &&
            !completedFactions.includes(fac),
    );
    const precludedFactions = allKnownFactions.filter((f) =>
        isPrecludedByCity(f),
    );
    skipFactions = options.skip
        .concat(cannotWorkForFactions)
        .concat(completedFactions)
        .concat(precludedFactions)
        .filter((fac) => !firstFactions.includes(fac));
    if (completedFactions.length > 0)
        ns.print(
            `${completedFactions.length} factions will be skipped (for having all augs purchased): ${completedFactions.join(", ")}`,
        );
    if (softCompletedFactions.length > 0)
        ns.print(
            `${softCompletedFactions.length} factions will initially be skipped (all desired augs purchased): ${softCompletedFactions.join(", ")}`,
        );
...
async function mainLoop(ns) {
    ...
    const cityPrecluded = (factionName) =>
        (["Aevum", "Sector-12"].includes(factionName) &&
            ["Chongqing", "New Tokyo", "Ishima", "Volhaven"].some((f) =>
        ...
        (["Volhaven"].includes(factionName) &&
            ["Aevum", "Sector-12", "Chongqing", "New Tokyo", "Ishima"].some(
                (f) => player.factions.includes(f),
            ));
    const cityInviteCandidates = cityFactions
        .filter(
            (f) =>
                !player.factions.includes(f) &&
        ...
        .filter((f) => (requiredMoneyByFaction[f] ?? 0) <= player.money);
    if (
        cityInviteCandidates.length > 0 &&
        Date.now() - lastTravel > waitForFactionInviteTime
    ) {
        const targetCity = cityInviteCandidates.includes(player.city)
            ? player.city
            : cityInviteCandidates[0];
        await goToCity(ns, targetCity);
    }
    const invites = await checkFactionInvites(ns);
    const invitesToAccept =
        options["get-invited-to-every-faction"] || options["prioritize-invites"]
            ? invites.filter((f) => !skipFactions.includes(f))
            : invites.filter(
                  (f) =>
                      !skipFactions.includes(f) &&
                      !softCompletedFactions.includes(f),
              );
    for (const invite of invitesToAccept) await tryJoinFaction(ns, invite);
    // Get some information about gangs (if unlocked)
    if (2 in dictSourceFiles) {
        if (!playerGang) {
            // Check if we've joined a gang since our last iteration
            const gangInfo = await getGangInfo(ns);
            playerGang = gangInfo ? gangInfo.faction : null;
        ...
        if (ns.heart.break() <= options["karma-threshold-for-gang-invites"]) {
            // Start trying to earn gang faction invites if we're close to unlocking gangs
            if (!playerGang) {
                log(
                    ns,
                    `INFO: We are nearing the Karma required to unlock gangs (${formatNumberShort(ns.heart.break())} / -54K). Prioritize earning gang faction invites.`,
                );
    ...
    if (await isValidInterruption(ns)) return await ns.sleep(loopSleepInterval);
    ...
    if (!options["crime-focus"] && options["prioritize-invites"])
        priorityFactions = cityFactions.concat(priorityFactions);
    priorityFactions = [...new Set(priorityFactions)];
    if (player.skills.hacking < fulcrumHackReq - 10) {
        // Assume that if we're within 10, we'll get there by the time we've earned the invite
        const fulcrumIdx = priorityFactions.findIndex(
            (c) => c == "Fulcrum Secret Technologies",
        );
        if (fulcrumIdx !== -1) {
            priorityFactions.splice(fulcrumIdx, 1);
            ns.print(
                `Fulcrum faction server requires ${fulcrumHackReq} hack, so removing from our initial priority list for now.`,
            );
        }
    }
    if (!options["crime-focus"] && priorityFactions.includes("The Dark Army")) {
        const requirement = requiredCombatByFaction["The Dark Army"] ?? 0;
        ...
        if (minCombat < requirement) {
            priorityFactions = priorityFactions
                .filter((f) => f != "The Dark Army")
                .concat(["The Dark Army"]);
        }
    }
    if (player.factions.includes("Fulcrum Secret Technologies")) {
        priorityFactions = priorityFactions.filter(
            (f) =>
                !(
                    preferredCompanyFactionOrder.includes(f) &&
                    mostExpensiveDesiredAugByFaction[f] == -1 &&
                    !firstFactions.includes(f)
                ),
        );
    }
    ...
    if (currentBitnode == 10 && !priorityFactions.includes("The Covenant")) {
        ...
        ns.print(
            `We're in BN10, which means we should add The Covenant to our priority faction list, so you can purchase sleeves and sleeve memory.`,
        );
    }

    // Strategy 1: Tackle a consolidated list of desired faction order, interleaving simple factions and megacorporations
    const factionWorkOrder = firstFactions.concat(
        priorityFactions.filter(
            (
                f, // Remove factions from our initial "work order" if we've bought all desired augmentations.
            ) =>
                !firstFactions.includes(f) &&
                !skipFactions.includes(f) &&
                !softCompletedFactions.includes(f),
    ...
    let knownFactions = factions.concat(
        joinedFactions.filter((f) => !factions.includes(f)),
    );
    let allIncompleteFactions = knownFactions
        .filter(
            (f) => !skipFactions.includes(f) && !completedFactions.includes(f),
        )
        .sort(
            (a, b) =>
                mostExpensiveAugByFaction[a] - mostExpensiveAugByFaction[b],
        ); // sort by least-expensive final aug (correlated to easiest faction-invite requirement)
    ...
    let allFactionsWorkOrder = factionWorkOrder
        .filter((f) => allIncompleteFactions.includes(f))
        .concat(
            allIncompleteFactions.filter((f) => !factionWorkOrder.includes(f)),
        );
    ...
    for (const faction of allFactionsWorkOrder.filter(
    ...
    let allFactionsWorkOrderReversed = factionWorkOrder
        .filter((f) => allIncompleteFactions.includes(f))
        .concat(
            allIncompleteFactions
                .reverse()
                .filter((f) => !factionWorkOrder.includes(f)),
        );
    for (const faction of allFactionsWorkOrderReversed)
        if (!breakToMainLoop())
            // Only continue on to the next faction if it isn't time for a high-level update.
    ...
    let factionsWeCanWorkFor = joinedFactions.filter(
        (f) =>
            !options.skip.includes(f) &&
            !cannotWorkForFactions.includes(f) &&
            f != playerGang,
    );
    let foundWork = false;
    ...
    if (factionsWeCanWorkFor.length > 0 && !options["crime-focus"]) {
        // Unless we've been asked to prioritize crime (e.g. for Karma)
        let mostFavorFaction = factionsWeCanWorkFor.sort(
            (a, b) => (dictFactionFavors[b] || 0) - (dictFactionFavors[a] || 0),
        )[0];
        let targetRep =
            1000 + (await getFactionReputation(ns, mostFavorFaction)) * 1.05; // Hack: Grow rep by ~5%, plus 1000 incase it's currently 0
        ns.print(
            `INFO: All useful work complete. Grinding an additional 5% rep (to ${formatNumberShort(targetRep)}) ` +
                `with highest-favor faction: ${mostFavorFaction} (${(dictFactionFavors[mostFavorFaction] || 0).toFixed(2)} favor)`,
        );
        foundWork = await workForSingleFaction(
            ns,
            mostFavorFaction,
            false,
            false,
    ...
    if (!foundWork && !options["no-crime"]) {
        ...
        await crimeForKillsKarmaStats(
            ns,
            0,
            -ns.heart.break() + 1000 /* Hack: Decrease Karma by 1000 */,
            0,
        );
    } else if (!foundWork) {
        // If our hands our tied, twiddle our thumbs a bit
        ns.print(
            `INFO: Nothing to do. Sleeping for 30 seconds to see if magically we join a faction`,
        );
...
function heuristic(player, stat, trainingBitnodeMult) {
    return Math.sqrt(
        player.mults[stat] *
            bitNodeMults[`${title(stat)}LevelMultiplier`] *
            /* */ player.mults[`${stat}_exp`] *
            trainingBitnodeMult,
    );
}
/** A heuristic for how long it'll take to train the specified stat via Crime. @param {Player} player @param {string} stat @param */
const crimeHeuristic = (player, stat) =>
    heuristic(player, stat, bitNodeMults.CrimeExpGain); // When training with crime
...
const classHeuristic = (player, stat) =>
    heuristic(player, stat, bitNodeMults.ClassGymExpGain); // When training in university
...
async function earnFactionInvite(ns, factionName) {
    ...
    let deficientStats = !requirement
        ? []
        : physicalStats
              .map((stat) => ({ stat, value: player.skills[stat] }))
              .filter((stat) => stat.value < requirement);
    const hackHeuristic = classHeuristic(player, "hacking");
    const crimeHeuristics = Object.fromEntries(
        physicalStats.map((s) => [s, crimeHeuristic(player, s)]),
    );
    const gymHeuristics = Object.fromEntries(
        physicalStats.map((s) => [s, classHeuristic(player, s)]),
    );
    ...
    if (
        ...
        deficientStats.length > 0 &&
        // Compare roughly how long it will take to train up our hacking stat
        (requiredHackByFaction[factionName] - player.skills.hacking) /
            hackHeuristic <
            // To the slowest time it will take to train up our deficient physical stats
            Math.min(
                ...deficientStats.map(
                    (s) =>
                        (requiredCombatByFaction[factionName] - s.value) /
                        crimeHeuristics[s.stat],
                ),
            )
    )
        ns.print(
            `Ignoring combat requirement for ${factionName} as we are more likely to unlock them via hacking stats.`,
        );
    else if (deficientStats.length > 0) {
        ns.print(
            `${reasonPrefix} you have insufficient combat stats. Need: ${requirement} of each, Have ` +
                physicalStats
                    .map((s) => `${s.slice(0, 3)}: ${player.skills[s]}`)
                    .join(", "),
        );
        const em = requirement / options["training-stat-per-multi-threshold"];
        if (deficientStats.some((s) => crimeHeuristics[s.stat] < em))
            return ns.print(
                "Some mults * exp_mults * bitnode mults appear to be too low to increase stats in a reasonable amount of time. " +
                    `You can control this with --training-stat-per-multi-threshold. Current sqrt(mult*exp_mult*bn_mult*bn_exp_mult) ` +
                    `should be ~${formatNumberShort(em, 2)}, have ` +
                    deficientStats
                        .map((s) => s.stat)
                        .map(
                            (s) =>
                                ...
                                `${formatNumberShort(bitNodeMults.CrimeExpGain)})=${formatNumberShort(crimeHeuristics[s])}`,
                        )
                        .join(", "),
            );
        const gymFaster = deficientStats.some(
            (s) => gymHeuristics[s.stat] > crimeHeuristics[s.stat],
        );
        if (gymFaster) {
            if (!(await trainCombatStats(ns, requirement))) return false;
    ...
    if (doCrime && options["no-crime"])
        return ns.print(
            `${reasonPrefix} Doing crime to meet faction requirements is disabled. (--no-crime or --no-focus)`,
        );
    if (doCrime)
    ...
    if ((requirement = requiredBackdoorByFaction[factionName])) {
        serverReqHackingLevel = await getServerRequiredHackLevel(
            ns,
            requirement,
        );
        if (player.skills.hacking < serverReqHackingLevel) {
            ns.print(
                `${reasonPrefix} you must first backdoor ${requirement}, which needs hack: ${serverReqHackingLevel}, Have: ${player.skills.hacking}`,
            );
        }
    }
    requirement = Math.max(
        serverReqHackingLevel,
        requiredHackByFaction[factionName] || 0,
    );
    if (
        requirement &&
        player.skills.hacking < requirement &&
        ...
        ns.print(
            `${reasonPrefix} you have insufficient hack level. Need: ${requirement}, Have: ${player.skills.hacking}`,
        );
        const em = requirement / options["training-stat-per-multi-threshold"];
        if (options["no-studying"])
            return ns.print(
                `--no-studying is set, nothing we can do to improve hack level.`,
            );
        if (hackHeuristic < em)
            return ns.print(
                `Your combination of Hacking mult (${formatNumberShort(player.mults.hacking)}), exp_mult ` +
        ...
        if (player.money > options["pay-for-studies-threshold"]) {
            // If we have sufficient money, pay for the best studies
            if (player.city != "Volhaven") await goToCity(ns, "Volhaven");
            studying = await study(ns, false, "Algorithms");
        } else if (uniByCity[player.city])
            // Otherwise only go to free university if our city has a university
            studying = await study(ns, false, "Study Computer Science");
        else
            return ns.print(
                `You have insufficient money (${formatMoney(player.money)} < --pay-for-studies-threshold ` +
        ...
        if (workedForInvite && serverReqHackingLevel) {
            ...
            if (player.skills.hacking > requirement) {
                ns.print(
                    `Current hacking level ${player.skills.hacking} seems to now meet the backdoor requirement ${requirement}. Spawning backdoor-all-servers.js...`,
                );
    ...
    if (
        ...
        if (
            stockValue > 0 &&
            player.money + stockValue >= requirement &&
            Date.now() - lastStockLiquidation > 60000
        ) {
            lastStockLiquidation = Date.now();
            ns.run(getFilePath("stockmaster.js"), 1, "--liquidate");
            workedForInvite = true;
        }
        return ns.print(
            `${reasonPrefix} you have insufficient money. Need: ${formatMoney(requirement)}, Have: ${formatMoney(player.money)}`,
        );
    }
    ...
    let travelToCityOrDidRecently = async (
        city, // Helper to consider us as having travelled for an invite if we did just now, or recently
    ) =>
        (player.city != city && (await goToCity(ns, city))) ||
        (player.city == city && Date.now() - lastTravel < 60000);
    ...
    if ("Silhouette" == factionName) {
        ns.print(
            `You must be a CO (e.g. CEO/CTO) of a company to earn an invite to "Silhouette". This may take a while!`,
        );
        let factionConfig = companySpecificConfigs.find(
            (f) => f.name == "Silhouette",
        ); // We set up Silhouette with a "company-specific-config" so that we can work for an invite like any megacorporation faction.
        let companyNames = preferredCompanyFactionOrder.map(
            (f) =>
                companySpecificConfigs.find((cf) => cf.name == f)
                    ?.companyName || f,
        );
        let favorByCompany = await getNsDataThroughFile(
            ns,
            dictCommand("ns.singularity.getCompanyFavor(o)"),
            "/Temp/getCompanyFavors.txt",
            companyNames,
        );
        let repByCompany = await getNsDataThroughFile(
            ns,
            dictCommand("ns.singularity.getCompanyRep(o)"),
            "/Temp/getCompanyReps.txt",
            companyNames,
        );
        ...
        factionConfig.companyName = companyNames.sort(
            (a, b) =>
                ((backdoorByServer[serverByCompany[a]] ? 0.75 : 1.0) * 3.2e6 -
                    repByCompany[a]) /
                    (100 + favorByCompany[a]) -
                ((backdoorByServer[serverByCompany[b]] ? 0.75 : 1.0) * 3.2e6 -
                    repByCompany[b]) /
                    (100 + favorByCompany[b]),
        )[0];
        // If the company we chose has a required stat modifier, we need to add it to the one for Silhouette
        factionConfig.statModifier =
            companySpecificConfigs.find(
                (c) => (c.companyName ?? c.name) == factionConfig.companyName,
            )?.statModifier || 0;
        ...
        serverByCompany["Silhouette"] =
            serverByCompany[factionConfig.companyName];
        ...
        workedForInvite = await workForMegacorpFactionInvite(
            ns,
            factionName,
            false,
        ); // Work until CTO and the external script joins this faction, triggering an exit condition.
    }

    // Special case: check hacknet stats before we try to join Netburners
    if ("Netburners" == factionName) {
        const [totalLevels, totalRam, totalCores] = await getNsDataThroughFile(
            ns,
...
async function goToCity(ns, cityName) {
    ...
    if (player.money < 200000)
        log(
            ns,
            `WARN: Insufficient funds to travel from ${player.city} to ${cityName}`,
            false,
            "warning",
        );
    else
        log(
            ns,
            `ERROR: Failed to travel from ${player.city} to ${cityName} for some reason...`,
            false,
            "error",
        );
    return false;
...
async function trainCombatStats(ns, reqStats) {
    const physicalStats = ["strength", "defense", "dexterity", "agility"];
    let player = await getPlayerInfo(ns);
    const focusArg = shouldFocus === undefined ? true : shouldFocus;
    let lastStatusUpdateTime = 0;
    while (physicalStats.some((s) => player.skills[s] < reqStats)) {
        if (breakToMainLoop()) return false;
        const gymCity = gymByCity[player.city] ? player.city : "Sector-12";
        if (!(await goToCity(ns, gymCity))) return false;
        const gym = gymByCity[gymCity];
        const targetStat = physicalStats.reduce(
            (min, stat) =>
                player.skills[stat] < player.skills[min] ? stat : min,
            physicalStats[0],
        );
        ...
        if (Date.now() - lastStatusUpdateTime > statusUpdateInterval) {
            ...
            ns.print(
                `Training ${targetStat} at ${gym} until all combat stats reach ${reqStats}. ` +
                    `Current: Str ${player.skills.strength}, Def ${player.skills.defense}, Dex ${player.skills.dexterity}, Agi ${player.skills.agility}`,
            );
        }
        await ns.sleep(loopSleepInterval);
        player = await getPlayerInfo(ns);
...
export async function crimeForKillsKarmaStats(
    ...
    const bestCrimesByDifficulty = [
        "Heist",
        "Assassination",
        "Homicide",
        "Mug",
    ]; // Will change crimes as our success rate improves
    ...
    doFastCrimesOnly =
        doFastCrimesOnly || (options ? options["fast-crimes-only"] : false);
    let player = await getPlayerInfo(ns);
    let strRequirements = [];
    let forever =
    ...
    if (reqKarma)
        strRequirements.push(
            () =>
                `-${reqKarma} Karma (Have ${Math.round(ns.heart.break()).toLocaleString("en")})`,
        );
    if (reqStats)
        strRequirements.push(
            () =>
                `${reqStats} of each combat stat (Have ` +
    ...
    let anyStatsDeficient = (p) =>
        p.skills.strength < reqStats ||
        p.skills.defense < reqStats ||
        /*                      */ p.skills.dexterity < reqStats ||
        p.skills.agility < reqStats;
    let crime, lastCrime, crimeTime, lastStatusUpdateTime, needStats;
    while (
        forever ||
        (needStats = anyStatsDeficient(player)) ||
        player.numPeopleKilled < reqKills ||
        -ns.heart.break() < reqKarma
    ) {
        if (!forever && breakToMainLoop())
            return ns.print(
                "INFO: Interrupting crime to check on high-level priorities.",
            );
        const crimeList = (
            ns.enums?.CrimeType
                ? Object.values(ns.enums.CrimeType)
        ...
        const crimeStats = await getNsDataThroughFile(
            ns,
            dictCommand("ns.singularity.getCrimeStats(o)"),
            "/Temp/crime-stats.txt",
            crimeList,
        );
        ...
        crime =
            crimeCount < 2
                ? crimeChances["Homicide"] > 0.75
                    ...
                    : "Mug"
                : !needStats &&
                    (player.numPeopleKilled < reqKills || karma < reqKarma)
                  ? "Homicide"
                  : bestCrimesByDifficulty.find((c, index) =>
                        doFastCrimesOnly && index <= 1
                            ? 0
                            : crimeChances[c] >= chanceThresholds[index],
                    );
        if (needStats && !doFastCrimesOnly) {
            const score = (c) => {
                const stats = crimeStats[c];
                if (!stats || !stats.time) return 0;
                const exp =
                    stats.strength_exp +
                    stats.defense_exp +
                    stats.dexterity_exp +
                    stats.agility_exp;
                return (crimeChances[c] || 0) * (exp / stats.time);
            };
            const bestCrime = crimeList.sort((a, b) => score(b) - score(a))[0];
            if (bestCrime) crime = bestCrime;
        }
        ...
        if (!lastCrime || !(crimeType && crimeType.includes(lastCrime))) {
            ...
            if (lastCrime) {
                log(
                    ns,
                    `Committing Crime "${lastCrime}" Interrupted. (Now: ${crimeType ?? currentWork.type}) Restarting...`,
                    false,
                    "warning",
                );
            ...
            crimeTime = await getNsDataThroughFile(
                ns,
        ...
        let sleepTime =
            1 + Math.ceil(loopSleepInterval / crimeTime) * crimeTime;
    ...
    ns.print(
        `Done committing crimes. Reached ${strRequirements.map((r) => r()).join(", ")}`,
    );
    return true;
}
...
async function study(ns, focus, course, university = null) {
    if (options["no-studying"]) {
        log(
            ns,
            `WARNING: Could not study '${course}' because --no-studying is set.`,
            false,
            "warning",
        );
        return;
    ...
    if (!university) {
        // Auto-detect the university in our city
        university = uniByCity[playerCity];
        if (!university) {
            log(
                ns,
                `WARNING: Could not study '${course}' because we are in city '${playerCity}' without a university.`,
                false,
                "warning",
            );
            return;
    ...
    if (
        ...
        log(
            ns,
            `Started studying '${course}' at '${university}'`,
            false,
            "success",
        );
        return true;
    ...
    log(
        ns,
        `ERROR: For some reason, failed to study '${course}' at university '${university}' (Not in correct city? Player is in '${playerCity}')`,
        false,
        "error",
    );
    return false;
...
async function monitorStudies(ns, stat, requirement) {
    ...
    let lastStatTime = null;
    const initialWork = await getCurrentWorkInfo(ns);
    while (!breakToMainLoop()) {
        const currentWork = await getCurrentWorkInfo(ns);
        if (
            !currentWork.classType ||
            currentWork.classType != initialWork.classType
        ) {
            log(
                ns,
                `WARNING: Something interrupted our studies.` +
                    `\nWAS: ${JSON.stringify(initialWork)}\nNOW: ${JSON.stringify(currentWork)}`,
                false,
                "warning",
            );
            return;
        ...
        if (player.skills[stat] >= requirement) {
            log(
                ns,
                `SUCCESS: Achieved ${stat} level ${player.skills[stat]} >= ${requirement} while studying`,
                false,
                "info",
            );
            return true;
        ...
        if (Date.now() - lastStatusUpdateTime > statusUpdateInterval) {
            lastStatusUpdateTime = Date.now();
            let eta = null;
            if (lastStatValue !== null && lastStatTime !== null) {
                const elapsed = (Date.now() - lastStatTime) / 1000;
                const delta = player.skills[stat] - lastStatValue;
                if (elapsed > 0 && delta > 0)
                    eta = formatDuration(
                        (1000 * (requirement - player.skills[stat])) /
                            (delta / elapsed),
                    );
            }
            lastStatValue = player.skills[stat];
            lastStatTime = Date.now();
            log(
                ns,
                `Studying "${currentWork.classType}" at ${currentWork.location} until ${stat} reaches ${requirement}. ` +
                    `Currently at ${player.skills[stat]}...${eta ? ` (ETA: ${eta})` : ""}`,
                false,
                "info",
            );
        }
...
export async function waitForFactionInvite(
    ns,
    factionName,
    maxWaitTime = waitForFactionInviteTime,
) {
    ns.print(
        `Waiting for invite from faction "${factionName}" (game may delay this up to ${formatDuration(maxWaitTime)})...`,
    );
    ...
    do {
        ...
        if (factionCount > lastFactionCount) {
            ns.print(
                `INFO: Recieved a new invite, but not from "${factionName}". ` +
        ...
        !invitations.includes(factionName) &&
        !joinedFactions.includes(factionName) &&
        (waitTime -= loopSleepInterval) > 0
    );
    if (joinedFactions.includes(factionName))
        // Another script may have auto-joined this faction before we could
        ns.print(
            `An external script has joined faction "${factionName}" for us.`,
        );
    else if (!invitations.includes(factionName)) {
        log(
            ns,
            `ERROR: Waited ${formatDuration(maxWaitTime)}, but still have not recieved an invite for faction: "${factionName}" (Requirements not met?)`,
            false,
            "error",
        );
        return;
        ...
        log(
            ns,
            `ERROR: Something went wrong. Earned "${factionName}" faction invite, but failed to join it.`,
            false,
            "error",
        );
        return;
...
async function getCurrentWorkInfo(ns) {
    return (
        (await getNsDataThroughFile(ns, "ns.singularity.getCurrentWork()")) ??
        {}
    ); // Easier than null-coalescing everywhere
}
...
async function getCurrentFactionFavour(ns, factionName) {
    return await getNsDataThroughFile(
        ns,
        `ns.singularity.getFactionFavor(ns.args[0])`,
        null,
        [factionName],
    );
}
...
async function daedalusSpecialCheck(ns, favorRepRequired, currentReputation) {
    if (favorRepRequired == 0 || currentReputation < favorRepRequired)
        return false;
    // If we would be unlocking donations, but actually, we're pretty close to just being able to afford TRP, no impetus to reset.
    if (currentReputation >= 0.9 * 2.5e6 * bitNodeMults.AugmentationRepCost)
        return false;
    log(
        ns,
        `INFO: You have enough reputation with Daedalus (have ${formatNumberShort(currentReputation)}) that you will ` +
            `unlock donations (needed ${formatNumberShort(favorRepRequired)}) with them on your next reset.`,
        !notifiedAboutDaedalus,
        "info",
    );
...
async function isValidInterruption(ns, currentWork = null) {
    ...
    // If bladeburner is currently active, but we do not yet have The Blade's Simulacrum, we may choose to we pause working.
        ...
        if (playerGang || scope >= 2 || [6, 7].includes(currentBitnode)) {
            ...
            if (playerInBladeburner) {
                ...
                if (currentWork.type) await stop(ns); // Stop working so bladeburner can run (bladeburner won't interrupt work for us)
            }
        }
    }
    ...
    if (interruptionNotice != "") {
        if (lastInterruptionNotice != interruptionNotice) {
            // If we haven't already notified that we're pausing activity, do this now
...
export async function workForSingleFaction(
    ...
    let favorRepRequired = Math.max(
        0,
        repToFavour(favorToDonate) - repToFavour(startingFavor),
    );
    ...
    if (forceUnlockDonations)
        // If forced, ensure we earn enough reputation to unlock donations on our next reset
        factionRepRequired = Math.max(factionRepRequired, favorRepRequired);
    if (forceBestAug)
        // If forced, ensure we earn enough rep to buy the highest rep augmentation
        factionRepRequired = Math.max(factionRepRequired, highestRepAug);
    if (forceRep !== true && forceRep > 0)
        // If forceRep is a number (not just a flag 'true'), ensure we earn the specified rep amount
        factionRepRequired = Math.max(factionRepRequired, forceRep);
    // Check for any reasons to skip working for this faction
    if (
        !forceRep &&
        highestRepAug == -1 &&
        !firstFactions.includes(factionName) &&
        !options["get-invited-to-every-faction"]
    )
        return ns.print(
    ...
    if (playerGang == factionName)
        // Cannot work for your own gang faction.
        return ns.print(
            `"${factionName}" is your gang faction. You can only earn rep in your gang via respect.`,
        );
    ...
    if (
        ...
        ns.print(
            `The last "${factionName}" aug is only ${mostExpensiveAugByFaction[factionName].toLocaleString("en")} rep, ` +
    ...
    if (factionName == "Daedalus")
        await daedalusSpecialCheck(ns, favorRepRequired, currentReputation);
    if (currentReputation >= factionRepRequired)
        return ns.print(
            `Faction "${factionName}" required rep of ${Math.round(factionRepRequired).toLocaleString("en")} has already been attained ` +
                `(Current rep: ${Math.round(currentReputation).toLocaleString("en")}). Skipping working for faction...`,
        );

    ns.print(
        `Faction "${factionName}" Highest Aug Req: ${highestRepAug?.toLocaleString("en")}, Current Favor (` +
    ...
    while (
        ...
        if (breakToMainLoop())
            return ns.print(
                "INFO: Interrupting faction work to check on high-level priorities.",
            );
        const currentWork = await getCurrentWorkInfo(ns);
        ...
        if (workAssigned && currentWork.factionName != factionName) {
            ...
            log(
                ns,
                `Work for faction ${factionName} was interrupted (Now: ${JSON.stringify(currentWork)}). Restarting...`,
                false,
                "warning",
            );
            workAssigned = false;
        ...
        if (
            currentWork.factionName == factionName &&
            factionJob != bestFactionJob
        ) {
            log(
                ns,
                `INFO: Detected that "${bestFactionJob}" gives more rep than previous work "${factionJob}". Switching...`,
            );
            workAssigned = false;
        }
        ...
        if (!workAssigned) {
            if (
                ...
                log(
                    ns,
                    `ERROR: Something went wrong, failed to start "${bestFactionJob}" work for faction "${factionName}" (Is gang faction, or not joined?)`,
                    false,
                    "error",
                );
                break;
        ...
        if (
            ...
            const repGainRate = await measureFactionRepGainRate(
                ns,
                factionName,
            );
            const eta_milliseconds =
                (1000 * (factionRepRequired - currentReputation)) / repGainRate;
            ns.print(
                `${status} Currently at ${Math.round(currentReputation).toLocaleString("en")}, ` +
                    `earning ${formatNumberShort(repGainRate)} rep/sec. ` +
                    (hasFocusPenalty && !shouldFocus
        ...
        if (!forceBestAug && !forceRep) {
            ...
            if (currentFavor === undefined)
                log(
                    ns,
                    `ERROR: WTF... getCurrentFactionFavour returned 'undefined' for factionName: ${factionName}`,
                    true,
                    "error",
                );
    ...
    if (currentReputation >= factionRepRequired)
        ns.print(
            `Attained ${Math.round(currentReputation).toLocaleString("en")} rep with "${factionName}" ` +
...
async function stop(ns) {
    return await getNsDataThroughFile(ns, `ns.singularity.stopAction()`);
}
...
async function detectBestFactionWork(ns, factionName) {
    ...
    for (const work of Object.values(ns.enums.FactionWorkType)) {
        ...
        const currentRepGainRate = await measureFactionRepGainRate(
            ns,
            factionName,
        );
    ...
    if (bestWork === undefined) {
        ...
        throw Error(
            `The faction "${factionName}" does not support any of the known work types. Cannot work for this faction!`,
        );
    }
    return bestWork;
}
...
export async function workForAllMegacorps(
    ...
    if (player.skills.hacking < 225)
        return ns.print(
            `Hacking Skill ${player.skills.hacking} is to low to work for any megacorps (min req. 225).`,
        );
    ...
    for (const factionName of megacorpFactionsInPreferredOrder) {
        const earnedInvite = await workForMegacorpFactionInvite(
            ns,
            factionName,
            doFactionWork,
        );
    ...
    if (alsoWorkForCompanyFactions && !oneCompanyFactionAtATime) {
        ...
        if (earnedAnyInvite)
            // Avoid log noise by only logging this when a new invite was earned
            ns.print(
...
async function trySpendHashes(ns, spendOn) {
    return await getNsDataThroughFile(
        ...
        "/Temp/hacknet-spendHashes-returnSpent.txt",
        [spendOn],
    );
}
...
export async function tryBuyReputation(ns) {
    ...
    if ((await getPlayerInfo(ns)).money > 100e9) {
        // If we're wealthy, hashes have relatively little monetary value, spend hacknet-node hashes on contracts to gain rep faster
...
async function checkForBackdoor(ns, companyName) {
    if (backdoorCache.has(companyName)) return true;
    const backdoored = await getNsDataThroughFile(
        ns,
        `ns.getServer(ns.args[0]).backdoorInstalled`,
        null,
        [serverByCompany[companyName]],
    );
    if (backdoored) backdoorCache.add(companyName);
    return backdoored;
}
...
async function backdoorStatusByServer(ns) {
    return await getNsDataThroughFile(
        ns,
        `Object.fromEntries(ns.args.map(s => [s, ns.getServer(s).backdoorInstalled]))`,
        "/Temp/getServer-backdoorInstalled-all.txt",
        Object.values(serverByCompany),
    );
}
...
export async function workForMegacorpFactionInvite(
    ...
    const companyConfig = companySpecificConfigs.find(
        (c) => c.name == factionName,
    ...
    if ((await checkFactionInvites(ns)).includes(factionName))
        return waitForInvite
            ? await waitForFactionInvite(ns, factionName)
            : false;
    const itJob = jobs.find((j) => j.name == "IT");
    const softwareJob = jobs.find((j) => j.name == "Software");
    if (itJob.reqHck[0] + statModifier > player.skills.hacking)
        // We don't qualify to work for this company yet if we can't meet IT qualifications (lowest there are)
        return ns.print(
            `Cannot yet work for "${companyName}": Need Hack ${itJob.reqHck[0] + statModifier} to get hired (current Hack: ${player.skills.hacking});`,
        );
    ns.print(`Going to work for Company "${companyName}" next...`);
    let currentReputation,
        currentRole = "",
        currentJobTier = -1;
    const currentJobName = player.jobs[companyName];
    if (currentJobName) {
        const lower = currentJobName.toLowerCase();
        if (lower.includes("software")) currentRole = "Software";
        else if (lower.includes("it")) currentRole = "IT";
    }
    let lastStatus = "",
    ...
    let repRequiredForFaction =
        (companyConfig?.repRequiredForFaction || 400_000) -
        (backdoored ? 100_000 : 0);
    while (
        (currentReputation = await getCompanyReputation(ns, companyName)) <
            repRequiredForFaction &&
        !player.factions.includes(factionName)
    ) {
        if (breakToMainLoop())
            return ns.print(
                "INFO: Interrupting corporation work to check on high-level priorities.",
            );
        // Determine the next promotion we're striving for (the sooner we get promoted, the faster we can earn company rep)
        const getTier = (job) =>
            Math.min(
                // Check all requirements for all job (taking into account modifiers) and find the minimum we meet
                job.reqRep.filter(
                    (r) => r * (backdoored ? 0.75 : 1) <= currentReputation,
                ).length,
                job.reqHck.filter(
                    (h) =>
                        (h === 0 ? 0 : h + statModifier) <=
                        player.skills.hacking,
                ).length,
                job.reqCha.filter(
                    (c) =>
                        (c === 0 ? 0 : c + statModifier) <=
        ...
        if (currentJobTier < 0 && currentRole) {
            const roleTier =
                currentRole == "IT" ? qualifyingItTier : qualifyingSoftwareTier;
            currentJobTier = roleTier;
        }
        ...
        const requiredRep =
            nextJob.reqRep[nextJobTier] * (backdoored ? 0.75 : 1); // Rep requirement is decreased when company server is backdoored
        const requiredHack =
            nextJob.reqHck[nextJobTier] === 0
                ? 0
                : nextJob.reqHck[nextJobTier] + statModifier; // Stat modifier only applies to non-zero reqs
        const requiredCha =
            nextJob.reqCha[nextJobTier] === 0
                ? 0
                : nextJob.reqCha[nextJobTier] + statModifier; // Stat modifier only applies to non-zero reqs
        let status =
            `Next promotion ('${nextJobName}' #${nextJobTier}) at Hack:${requiredHack} Cha:${requiredCha} Rep:${requiredRep?.toLocaleString("en")}` +
            (repRequiredForFaction > requiredRep
                ? ""
                : `, but we won't need it, because we'll sooner hit ${repRequiredForFaction.toLocaleString("en")} reputation to unlock company faction "${factionName}"!`);
        if (nextJobTier >= nextJob.reqHck.length)
            // Special case status message if we're at the maximum promotion, but need additional reputation to unlock the company
        ...
        if (
            ...
            player.skills.hacking >= requiredHack &&
            player.skills.charisma < requiredCha &&
            !options["no-studying"]
        ) {
            // Check whether we can train stats in a "reasonable amount of time"
            const em =
                requiredCha / options["training-stat-per-multi-threshold"];
            const chaHeuristic = classHeuristic(player, "charisma");
            if (chaHeuristic < em) {
                if (!decidedNotToStudy)
                    // Only generate the log below once
                    log(
                        ns,
                        `INFO: You are only lacking in Charisma to get our next promotion. Need: ${requiredCha}, Have: ${player.skills.charisma}` +
            ...
            } // On any loop, we can change our mind and decide studying is worthwhile
            else decidedNotToStudy = false;
            if (!decidedNotToStudy) {
                status =
                    `Studying at ZB university until Cha reaches ${requiredCha}...\n` +
                    status;
                if (requiredCha - player.skills.charisma > 10) {
                    let spentHashes = await trySpendHashes(
                        ns,
                        "Improve Studying",
                    );
                    if (spentHashes > 0) {
                        log(
                            ns,
                            'Bought a "Improve Studying" upgrade.',
                            false,
                            "success",
                        );
                    }
                ...
                if (await studyForCharisma(ns, shouldFocus)) {
                    ...
                    const studyResult = await monitorStudies(
                        ns,
                        "charisma",
                        requiredCha,
                    );
        ...
        if (!backdoored) {
            // Don't need to check again once we've confirmed a backdoor.
        ...
        if (await tryApplyToCompany(ns, companyName, currentRole)) {
            ...
            log(
                ns,
                `Unexpected '${currentRole}' promotion from ${currentJob} to "${player.jobs[companyName]}. Promotion logic must be off..."`,
                false,
                "warning",
            );
        }
        ...
        if (
            !isStudying &&
            (!isWorking || currentWork.companyName != companyName)
        ) {
            if (isWorking) {
                // Log a warning if we discovered that work we previously began was disrupted
                if (await isValidInterruption(ns, currentWork)) return false;
                log(
                    ns,
                    `Work for company ${companyName} was interrupted (Now: ${JSON.stringify(currentWork)}). Restarting...`,
                    false,
                    "warning",
                );
                isWorking = false;
        ...
        if (
            ...
            const repGainRate = !isWorking
                ? 0
                : await measureCompanyRepGainRate(ns, companyName);
            const eta = !isWorking
                ? "?"
                : formatDuration(
                      (1000 *
                          ((requiredRep || repRequiredForFaction) -
                              currentReputation)) /
                          repGainRate,
                  );
            player = await getPlayerInfo(ns);
            ns.print(
                `Currently a "${player.jobs[companyName]}" ('${currentRole}' #${currentJobTier}) for "${companyName}" earning ${formatNumberShort(repGainRate)} rep/sec. ` +
                    (hasFocusPenalty && !shouldFocus
                        ? `(after 20% non-focus Penalty)`
                        : "") +
                    `\n` +
    ...
    if (currentReputation >= repRequiredForFaction) {
        ns.print(
            `Attained ${repRequiredForFaction.toLocaleString("en")} rep with "${companyName}".`,
        );
    ...
    ns.print(
        `Stopped working for "${companyName}" repRequiredForFaction: ${repRequiredForFaction.toLocaleString("en")} ` +
