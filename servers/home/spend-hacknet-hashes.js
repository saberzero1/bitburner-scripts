...
const parameterizedSpendOptions = [
...
export async function main(ns) {
    ...
    if (toBuy.length == 0)
        return log(
            ns,
            "ERROR: You must specify at least one thing to spend hashes on via the --spend-on argument.",
            true,
            "error",
        );
    ...
    if (unrecognized.length > 0)
        return log(
            ns,
            `ERROR: One or more --spend-on arguments are not recognized: ${unrecognized.join(", ")}`,
            true,
            "error",
        );
    // Operate in "low-priority" mode if our only job is to sell for money when nearing our hash capacity
    const lowPriority =
        !liquidate && toBuy.length == 1 && toBuy[0] == sellForMoney;

    disableLogs(ns, ["sleep", "getServerMoneyAvailable"]);
    ns.print(
        `Starting spend-hacknet-hashes.js... Will check in every ${formatDuration(interval)}`,
    );
    ns.print(
        liquidate
            ? `-l --liquidate mode active! Will spend all hashes as soon as possible.`
    ...
    const getMinCost = (spendActions) =>
        Math.min(...spendActions.map((p) => ns.hacknet.hashCost(p)));
    // Helper to format hashes in log message
    const formatHashes = (hashes) => formatNumberShort(hashes, 6, 3);
    while (true) {
        await ns.sleep(interval);
        if (lowPriority && ns.hacknet.numHashes() > 0)
            // Low priority mode means any competing scripts should get to spend hashes first.
        ...
        try {
            ...
            if (lastHashBalance != capacity && lastHashBalance == currentHashes)
                continue;
            ...
            if (nodes == 0) {
                ...
                return log(
                    ns,
                    "INFO: You have hacknet nodes, not hacknet servers, so spending hashes is not applicable.",
                );
            // Helper function to get total hash production across all nodes
            let globalProduction = Array.from({ length: nodes }, (_, i) =>
                ns.hacknet.getNodeStats(i),
            ).reduce((total, node) => total + node.production, 0);
            const hashesEarnedNextTick =
                (globalProduction * interval) / 1000 +
                options["reserve-buffer"]; // If we are this far from our capacity, start spending
            ...
            const fnSpendHashes = async (purchases, spendAllHashes) => {
                ...
                let maxHashSpend = () =>
                    ...
                    (spendAllHashes
                        ? 0
                        : Math.max(0, capacity - hashesEarnedNextTick));
                ...
                while (
                    ...
                    purchasesThisLoop < maxPurchasesPerLoop &&
                    getMinCost(purchases) <= maxHashSpend()
                ) {
                    ...
                    for (const spendAction of purchases) {
                        ...
                        const serverTarget = parameterizedSpendOptions.includes(
                            spendAction,
                        )
                            ? spendOnServer
                            : undefined;
                        ...
                        if (quantity > 1 && spendAction != sellForMoney) {
                            ...
                            while (attempt > 1 && !purchaseSucceeded) {
                                purchaseSucceeded = ns.hacknet.spendHashes(
                                    spendAction,
                                    serverTarget,
                                    attempt,
                                );
                        ...
                        if (!lastPurchaseSucceeded) {
                            // Note: Even if we had enough hashes, we may fail if another script spends them first
                            log(
                                ns,
                                `WARN: Failed to spend hashes on ${quantity}x '${spendAction}'. Cost was: ${formatHashes(totalCost)} of ${formatHashes(budget)} ` +
                        ...
                        if (purchasesThisLoop < 10) {
                            // If we purchase more than 10 things, don't even bother logging each one, it'll slow us down
                            log(
                                ns,
                                `SUCCESS: ${purchasesThisLoop == 1 ? "" : `(${purchasesThisLoop}) `}Spent ${formatHashes(totalCost)} hashes on ` +
                                    `${quantity}x '${spendAction}'. Next upgrade will cost ${formatHashes(ns.hacknet.hashCost(spendAction))}.`,
                                false,
                                "success",
                            );
                        }
                        if (purchasesThisLoop % 100 == 0) await ns.sleep(1); // Periodically yield to the game briefly if we're making many purchases at once.
                    }
                }
                if (purchasesThisLoop > 10)
                    log(
                        ns,
                        `SUCCESS: Made ${purchasesThisLoop} purchases this loop (but silenced logs to speed things up)`,
                    );
                if (ns.hacknet.numHashes() < startingHashes)
                    log(
                        ns,
                        `INFO: Summary: Spent ${formatHashes(startingHashes - ns.hacknet.numHashes())} hashes on ${purchasesThisLoop} purchases ` +
                            (spendAllHashes
                                ? ""
                                : `to avoid reaching capacity (${formatHashes(capacity)}) `) +
                            `while earning ${formatHashes(globalProduction)} hashes per second.`,
                    );
            };
            ...
            if (getMinCost(toBuy) > capacity - options["reserve-buffer"])
                capacityMessage =
                    `Our hash capacity is ${formatHashes(capacity)}, but the cheapest upgrade we wish to purchase ` +
                ...
                capacityMessage =
                    `We're still at or near our hash capacity (${formatHashes(capacity)}) after spending hashes as instructed. ` +
            ...
            else continue; // Current hash capacity suffices, go back to sleep
            ...
            const warnToast =
                remaining < hashesEarnedNextTick ? "warning" : undefined;
            if (options["no-capacity-upgrades"]) {
                // If we aren't allowed to purchase capacity upgrades by configuration, warn the user so they can intervene
                log(
                    ns,
                    `WARNING: Upgrade your hacknet cache! spend-hacknet-hashes.js --no-capacity-upgrades is set, ` +
                        `so we cannot increase our hash capacity. ${capacityMessage}`,
                    false,
                    warnToast,
                );
            } else {
                // Otherwise, try to upgrade hacknet capacity so we can save up for more upgrades
                if (!notifiedMaxCapacity)
                    // Log that we want to increase hash capacity (unless we've previously seen that we are maxed out)
                ...
                for (let i = 0; i < nodes; i++)
                    if (ns.hacknet.getNodeStats(i).hashCapacity < lowestLevel)
                        ((lowestIndex = i),
                            (lowestLevel =
                                ns.hacknet.getNodeStats(i).hashCapacity));
                const nextCacheUpgradeCost =
                    lowestIndex == null
                        ? Number.POSITIVE_INFINITY
                        : ns.hacknet.getCacheUpgradeCost(lowestIndex, 1);
                const nextNodeCost = ns.hacknet.getPurchaseNodeCost();
                const reservedMoney =
                    options["reserve"] ?? Number(ns.read("reserve.txt") || 0);
                const playerMoney = ns.getServerMoneyAvailable("home");
                const spendableMoney = Math.max(
                    0,
                    playerMoney - reservedMoney,
                    // Hack: Because managing global reserve is tricky. We tend to always want to purchase cheap upgrades
                    playerMoney *
                        options["ignore-reserve-if-upgrade-cost-less-than-pct"],
                );
                // If it's cheaper to buy a new hacknet node than to upgrade the cache of an existing one, do so
                if (
                    nextNodeCost < nextCacheUpgradeCost &&
                    nextNodeCost < spendableMoney
                ) {
                    if (ns.hacknet.purchaseNode())
                        log(
                            ns,
                            `SUCCESS: spend-hacknet-hashes.js spent ${formatMoney(nextNodeCost)} to purchase a new hacknet node ${nodes + 1} ` +
                                `in order to increase hash capacity and afford further purchases (${toBuy.join(", ")}). (You can disable this with --no-capacity-upgrades)`,
                            false,
                            "success",
                        );
                    else
                        log(
                            ns,
                            `WARNING: spend-hacknet-hashes.js attempted to spend ${formatMoney(nextNodeCost)} to purchase hacknet node ${nodes + 1}, ` +
                                `but the purchase failed for an unknown reason (despite appearing to have ${formatMoney(spendableMoney)} to spend after reserves.)`,
                            false,
                            "warning",
                        );
                    ...
                    lowestIndex !== null &&
                    nextCacheUpgradeCost < spendableMoney
                ) {
                    if (ns.hacknet.upgradeCache(lowestIndex, 1))
                        log(
                            ns,
                            `SUCCESS: spend-hacknet-hashes.js spent ${formatMoney(nextCacheUpgradeCost)} to upgrade hacknet node ${lowestIndex} hash capacity ` +
                                `in order to afford further purchases (${toBuy.join(", ")}). (You can disable this with --no-capacity-upgrades)`,
                            false,
                            "success",
                        );
                    else
                        log(
                            ns,
                            `WARNING: spend-hacknet-hashes.js attempted to spend ${formatMoney(nextCacheUpgradeCost)} to upgrade hacknet node ${lowestIndex} hash capacity, ` +
                                `but the purchase failed for an unknown reason (despite appearing to have ${formatMoney(spendableMoney)} to spend after reserves.)`,
                            false,
                            "warning",
                        );
                    ...
                    const nextCheapestCacheIncreaseCost = Math.min(
                        nextCacheUpgradeCost,
                        nextNodeCost,
                    );
                    const nextCheapestCacheIncrease =
                        nextNodeCost < nextCacheUpgradeCost
                            ? `buy hacknet node ${nodes + 1}`
                            : `upgrade hacknet node ${lowestIndex} hash capacity`;
                    if (!Number.isFinite(nextCheapestCacheIncreaseCost))
                        ...
                        message +=
                            ` We cannot afford to increase our hash capacity (${formatMoney(nextCheapestCacheIncreaseCost)} to ${nextCheapestCacheIncrease}).` +
                            (playerMoney < nextCheapestCacheIncreaseCost
                                ? "" // Don't bother mentioning budget if the cost exceeds all player money
                                : `on our budget of ${formatMoney(spendableMoney)}` +
                                  (reservedMoney > 0
                                      ? ` (after respecting reserve of ${formatMoney(reservedMoney)}).`
                                      : "."));
                    ...
                    if (nextPurchaseCost > capacity)
                        message +=
                            ` We have insufficient hashes to buy any of the desired upgrades (${toBuy.join(", ")}) at our current hash capacity. ` +
                    ...
                    if (Number.isFinite(nextCheapestCacheIncreaseCost)) {
                        if (playerMoney > nextCheapestCacheIncreaseCost)
                            message +=
                                " Feel free to manually purchase this upgrade (despite the reserve/budget) if you deem it worthwhile.";
                        log(
                            ns,
                            `WARNING: spend-hacknet-hashes.js ${message}`,
                            false,
                            warnToast,
                        );
                    } else if (nextPurchaseCost > capacity)
                        // If we can't afford anything, and have maxed our hash capacity, we may as well shut down.
                        return log(
                            ns,
                            `SUCCESS: We've maxed all purchases. ${message}`,
                        );
                    // Shut down, because we will never be able to buy anything further.
                    else if (!notifiedMaxCapacity) {
                        // The first time we discover we are at max hash capacity (infinite cost) notify the user
                        log(
                            ns,
                            `INFO: spend-hacknet-hashes.js ${message}`,
                            true,
                            "info",
                        ); // Only inform the user of this the first time it happens.
            ...
            log(
                ns,
                `WARNING: spend-hacknet-hashes.js Caught (and suppressed) an unexpected error in the main loop:\n` +
                    getErrorInfo(err),
                false,
                "warning",
            );
