...
export async function main(ns) {
    ...
    const escaped = escapeChars.some(
        (c) => firstArg.startsWith(c) && firstArg.endsWith(c),
    );
    ...
    if (command.includes(";")) {
        ...
        if (
            !command
                .slice(lastStatement + 1)
                .trim()
                .startsWith("return")
        )
            command =
                command.slice(0, lastStatement + 1) +
                `return ` +
                command.slice(lastStatement + 1);
    ...
    command =
        `ns.tprint(JSON.stringify(await (async () => ${command})() ?? "(no output)", jsonReplacer, 2)` +
    ...
    return await runCommand(
        ...
        escaped ? args.slice(1) : undefined,
        !silent,
    );
}
