...
export async function main(ns) {
    ...
    if (resetInfo.currentNode == 8)
        return log(
            ns,
            "ERROR: Bladeburner is completely disabled in Bitnode 8 :`(\nHappy stonking",
            true,
            "error",
        );
    ...
    while (true) {
        ...
        await ns.sleep(
            Math.min(
                ...
                nextTaskComplete > 0 ? nextTaskComplete : Number.MAX_VALUE,
            ),
        );
    }
}
...
async function gatherBladeburnerInfo(ns) {
    ...
    log(
        ...
        `There are ${remainingBlackOpsNames.length} remaining BlackOps operations to complete in order:\n` +
            remainingBlackOpsNames
                .map((n) => `${n} (${blackOpsRanks[n]})`)
                .join(", "),
    );
    maxRankNeeded =
        blackOpsRanks[
            remainingBlackOpsNames[remainingBlackOpsNames.length - 1]
        ];
...
async function mainLoop(ns) {
    ...
    remainingBlackOpsNames = remainingBlackOpsNames.filter(
        (n) => blackOpsToBeDone[n] === 1,
    );
    const nextBlackOp =
        remainingBlackOpsNames.length === 0 ? null : remainingBlackOpsNames[0];
    ...
    const contractCounts = await getBBDictByActionType(
        ns,
        "getActionCountRemaining",
        "Contracts",
        contractNames,
    );
    const operationCounts = await getBBDictByActionType(
        ns,
        "getActionCountRemaining",
        "Operations",
        operationNames,
    ...
    const reservedActions = ["Raid", "Stealth Retirement Operation"].concat(
        populationActions
            ...
            .filter(
                (a) =>
                    getCount(a) <=
                    options["reserved-action-count"] *
                        (1 - rank / (0.99 * maxRankNeeded)),
            ),
    );
    if (nextBlackOp && rank < blackOpsRanks[nextBlackOp])
        reservedActions.push(nextBlackOp); // Remove blackop from "available actions" if we have insufficient rank.
    const unreservedActions = limitedActions.filter(
        (o) => !reservedActions.includes(o),
    );
    ...
    const populationByCity = await getBBDict(
        ns,
        "getCityEstimatedPopulation(%)",
        cityNames,
    );
    const communitiesByCity = await getBBDict(
        ns,
        "getCityCommunities(%)",
        cityNames,
    );
    ...
    if (
        getCount("Raid") > 0 &&
        unreservedActions.every((c) => getCount(c) == 0)
    ) {
        const raidableCities = cityNames.filter(
            (c) => communitiesByCity[c] > 0,
        ); // Cities with at least one community
        ...
        goingRaiding =
            ...
            (raidableCities[0] != highestPopCity ||
                options["allow-raiding-highest-pop-city"]);
        if (goingRaiding) {
            // Select the raid-able city with the smallest population
            [goToCity, population] = getMinKeyValue(
                populationByCity,
                raidableCities,
            );
    ...
    if (!goToCity && unreservedActions.every((c) => getCount(c) == 0)) {
        ...
        if (
            getCount("Stealth Retirement Operation") &&
            maxChaos > options["chaos-recovery-threshold"]
        ) {
    ...
    if (!goToCity) {
        ...
        let acceptableCities = cityNames.filter(
            (city) => chaosByCity[city] <= options["chaos-recovery-threshold"],
        );
    ...
    if (
        ...
        (await switchToCity(ns, goToCity, travelReason))
    )
        currentCity = goToCity;
    ...
    const actionSummaryString = (action) =>
        ...
        (maxChance(action) - minChance(action) < 0.001
            ? ""
            : ` to ${(100 * maxChance(action)).toFixed(1)}%`) +
    ...
    lowStaminaTriggered =
        staminaPct < options["low-stamina-pct"] ||
        (lowStaminaTriggered && staminaPct < options["high-stamina-pct"]);
    // If we are suffering a stamina penalty, perform an action that consumes no stamina
    if (lowStaminaTriggered) {
        bestActionName =
            chaosByCity[currentCity] > options["max-chaos"]
                ? "Diplomacy"
                : "Field Analysis";
        ...
        bestActionName =
            getCount(antiChaosOperation) > 0 &&
            minChance(antiChaosOperation) > 0.8
                ? antiChaosOperation
                : "Diplomacy";
        ...
        let populationUncertain = candidateActions.some(
            (a) =>
                maxChance(a) > options["success-threshold"] &&
                minChance(a) < options["success-threshold"],
        );
        ...
        if (populationUncertain) {
            const operationPopulationActions = populationActions.filter(
                (actionName) => operationNames.includes(actionName),
            );
            const contractPopulationActions = populationActions.filter(
                (actionName) => contractNames.includes(actionName),
            );
            const generalPopulationActions = populationActions.filter(
                (actionName) => generalActionNames.includes(actionName),
            );
            const populationActionTimes = {
                ...
                ...(generalPopulationActions.length > 0
                    ? await getBBDictByActionType(
                          ...
                          "getActionTime",
                          "General",
                          generalPopulationActions,
                      )
                    : {}),
            };
            const getEffectiveness = (actionName) =>
                (minChance(actionName) + maxChance(actionName)) /
                2 /
                (populationActionTimes[actionName] || 1);
            populationActionsByEffectiveness = populationActions
                .slice()
                .sort((a, b) => getEffectiveness(b) - getEffectiveness(a));
        }
        candidateActions = populationUncertain
            ? populationActionsByEffectiveness
            : unreservedActions;
        ...
        if (!bestActionName) {
            if (populationUncertain) {
                ...
                let [maxChaosCity, maxChaos] = getMaxKeyValue(
                    chaosByCity,
                    cityNames,
                );
                reason =
                ...
                unreservedActions.some(
                    (a) => maxChance(a) < options["success-threshold"],
                ) && // Only if we aren't at 100% chance for everything
    ...
    if (
        currentAction?.name ==
        remainingBlackOpsNames[remainingBlackOpsNames - 1]
    )
    ...
    if (
        lastAssignedTask &&
        lastAssignedTask != currentAction?.name &&
        getCount(lastAssignedTask) > 0
    ) {
        log(
            ns,
        ...
        nextBlackOp == bestActionName
            ...
            : contractNames.includes(bestActionName)
              ...
              : operationNames.includes(bestActionName)
                ? "Operations"
                : "General";
    const success = await getBBInfo(
        ns,
        `startAction(ns.args[0], ns.args[1])`,
        bestActionType,
        bestActionName,
    );
...
async function switchToCity(ns, city, reason) {
    ...
    log(
        ns,
        (success ? "INFO: Switched" : "ERROR: Failed to switch") +
            ` to Bladeburner city "${city}" (${reason})`,
        !success,
...
async function spendSkillPoints(ns) {
    while (true) {
        ...
        const skillCosts = await getBBDict(
            ns,
            "getSkillUpgradeCost(%)",
            skillNames,
        );
        ...
        for (const skillName of skillNames) {
            ...
            let percievedCost =
                (skillCosts[skillName] ?? Number.POSITIVE_INFINITY) *
                (costAdjustments[skillName] || 1);
            // Bitburner pre-2.6.0 workaround: Overclock is capped at lvl 90, but the cost makes it seem upgradable
            if (skillName === "Overclock" && skillLevels[skillName] == 90)
                percievedCost = Number.POSITIVE_INFINITY;
        ...
        if (minPercievedCost > unspent || skillCosts[skillToUpgrade] > unspent)
            return;
...
async function tryJoinFaction(ns, rank) {
    ...
    if (rank >= 25 && (await getBBInfo(ns, "joinBladeburnerFaction()"))) {
        ...
        log(
            ns,
            `WARNING: Failed to join the Bladeburner faction despite rank of ${rank.toFixed(1)}`,
            false,
            "warning",
        );
}
...
async function beingInBladeburner(ns) {
    ...
    if (options["disable-spending-hashes"])
        return log(
            ns,
            `INFO: Not spending hashes on bladeburner (--disable-spending-hashes flag is set)`,
        );
    const fPath = getFilePath("spend-hacknet-hashes.js");
    const args = [
        "--spend-on",
        "Exchange_for_Bladeburner_Rank",
        "--spend-on",
        "Exchange_for_Bladeburner_SP",
        "--liquidate",
    ];
