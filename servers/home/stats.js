...
export async function main(ns) {
    ...
    while (true) {
        try {
            const hudData = await getHudData(
                ns,
                bitNode,
                dictSourceFiles,
                options,
...
function prepareHudElements(hudData) {
    ...
    for (const [header, _, value, toolTip] of hudData) {
        ...
        hook0.appendChild(
            newline(id + "-title", header.padEnd(9, " "), toolTip),
        );
...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
            ...
            if (hashes[1] > 0) {
                ...
                if (liquidatingHashes.length > 0)
                    val2.push(
                        true,
                        "Liquidating",
                        `You have ${liquidatingHashes.length} script` +
                            (liquidatingHashes.length > 1
                                ? "s running that are"
                                : " running that is") +
                            ...
                            liquidatingHashes
                                .map(
                                    (s) =>
                                        `${s.filename} ${s.args.join(" ")} (PID: ${s.pid})`,
                                )
                                .join("\n"),
                    );
            }
    ...
    // Show Bladeburner Rank and Skill Points
    {
        ...
        if (
            ...
            const bbRank = await getNsDataThroughFile(
                ns,
        ...
        if (!options["hide-RAM-utilization"]) {
            ...
            val1.push(
                ...
                `The number of servers on the network (${servers.length}) / ` +
                    ...
                    (hnServers.length > 0
                        ? `(${nPurchased - hnServers.length} servers + ${hnServers.length} hacknet servers)`
                        : `(${nPurchased})`),
            );
