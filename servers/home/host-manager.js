...
const argsSchema = [
    ...
    ["utilization-trigger", 0.8], // the percentage utilization that will trigger an attempted purchase
...
export async function main(ns) {
    ...
    maxPurchasedServers = await getNsDataThroughFile(
        ns,
        "ns.cloud.getServerLimit()",
    );
    log(
        ns,
        `INFO: Max purchasable servers has been detected as ${maxPurchasedServers.toFixed(0)}.`,
    );
    if (maxPurchasedServers == 0)
        return log(
            ns,
            `INFO: Shutting down due to host purchasing being disabled in this BN...`,
        );

    // Get the maximum size of purchased servers in this bitnode
    const purchasedServerMaxRam = await getNsDataThroughFile(
        ns,
    ...
    log(
        ns,
        `INFO: Max purchasable RAM has been detected as 2^${maxPurchasableServerRamExponent} (${formatRam(2 ** maxPurchasableServerRamExponent)}).`,
    );
    ...
    costByRamExponent = await getNsDataThroughFile(
        ns,
    ...
    if (minRamExponent > maxPurchasableServerRamExponent) {
        log(
            ns,
            `WARN: --min-ram-exponent was set to ${minRamExponent} (${formatRam(2 ** minRamExponent)}), ` +
        ...
        log(
            ns,
            `INFO: --min-ram-exponent is set to ${minRamExponent}: New servers will only be purchased ` +
                `if we can afford 2^${minRamExponent} (${formatRam(2 ** minRamExponent)}) or more in size.`,
        );
    log(
        ns,
        `INFO: --compare-to-home-threshold is set to ${options["compare-to-home-threshold"] * 100}%: ` +
            `New servers are deemed "not worthwhile" unless they are at least this big compared to your home server.`,
    );
    log(
        ns,
        `INFO: --compare-to-network-ram-threshold is set to ${options["compare-to-network-ram-threshold"] * 100}%: ` +
            `New servers are deemed "not worthwhile" unless they are this big compared to total ram on the entire network.`,
    );
    log(
        ns,
        `INFO: --utilization-trigger is set to ${options["utilization-trigger"] * 100}%: ` +
    ...
    if (options["reserve-by-time"])
        log(
            ns,
            `INFO: --reserve-by-time is active! This community-contributed option will spend more of your money on servers early on, and less later on.`,
        );
    else
        log(
            ns,
            `INFO: --reserve-percent is set to ${pctReservedMoney * 100}%: ` +
    ...
    do {
        absReservedMoney =
            options["absolute-reserve"] != null
                ? Number(options["absolute-reserve"])
                : Number(ns.read("reserve.txt") || 0);
        await tryToBuyBestServerPossible(ns);
        if (keepRunning) await ns.sleep(options["interval"]);
    } while (keepRunning);
}
...
function setStatus(ns, logMessage) {
    return logMessage != lastStatus
        ? ns.print((lastStatus = logMessage))
        : false;
}
...
async function tryToBuyBestServerPossible(ns) {
    ...
    let rootedServers = await getNsDataThroughFile(
        ns,
        "scanAllServers(ns).filter(s => ns.hasRootAccess(s))",
        "/Temp/rooted-servers.txt",
    );
    // Gether the list of all purchased servers.
    let purchasedServers = null;
    try {
        purchasedServers = await getNsDataThroughFile(
            ns,
    ...
    if (purchasedServers == null)
        // Early game, if we have insufficient RAM (1.05 GB cloud.getServerNames + 1.6 base cost), we can fall-back to guessing based on their name
        purchasedServers = rootedServers.filter((s) =>
            s.startsWith(purchasedServerName),
        );
    ...
    const likelyHacknet = rootedServers.filter(
        (s) => s.startsWith("hacknet-node-") || s.startsWith("hacknet-server-"),
    );
    if (likelyHacknet.length > 0) {
        const totalHacknetUsedRam = likelyHacknet.reduce(
            (t, s) => t + ns.getServerUsedRam(s),
            0,
        );
        if (totalHacknetUsedRam == 0) {
            rootedServers = rootedServers.filter(
                (s) => !likelyHacknet.includes(s),
            );
            log(
                ns,
                `Removing ${likelyHacknet.length} hacknet servers from RAM statistics since they are not being utilized.`,
            );
        } else if (!keepRunning)
            log(
                ns,
                `We are currently using ${formatRam(totalHacknetUsedRam)} of hacknet RAM, so including hacknet in our utilization stats.`,
            );
    }

    const totalMaxRam = rootedServers.reduce(
        (t, s) => t + ns.getServerMaxRam(s),
        0,
    );
    const totalUsedRam = rootedServers.reduce(
        (t, s) => t + ns.getServerUsedRam(s),
        0,
    );
    if (options["utilization-trigger"] > 0) {
        ...
        setStatus(
            ns,
            `Using ${Math.round(totalUsedRam).toLocaleString("en")}/${formatRam(totalMaxRam)} (` +
    ...
    let cashMoney = await getNsDataThroughFile(
        ns,
    ...
    if (options["reserve-by-time"]) {
        // Option to vary pctReservedMoney by time since augment.
        ...
        const timeSinceLastAug =
            Date.now() -
            (await getNsDataThroughFile(ns, "ns.getResetInfo()")).lastAugReset;
        const t = timeSinceLastAug / (60 * 60 * 1000); // Time since last aug, in hours.
        const decayFactor = options["reserve-by-time-decay-factor"];
        pctReservedMoney =
            1.0 -
            (1.0 - options["reserve-percent"]) * Math.pow(1 - decayFactor, t);
        if (!keepRunning)
            log(
                ns,
                `After spending ${formatDuration(timeSinceLastAug)} in this augmentation, reserve % has grown from ` +
                    `${(options["reserve-percent"] * 100).toFixed(1)}% to ${(pctReservedMoney * 100).toFixed(1)}%`,
            );
    }

    let spendableMoney = Math.min(
        budget,
        cashMoney * (1.0 - pctReservedMoney),
        cashMoney - absReservedMoney,
    );
    if (spendableMoney <= 0.01) {
        if (!keepRunning)
            // Show a more detailed log if we aren't running continuously

            return setStatus(
                ns,
                `${prefix}all player cash (${formatMoney(cashMoney)}) is currently reserved (budget: ${formatMoney(budget)}, ` +
        ...
        return setStatus(
            ns,
            `${prefix}all player cash is currently reserved (budget: ${formatMoney(budget)}, ` +
    ...
    for (; exponentLevel < maxPurchasableServerRamExponent; exponentLevel++)
        if (costByRamExponent[exponentLevel + 1] > spendableMoney) break;
    ...
    if (spendableMoney < cost)
        return setStatus(
            ns,
            `${prefix}spendableMoney (${formatMoney(spendableMoney)}) is less than the cost ` +
    ...
    if (exponentLevel < minRamExponent)
        return setStatus(
            ns,
            `${prefix}The highest ram exponent we can afford (2^${exponentLevel} for ${formatMoney(cost)}) on our budget ` +
    ...
    if (exponentLevel < maxPurchasableServerRamExponent) {
        ...
        if (maxRamPossibleToBuy < ns.getServerMaxRam("home") * homeThreshold)
            return setStatus(
                ns,
                `${prefix}the most RAM we can buy (${formatRam(maxRamPossibleToBuy)}) on our budget of ${formatMoney(spendableMoney)} ` +
    ...
    const maxPurchasableServerRam = Math.pow(
        2,
        maxPurchasableServerRamExponent,
    );
    const ramByServer = Object.fromEntries(
        purchasedServers.map((server) => [server, ns.getServerMaxRam(server)]),
    );
    let [worstServerName, worstServerRam] = purchasedServers.reduce(
        ([minS, minR], s) =>
            ramByServer[s] < minR ? [s, ramByServer[s]] : [minS, minR],
        [null, maxPurchasableServerRam],
    );
    let [bestServerName, bestServerRam] = purchasedServers.reduce(
        ([maxS, maxR], s) =>
            ramByServer[s] > maxR ? [s, ramByServer[s]] : [maxS, maxR],
        [null, 0],
    );
    ...
    if (
        worstServerName != null &&
        maxRamPossibleToBuy < worstServerRam &&
        !options["allow-worse-purchases"]
    )
        return setStatus(
            ns,
            `${prefix}the most RAM we can buy (${formatRam(maxRamPossibleToBuy)}) on our budget of ` +
    ...
    if (
        bestServerRam != null &&
        maxRamPossibleToBuy < bestServerRam &&
        !options["allow-worse-purchases"]
    )
        return setStatus(
            ns,
    ...
    if (purchasedServers.length >= maxPurchasedServers) {
        if (worstServerRam == maxPurchasableServerRam) {
            ...
            return setStatus(
                ns,
                `INFO: We are at the max number of servers ${maxPurchasedServers}, ` +
        ...
        isUpgrade = true;
        purchasedServer = (await getNsDataThroughFile(
            ns,
            `ns.cloud.upgradeServer(ns.args[0], ns.args[1])`,
            null,
            [worstServerName, maxRamPossibleToBuy],
        ))
            ? worstServerName
            : "";
    } else {
    ...
    if (!purchasedServer)
        setStatus(
            ns,
            `${prefix}Could not ${isUpgrade ? "upgrade" : "purchase"} a server with ${formatRam(maxRamPossibleToBuy)} ` +
                `RAM for ${formatMoney(cost)} with a budget of ${formatMoney(spendableMoney)}.`,
        );
    else {
        log(
            ns,
            `SUCCESS: ${isUpgrade ? "Upgraded" : "Purchased"} server ${purchasedServer} with ${formatRam(maxRamPossibleToBuy)} ` +
                `RAM for ${formatMoney(cost)} (budget was ${formatMoney(spendableMoney)})`,
            true,
            "success",
        );
        budget -= cost;
