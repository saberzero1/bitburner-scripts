...
export async function main(ns) {
    ...
    if (timeSinceLastAug > 20 * 60 * 1000) {
        return log(
            ns,
            `WARNING: It's been ${formatDuration(timeSinceLastAug)} since your last reset. ` +
        ...
        log(
            ns,
            `Resetting to get a list of instantly-available invites...`,
            true,
        );
    ...
    if (invites.length < 10)
        return log(
            ns,
            `WARNING: You only have invites to join ${invites.length} factions. ` +
    ...
    log(
        ns,
        `SUCCESS: Beginning soft-reset loop. It may look like nothing's happening, but watch your intelligence stat...`,
        true,
        "success",
    );
}
