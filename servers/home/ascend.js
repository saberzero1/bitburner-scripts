...
export function autocomplete(data, args) {
    ...
    if (["--on-reset-script"].includes(lastFlag)) return data.scripts;
    return [];
}
...
export async function main(ns) {
    ...
    if (!(4 in dictSourceFiles))
        return log(
            ns,
            "ERROR: You cannot automate installing augmentations until you have unlocked singularity access (SF4).",
            true,
            "error",
        );
    ns.disableLog("sleep");
    if (options["prioritize-augmentations"])
        log(
            ns,
            "INFO: The --prioritize-augmentations flag is deprecated, as this is now the default behaviour. Use --prioritize-home-ram to get back the old behaviour.",
        );
    ...
    if (hasTixApiAccess) {
        ...
        const countOwnedStocks = async () =>
            await getNsDataThroughFile(
                ns,
                `ns.args.map(sym => ns.stock.getPosition(sym))` +
                    `.reduce((t, stk) => t + (stk[0] + stk[2] > 0 ? 1 : 0), 0)`,
                "/Temp/owned-stocks.txt",
                stkSymbols,
            );
        let ownedStocks;
        do {
            log(
                ns,
                `INFO: Waiting for ${ownedStocks} owned stocks to be sold...`,
                false,
                "info",
            );
            ...
            if (pid) await waitForProcessToComplete(ns, pid, true);
                ...
                log(
                    ns,
                    `ERROR: Failed to run "stockmaster.js --liquidate" to sell ${ownedStocks} owned stocks. Will try again soon...`,
                    false,
                    "true",
                );
            await ns.sleep(1000);
    ...
    const spendOnHomeRam = async () => {
        ...
        pid = ns.run(
            getFilePath("Tasks/ram-manager.js"),
            1,
            "--reserve",
            "0",
            "--budget",
    ...
    if (13 in dictSourceFiles) {
        ...
        let isInBn8 =
            8 ===
            (await getNsDataThroughFile(ns, `ns.getResetInfo()`)).currentNode;

        if (options["skip-staneks-gift"])
            log(
                ns,
                "INFO: --skip-staneks-gift was set, we will not accept it.",
            );
        else if (isInBn8) {
            log(
                ns,
                "INFO: Stanek's gift is useless in BN8, setting the --skip-staneks-gift argument automatically.",
            );
            options["skip-staneks-gift"] = true;
        } else {
            log(
                ns,
                "Accepting Stanek's Gift (if this is the first reset)...",
                true,
                "info",
            );
            const haveStanek = await getNsDataThroughFile(
                ns,
            ...
            if (haveStanek)
                log(
                    ns,
                    "INFO: Confirmed that we have Stanek's Gift",
                    true,
                    "info",
                );
            else {
                log(
                    ns,
                    "WARNING: It looks like we can't get Stanek's Gift. (Did you manually purchase some augmentations?)",
                    true,
                    "warning",
                );
    ...
    if (options["skip-staneks-gift"]) {
        log(
            ns,
            "INFO: Sending the --ignore-stanek argument to faction-manager.js",
        );
    ...
    let purchasedAugmentations = await getNsDataThroughFile(
        ns,
    ...
    let noAugsToInstall =
        purchasedAugmentations.length == installedAugmentations.length;
    if (noAugsToInstall && !options["allow-soft-reset"])
        return log(
            ns,
            `ERROR: See above faction-manager.js logs - there are no new purchased augs. ` +
                `Specify --allow-soft-reset to proceed without any purchased augs.`,
            true,
            "error",
        );
    ...
    let hasStockApi = await getNsDataThroughFile(
        ns,
    ...
    if (invites.length > 0) {
        pid = await runCommand(
            ns,
            "ns.args.forEach(f => ns.singularity.joinFaction(f))",
            "/Temp/join-factions.js",
            invites,
        );
    ...
    try {
        ...
        if (hasCorp) {
            const corpInfo = await getNsDataThroughFile(
                ns,
            ...
            if (corpInfo.public && corpInfo.issuedShares > 0) {
                ...
                const affordableShares = Math.floor(
                    player.money / corpInfo.sharePrice,
                );
                const sharesToBuy = Math.min(
                    corpInfo.issuedShares,
                    affordableShares,
                );
                if (sharesToBuy > 0) {
                    log(
                        ns,
                        `Buying back ${sharesToBuy} corporation shares...`,
                        true,
                        "info",
                    );
                    await getNsDataThroughFile(
                        ns,
                        "ns.corporation.buyBackShares(ns.args[0])",
                        null,
                        [sharesToBuy],
                    );
                }
    ...
    let money = 0,
        lastMoney = 0,
        ticksWithoutPurchases = 0;
    const maxWait = Date.now() + options["max-wait-time"];
    while (
        ticksWithoutPurchases <
            options["ticks-to-wait-for-additional-purchases"] &&
        Date.now() < maxWait
    ) {
        const start = Date.now(); // Used to wait for the game to tick.
        const refreshMoney = async () =>
            (money = await getNsDataThroughFile(
                ns,
        ...
        ticksWithoutPurchases =
            money < lastMoney ? 0 : ticksWithoutPurchases + 1;
        lastMoney = money;
    }

    const factionInvites = await getNsDataThroughFile(
        ns,
        "ns.singularity.checkFactionInvitations()",
    );
    ...
    const canExport = await getNsDataThroughFile(
        ns,
    ...
    if (options.reset || options["install-augmentations"]) {
        ...
        const resetScript =
            options["on-reset-script"] ??
            // Default script (if none is specified) is stanek.js if we have it (which in turn will spawn daemon.js when done)
            (purchasedAugmentations.includes(`Stanek's Gift - Genesis`)
                ? getFilePath("stanek.js")
                : getFilePath("daemon.js"));
        if (noAugsToInstall)
            await runCommand(ns, `ns.singularity.softReset(ns.args[0])`, null, [
                resetScript,
            ]);
        else
            await runCommand(
                ns,
                `ns.singularity.installAugmentations(ns.args[0])`,
                null,
                [resetScript],
            );
    } else
        log(
            ns,
            `SUCCESS: Ready to ascend. In the future, you can run with --reset (or --install-augmentations) ` +
                `to actually perform the reset automatically.`,
            true,
            "success",
        );
}
