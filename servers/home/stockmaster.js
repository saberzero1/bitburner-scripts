...
export async function main(ns) {
    ...
    const hasTixApiAccess = await getNsDataThroughFile(
        ns,
    ...
    if (runOptions.l || runOptions.liquidate) {
        ...
        log(
            ns,
            "INFO: Killing any other stockmaster processes...",
            false,
            "info",
        );
        await runCommand(
        ...
        log(
            ns,
            "INFO: Checking for and liquidating any stocks...",
            false,
            "info",
        );
    ...
    if (!hasTixApiAccess) {
        ...
        log(
            ...
            `INFO: You are missing stock market API access. (NOTE: This is granted for free once you have SF8). ` +
                `Waiting until we have the 5b needed to buy it. (Run with --disable-purchase-tix-api to disable this feature.)`,
            true,
        );
        do {
            ...
            try {
                ...
                success = await tryGetStockMarketAccess(
                    ns,
        ...
        try {
            ...
            if (
                ...
                (await tryGet4SApi(
                    ns,
                    playerStats,
                    corpus * (options["buy-4s-budget"] - fracH) - reserve,
                ))
            )
                ...
                if (
                    ...
                    if (pre4s && stk.ticksHeld < pre4sMinHoldTime) {
                        if (!stk.warnedBadPurchase)
                            log(
                                ns,
                                `WARNING: Thinking of selling ${stk.sym} with ER ${formatBP(stk.absReturn())}, but holding out as it was purchased just ${stk.ticksHeld} ticks ago...`,
                            );
                ...
                for (const stk of allStocks.sort(purchaseOrder)) {
                    ...
                    if (
                        ...
                        (stk.lastInversion < minTickHistory ||
                            Math.abs(stk.prob - 0.5) <
                                pre4sBuyThresholdProbability)
                    )
                        continue;
                    ...
                    let budget = Math.min(
                        ...
                        maxHoldings * (diversification + stk.spread_pct) -
                            stk.positionValue() * (1.01 + stk.spread_pct),
                    );
                    let purchasePrice = stk.bullish()
                        ? stk.ask_price
                        : stk.bid_price; // Depends on whether we will be buying a long or short position
                    let affordableShares = Math.floor(
                        (budget - commission) / purchasePrice,
                    );
...
async function getStockInfoDict(ns, stockFunction) {
    ...
    if (allStockSymbols == null)
        throw new Error(
            `No WSE API Access yet, this call to ns.stock.${stockFunction} is premature.`,
        );
...
async function initAllStocks(ns) {
    ...
    return allStockSymbols.map((s) => ({
        ...
        positionValueShort: function () {
            return (
                this.sharesShort * (2 * this.boughtPriceShort - this.ask_price)
            );
...
async function refresh(ns, has4s, allStocks, myStocks) {
    ...
    const dictPositions = mock
        ? null
        : await getStockInfoDict(ns, "getPosition");
    const ticked = allStocks.some(
        (stk) => stk.ask_price != dictAskPrices[stk.sym],
    ); // If any price has changed since our last update, the stock market has "ticked"
    ...
    if (ticked) {
        if (Date.now() - lastTick < expectedTickTime - sleepInterval) {
            if (Date.now() - lastTick < catchUpTickTime - sleepInterval) {
                ...
                log(
                    ns,
                    `WARNING: Detected a stock market tick after only ${formatDuration(Date.now() - lastTick)}, but expected ~${formatDuration(expectedTickTime)}. ` +
                        (changedPrices.length >= 33
                            ? "(All stocks updated)"
                            : `The following ${changedPrices.length} stock prices changed: ${changedPrices
                                  .map(
                                      (stk) =>
                                          `${stk.sym} ${formatMoney(stk.ask_price)} -> ${formatMoney(dictAskPrices[stk.sym])}`,
                                  )
                                  .join(", ")}`),
                    false,
                    "warning",
                ...
                log(
                    ns,
                    `INFO: Detected a rapid stock market tick (${formatDuration(Date.now() - lastTick)}), likely to make up for lag / offline time.`,
                );
        }
        lastTick = Date.now();
    }
    ...
    for (const stk of allStocks) {
        ...
        stk.boughtPriceShort = mock
            ? stk.boughtPrice_short || 0
            : stk.position[3];
...
async function updateForecast(ns, allStocks, has4s) {
    ...
    for (const stk of allStocks) {
            ...
            stk.vol = stk.priceHistory.reduce(
                (max, price, idx) =>
                    Math.max(
                        ...
                        idx == 0
                            ? 0
                            : Math.abs(stk.priceHistory[idx - 1] - price) /
                                  price,
                    ),
                0,
            );
        ...
        let preNearTermWindowProb = forecast(
            stk.priceHistory.slice(
                nearTermForecastWindowLength,
                nearTermForecastWindowLength + marketCycleLength,
            ),
    ...
    if (inversionsDetected.length > 0) {
        ...
        if (
            ...
            const newPredictedCycleTick = has4s
                ? 0
                : nearTermForecastWindowLength; // By the time we've detected it, we're this many ticks past the cycle start
            ...
            inversionAgreementThreshold = Math.max(
                14,
    ...
    for (const stk of allStocks) {
        ...
        stk.longTermForecast = forecast(
            stk.priceHistory.slice(0, probWindowLength),
        );
        if (!has4s) {
            stk.prob = stk.longTermForecast;
            stk.probStdDev = Math.sqrt(
                (stk.prob * (1 - stk.prob)) / probWindowLength,
            );
        }
        const signalStrength =
            1 +
            (stk.bullish()
                ? (stk.nearTermForecast > stk.prob ? 1 : 0) +
                  (stk.prob > 0.8 ? 1 : 0)
                : (stk.nearTermForecast < stk.prob ? 1 : 0) +
                  (stk.prob < 0.2 ? 1 : 0));
        if (prepSummary) {
    ...
    if (prepSummary) {
        summary +=
            `Market day ${detectedCycleTick + 1}${marketCycleDetected ? "" : "?"} of ${marketCycleLength} (${marketCycleDetected ? ((100 * inversionAgreementThreshold) / 19).toPrecision(2) : "0"}% certain) ` +
...
let launchSummaryTail = async (ns) => {
    ...
    await runCommand(
        ns,
        `ns.disableLog('sleep'); tail(ns); let lastRead = '';
        while (true) {
...
let buyStockWrapper = async (ns, sym, numShares) =>
    await transactStock(ns, sym, numShares, "buyStock"); // ns.stock.buyStock(sym, numShares);
let buyShortWrapper = async (ns, sym, numShares) =>
    await transactStock(ns, sym, numShares, "buyShort"); // ns.stock.buyShort(sym, numShares);
let sellStockWrapper = async (ns, sym, numShares) =>
    await transactStock(ns, sym, numShares, "sellStock"); // ns.stock.sellStock(sym, numShares);
...
let transactStock = async (ns, sym, numShares, action) =>
    await getNsDataThroughFile(
        ns,
        `ns.stock.${action}(ns.args[0], ns.args[1])`,
        null,
        [sym, numShares],
    );
...
async function doBuy(ns, stk, sharesToBuy) {
    ...
    if (price == 0) {
        ...
        if (playerMoney < sharesToBuy * expectedPrice)
            log(
                ns,
                `WARN: Failed to ${long ? "buy" : "short"} ${stk.sym} because money just recently dropped to ${formatMoney(playerMoney)} and we can no longer afford it.`,
                noisy,
            );
        else
            log(
                ns,
                `ERROR: Failed to ${long ? "buy" : "short"} ${stk.sym} @ ${formatMoney(expectedPrice)} (0 was returned) despite having ${formatMoney(playerMoney)}.`,
                true,
                "error",
            );
        return 0;
        ...
        log(
            ns,
            `WARNING: ${long ? "Bought" : "Shorted"} ${stk.sym} @ ${formatMoney(price)} but expected ${formatMoney(expectedPrice)} (spread: ${formatMoney(stk.spread)})`,
            false,
            "warning",
        );
    ...
    if (mock && long)
        stk.boughtPrice =
            (stk.boughtPrice * stk.sharesLong + price * sharesToBuy) /
            (stk.sharesLong + sharesToBuy);
    if (mock && !long)
        stk.boughtPriceShort =
            (stk.boughtPriceShort * stk.sharesShort + price * sharesToBuy) /
            (stk.sharesShort + sharesToBuy);
...
async function doSellAll(ns, stk) {
    ...
    log(
        ns,
        `${profit > 0 ? "SUCCESS" : "WARNING"}: Sold all ${formatNumberShort(sharesSold, 3, 3).padStart(5)} ${stk.sym.padEnd(5)} ${long ? " long" : "short"} positions ` +
    ...
    if (price == 0) {
        log(
            ns,
            `ERROR: Failed to sell ${sharesSold} ${stk.sym} ${long ? "shares" : "shorts"} @ ${formatMoney(expectedPrice)} - 0 was returned.`,
            true,
            "error",
        );
        return 0;
        ...
        log(
            ns,
            `WARNING: Sold ${stk.sym} ${long ? "shares" : "shorts"} @ ${formatMoney(price)} but expected ${formatMoney(expectedPrice)} (spread: ${formatMoney(stk.spread)})`,
            false,
            "warning",
        );
...
let formatBP = (fraction) =>
    formatNumberShort(fraction * 100 * 100, 3, 2) + " BP";
...
function doStatusUpdate(ns, stocks, myStocks, hudElement = null) {
    ...
    let liquidation_value = myStocks.reduce(
        (sum, stk) =>
            sum - (stk.owned() ? commission : 0) + stk.positionValue(),
        0,
    );
    let status =
        `Long ${myStocks.filter((s) => s.sharesLong > 0).length}, Short ${myStocks.filter((s) => s.sharesShort > 0).length} of ${stocks.length} stocks ` +
        (myStocks.length == 0
            ? ""
...
async function liquidate(ns) {
    ...
    for (const sym of allStockSymbols) {
        ...
        if (sharesLong > 0)
            totalRevenue +=
                (await sellStockWrapper(ns, sym, sharesLong)) * sharesLong -
                commission;
        if (sharesShort > 0)
            totalRevenue +=
                (2 * avgShortCost -
                    (await sellShortWrapper(ns, sym, sharesShort))) *
                    sharesShort -
                commission;
    }
    log(
        ns,
        `Sold ${totalSharesLong.toLocaleString("en")} long shares and ${totalSharesShort.toLocaleString("en")} short shares ` +
...
async function tryGet4SApi(ns, playerStats, budget) {
    ...
    if (!has4S) {
        if (await tryBuy(ns, "purchase4SMarketData"))
            ...
            log(
                ns,
                "ERROR attempting to purchase 4SMarketData!",
                false,
                "error",
            );
    }
    if (await tryBuy(ns, "purchase4SMarketDataTixApi")) {
        ...
        log(
            ns,
            "ERROR attempting to purchase 4SMarketDataTixApi!",
            false,
            "error",
        );
    }
...
async function tryGetStockMarketAccess(ns, budget) {
    ...
    if (!hasWSE) {
        if (await tryBuy(ns, "purchaseWseAccount"))
            ...
            log(
                ns,
                "ERROR attempting to purchase WSE account!",
                false,
                "error",
            );
    }
