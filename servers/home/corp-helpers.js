 ...
 */
export function calculateWarehouseSpaceBudget(
    industryType,
    warehouseSize,
    productionRate = 10,
) {
    ...
    const boostSpace = Math.max(
        0,
        (warehouseSize - reservedForProduction) * 0.9,
    );
...
export function calculateOptimalBoostMaterials(industryType, availableSpace) {
    ...
    const result = {
        "Real Estate": Math.floor(
            (availableSpace * realEstateWeight) / MATERIAL_SIZES["Real Estate"],
        ),
        Hardware: Math.floor(
            (availableSpace * hardwareWeight) / MATERIAL_SIZES["Hardware"],
        ),
        Robots: Math.floor(
            (availableSpace * robotsWeight) / MATERIAL_SIZES["Robots"],
        ),
        "AI Cores": Math.floor(
            (availableSpace * aiCoresWeight) / MATERIAL_SIZES["AI Cores"],
        ),
    };
 ...
 */
export function calculateOptimalEmployeeDistribution(
    totalEmployees,
    forProducts = false,
) {
    if (totalEmployees < 4) {
    ...
    if (forProducts) {
        ...
        return {
            Operations: Math.floor(totalEmployees * 0.2),
        ...
        return {
            ...
            Business: Math.floor(totalEmployees * 0.15),
            Management: Math.floor(totalEmployees * 0.15),
            "Research & Development": Math.floor(totalEmployees * 0.1),
        };
    }
}
 ...
 */
export function estimateOfficeProductivity(
    employeeJobs,
    avgMorale = 100,
    avgEnergy = 100,
) {
    ...
    const opsProdMult =
        0.6 * avgStat + 0.1 * avgStat + avgExp + 0.5 * avgStat + avgStat;
    ...
    const prod =
        (Math.pow(opsProd, 0.4) + Math.pow(engrProd, 0.3)) * mgmtFactor;
 ...
 */
export function calculateSmartSupplyQuantities(
    divData,
    warehouse,
    materials,
...
export function calculateOptimalPrice(
    ...
    const businessFactor =
        Math.pow(1 + businessProd, 0.26) + (1 + businessProd) / 10000;
    ...
    const advertisingFactor =
        Math.pow(awareness, industry.advertisingFactor) *
    ...
    const sellAmt =
        productionAmount || item.productionAmount || item.stored / 10 || 1;
...
export const INVESTMENT_ROUNDS = {
    1: { sharePercent: 0.1, multiplier: 3.0, description: "Seed funding" },
    2: { sharePercent: 0.35, multiplier: 2.0, description: "Series A" },
    3: { sharePercent: 0.25, multiplier: 2.0, description: "Series B" },
    4: { sharePercent: 0.2, multiplier: 1.5, description: "Pre-IPO" },
};
 ...
 */
export function shouldAcceptInvestment(
    offer,
    round,
    customMinimum = 0,
    currentFunds = null,
 ...
 * From game source
 */
export function calculateOptimalPartyCost(
    currentMorale,
    targetMorale = 100,
    perfMult = 1.002,
) {
...
export function getUpgradeCost(
    ...
    return (
        (basePrice *
            (Math.pow(priceMult, targetLevel) -
                Math.pow(priceMult, currentLevel))) /
        (priceMult - 1)
    );
}
...
export function getOfficeSizeUpgradeCost(currentSize, targetSize) {
    ...
    return (
        (basePrice *
            (Math.pow(1.09, targetSize / 3) -
                Math.pow(1.09, currentSize / 3))) /
        0.09
    );
}
...
export function diagnoseZeroProduction(divData, warehouse, office, materials) {
    ...
    if (industry?.outputMaterials) {
        for (const mat of industry.outputMaterials) {
            ...
            if (
                matData &&
                (!matData.desiredSellPrice || matData.desiredSellPrice === "0")
            ) {
                issues.push({
                    severity: "medium",
 ...
 */
export function checkExportDilution(
    destQuality,
    destStored,
    srcQuality,
    srcAmount,
    ...
    if (destStored === 0) {
        ...
        return {
            wouldDilute: false,
            newQuality: srcQuality,
            qualityChange: srcQuality,
        };
    }

    const newQuality = Math.max(
        0.1,
        (destQuality * destStored + srcQuality * srcAmount) /
            (destStored + srcAmount),
    );
...
export function checkProductReadiness(
    ...
    return {
        ...
        effectiveRatingPercent: isReady
            ? 100
            : (avgInputQuality / threshold) * 100,
    };
}
 ...
 * @returns {Object} Job assignments
 */
export function calculateQualityFocusedDistribution(
    totalEmployees,
    focus = "quality",
) {
    if (totalEmployees < 4) {
    ...
    if (focus === "quality") {
        ...
        return {
            ...
            Management: Math.floor(totalEmployees * 0.1),
 ...
 */
export function determineCityRole(
    cityQuality,
    bestQuality,
    qualityThreshold = 10,
) {
    if (cityQuality >= bestQuality * 0.95) {
        ...
        return {
            role: "secondary",
            reason: "Good quality - secondary export source",
        };
    } else {
        // Below threshold - sell locally, don't export
        return {
            role: "local",
            reason: "Low quality - sell locally to avoid dilution",
        };
    }
}
 ...
 */
export function calculateMaxExportWithoutDilution(
    destQuality,
    destStored,
    srcQuality,
    minQuality,
    ...
    return (
        (destStored * (destQuality - minQuality)) / (minQuality - srcQuality)
    );
}
