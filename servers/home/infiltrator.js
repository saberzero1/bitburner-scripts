...
const argsSchema = [
    ...
    ["max-loop", 10], //15// Max Loops per Faction
...
export async function main(ns) {
    ...
    if (!options["info"]) ns.ui.openTail();
...
async function infiltrateForFaction(ns, stack) {
    ...
    while (currentReputation < highestRepAug) {
        ...
        log(
            ns,
            `Infiltrating ${stack.target.name} at loop ${loop} to push ${stack.faction}´s Reputation`,
        );
        let completet = await infiltrate(
            ns,
            city,
            stack.target.name,
            stack.faction,
...
async function infiltrateForMoney(ns, player, maxMoney, target, stock = true) {
    ...
    if (!target) {
        let locationsfiltered = locations.filter(
            (location) => location.reward.sellCash > moneyNeed,
        );
        if (locationsfiltered.length > 0) {
            let min = Math.min(
                ...locationsfiltered.map(
                    (location) => location.reward.sellCash,
                ),
            );
            target = locations.filter(
                (location) => location.reward.sellCash === min,
            )[0];
        } else {
            let max = Math.max(
    ...
    log(
        ns,
        `Infiltrate ${target.name} to get ${formatMoney(moneyNeed)}, need ${Math.ceil(moneyNeed / target.reward.sellCash)} loops`,
        verbose,
    );
    ...
    while (currentMoney < maxMoney) {
        ...
        if (city && player.money < 2e5) {
            ...
            log(
                ns,
                `Player money is too low to travel (${formatMoney(player.money, 6, 1)}), will Infiltrate 1x ${cityTarget.name}`,
                verbose,
            );
            await infiltrateForMoney(ns, player, 2e5, cityTarget, false);
            player = await getPlayerInfo(ns);
        ...
        log(
            ns,
            `Infiltrating ${target.name} at loop ${loop} to get ${formatMoney(maxMoney)} (currently at ${formatMoney(currentMoney)})`,
            verbose,
        );
...
async function infiltrate(ns, city, target, faction) {
    ...
    try {
        ...
        ); // Workaraound, sometimes click on "City" wil not show the right City
        await click(
            ns,
            await findRetry(
                ns,
...
function getTarget(ns, locations, faction, target = undefined, loop = 1) {
    ...
    log(
        ...
        `Faction ${factionName} need ${formatNumberShort(repNeed, 5)} Rep, infiltrate ${loop}x ${target.name} (${formatNumberShort(target.reward.tradeRep, 6, 1)}/loop)` +
            (target2
                ? ` and 1x ${target2.name} (${formatNumberShort(target2.reward.tradeRep, 6, 1)})/loop)`
                : ""),
        verbose,
    );
...
function launchScriptHelper(
    ...
    ns.ui.openTail(); // If we're going to be launching scripts, show our tail window so that we can easily be killed if the user wants to interrupt.
    const pid = ns.run(
