...
export async function main(ns) {
    ...
    if (sleepDuration < 0) {
        if (!silentMisfires)
            ns.toast(
                `Misfire: Grow started ${-sleepDuration} ms too late. ${JSON.stringify(ns.args)}`,
                "warning",
            );
        sleepDuration = 0;
    }
    ...
    if (loopingMode) hgwOptions.additionalMsec += duration * 0.25; // (duration*4/3.2 (time-to-weaken) - duration)
    ...
    do {
        ...
        if (growPct == 0 && !silentMisfires)
            ns.toast(
                `Misfire: Grow achieved no growth. ${JSON.stringify(ns.args)}`,
                "warning",
            );
