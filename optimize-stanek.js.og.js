...
async function planFragments(ns, width, height, statFrags, boosterFrags) {
    ...
    const statFragsKeys = [...statFrags]
        .sort(
            (a, b) =>
                statFragsPlacements.get(b.id).length -
                statFragsPlacements.get(a.id).length,
        )
        .map((frag) =>
...
function rotations(fragment) {
    ...
    for (const rotMask of rotMasks)
        rotMask.mask = rotMask.mask
            .map((row, y) =>
                row.map((filled, x) => (filled ? [x, y] : undefined)),
            )
            .flat()
