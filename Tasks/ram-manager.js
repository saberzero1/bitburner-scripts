...
export async function main(ns) {
    ...
    const freeRam = Math.max(0, ns.getServerMaxRam("home") - ns.getServerUsedRam("home"));
    if (freeRam < 7) {
        return log(ns, `INFO: Skipping ram-manager (free RAM ${formatRam(freeRam)} too low for temp scripts)`);
    }
    const money = await safeGetNsData(ns, `ns.getServerMoneyAvailable(ns.args[0])`, ["home"]);
    ...
    do {
        let cost = await safeGetNsData(ns, `ns.singularity.getUpgradeHomeRamCost()`);
        let currentRam = await safeGetNsData(ns, `ns.getServerMaxRam(ns.args[0])`, ["home"]);
        ...
        const newMaxRam = await safeGetNsData(ns, `ns.getServerMaxRam(ns.args[0])`, ["home"]);
...
async function safeGetNsData(ns, command, args = null) {
    try {
        ...
        log(ns, `WARNING: ram-manager skipped '${command}' due to insufficient RAM`, false, 'warning');
        return null;
    }
}
