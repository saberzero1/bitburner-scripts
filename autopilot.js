import {
    ...
    formatMoney, formatDuration, autoRetry
} from './helpers.js'
...
const argsSchema = [ // The set of all command line arguments
    ...
    ['install-at-aug-count', 6], //11// Automatically install when we can afford this many new augmentations (with NF only counting as 1)
    ['install-at-aug-plus-nf-count', 9], //14// or... automatically install when we can afford this many augmentations including additional levels of Neuroflux
    ...
    ['reduced-aug-requirement-per-hour', 3], //0.5// For every hour since the last reset, require this many fewer augs to install.
...
async function maybeDoInfiltration(ns, player, stocksValue) {
	...
	if ((player.money + stocksValue) < 5E15 && player.bitNodeN != 8 /*&& bitnodeMults?.InfiltrationMoney > 0.5 && !ranGetMoney*/ && stack?.length === 0){
